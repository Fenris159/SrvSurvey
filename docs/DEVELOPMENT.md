...
The branch is versioned as **SrvSurvey-XP 2.1.3.0-rc.51**. Its one-time bridge
tag is `xp-v2.1.3.0-rc.51`, package manifests use `SrvSurvey.XP`, and
distributable filenames begin with `SrvSurvey-XP-2.1.3.0-rc.51`. The assembly
...
Do not dispatch RC51 until its pull request has merged into
...
pwsh ./scripts/Resolve-CrossPlatformReleaseContract.ps1 -Version 2.1.3.0-rc.51
...
The resolver output must report `xp-v2.1.3.0-rc.51` and schema 1. After the
workflow succeeds, verify rather than replace its published assets:

```console
gh release view xp-v2.1.3.0-rc.51 \
  --repo Fenris159/SrvSurvey \
  --json tagName,isDraft,isPrerelease,targetCommitish,assets
gh release download xp-v2.1.3.0-rc.51 \
  --repo Fenris159/SrvSurvey \
  --pattern release-index.json \
  --dir artifacts/verify-rc51
pwsh -Command '$index = Get-Content artifacts/verify-rc51/release-index.json -Raw | ConvertFrom-Json; if ($index.schemaVersion -ne 1 -or $index.packages.Count -ne 2) { throw "RC51 bridge index contract failed." }'
```

For RC52, increment the desktop project version and release notes to
`2.1.3.0-rc.52`, merge those changes, and repeat the workflow commands with the
same branch and development channel. The resolver must then report
`xp2-v2.1.3.0-rc.52` and schema 2. Continue sequential RC version increments in
the `xp2-v` namespace; do not create any additional `xp-v` tag.
