...
The branch is versioned as **SrvSurvey-XP 2.1.3.0-rc.54**. Its development tag
is `xp2-v2.1.3.0-rc.54`, package manifests use `SrvSurvey.XP`, and distributable
filenames begin with `SrvSurvey-XP-2.1.3.0-rc.54`. The assembly
...
pwsh ./scripts/Resolve-CrossPlatformReleaseContract.ps1 -Version 2.1.3.0-rc.54
...
The resolver output must report `xp2-v2.1.3.0-rc.54` and schema 2. After the
workflow succeeds, verify rather than replace its published assets:

```console
gh release view xp2-v2.1.3.0-rc.54 \
  --repo Fenris159/SrvSurvey \
  --json tagName,isDraft,isPrerelease,targetCommitish,assets
gh release download xp2-v2.1.3.0-rc.54 \
  --repo Fenris159/SrvSurvey \
  --pattern release-index.json \
  --dir artifacts/verify-rc54
pwsh -Command '$index = Get-Content artifacts/verify-rc54/release-index.json -Raw | ConvertFrom-Json; if ($index.schemaVersion -ne 2 -or $index.packages.Count -ne 3) { throw "RC54 release index contract failed." }'
```

Confirm that legacy clients still select `xp-v2.1.3.0-rc.51` and an RC51 client
selects `xp2-v2.1.3.0-rc.54`. Continue sequential RC version increments in the
