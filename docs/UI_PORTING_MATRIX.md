...
| `FormPlayComms` / `FormPlayComms2` | Quest communications | Legacy local commander/development-quest state and definition loading is implemented read-only with byte-preservation coverage; Raven APIs, Lua/journal runtime, player workspace, and compact quest indicator remain |
