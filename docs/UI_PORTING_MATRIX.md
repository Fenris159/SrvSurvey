...
| Settings | `FormSettings`, `FormSetKeyChord`, `FormAdjustOverlay` | Themes, paths, overlays, input and privacy | Raven themes, checksum-verified legacy profile import with empty/unreadable-source filtering, live-monitor shutdown, concurrent-write rollback, immediate non-destructive preference translation, and read-only activation of matching imported system/body exploration history with journal-authoritative merging, unknown-field-preserving screenshot/notification/Guardian setting saves, lossless live overlay position/opacity editing, cross-platform verified BMP-to-PNG screenshot conversion, persisted next-jump/system-survey/prior-scan/surface-radar/combat/Guardian/human-settlement/station-information/notification/multi-game-label preferences, validated Raven service override shared by Colonisation/Quest/Green Gas Giant clients, migrated external-biology/EDDN/Green-Gas-Giant privacy choices, bootstrap-safe current-schema EDDN publication for all 16 original supported event contracts, default-off live settlement-geometry publication consent, all 30 editable keyboard/controller bindings, opt-in SharpHook keyboard capture, SDL controller discovery/polling, and the original Raven API-key/GGG help launchers implemented; all 133 tagged legacy options are migrated or deliberately superseded by the native Avalonia overlay model |
