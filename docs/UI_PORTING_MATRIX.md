...
| `FormEditMap` | Guardian / Map editor | Partially implemented with native template rendering plus site/relic headings, notes, POI states, relic headings, obelisk-group editing, live commander-origin distance/angle/rotation measurement, and duplicate-guarded local raw-POI add/remove; master template-authoring/export tools remain |
...
| `FormRuins` | Guardian / Survey maps | Partially implemented through the unified site browser, direct selected-survey navigation, native map renderer, live-site card, lossless survey/raw-POI editor, and guarded share-bundle workflow; advanced master-map authoring remains |
