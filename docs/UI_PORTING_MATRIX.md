...
| `FormStartNewCmdr` | Multiple commanders/windows | Partial: the Overview commander card discovers live/legacy profiles with malformed-file isolation, excludes the current identity, launches an argument-safe additional process, and uses legacy `-fid` or new `--frontier-id` journal isolation; next-Elite-window focus remains |
