...
| `FormPlayComms` / `FormPlayComms2` | Quest communications | Lossless legacy local loading/saves, Raven service contracts, the cancellable Lua chapter runtime, and source-safe journal/faction/status/surface compatibility are implemented; live composition, player workspace, and compact quest indicator remain |
