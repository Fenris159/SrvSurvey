...
| Colonisation | `Main` Colonise menu, project forms, and `PlotBuildCommodities` | Raven projects and construction state | Opt-in Raven project loading/selection, set/clear primary, creation, bootstrap-safe live contribution/depot/completion/docking synchronization, untracked construction-site lookup, live depot progress, cargo planning, Market guidance, corruption-safe current Fleet Carrier publish/link plus linked-carrier cargo sync, and a passive shopping overlay with the legacy Market-after-docking, construction-site, right-panel, and Squadron-bank music rules implemented; registration preserves existing Raven cargo with a null payload before reconciling a fresh matching market, while ambiguous multi-client ship cargo is cleared and cannot enter plans or Raven publishing |
