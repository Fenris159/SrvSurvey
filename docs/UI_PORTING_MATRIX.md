...
| `FormSettings` | Settings pages | Raven themes, migration, next-jump/system-survey/Canonn prior-scan/radar/combat/Guardian preferences, live overlay adjustment, verified screenshot conversion/banner/source-deletion controls, and global keyboard/controller input implemented; privacy and remaining legacy options remain |
...
| `FormSwapStarCache` | Diagnostics / Reference data | Partial: EDGalaxy download validation, size bounds, persistent checksum-backed original backup, same-directory staged activation, transaction rollback, and verified restore are implemented in Core; Diagnostics UI and platform target selection remain |
