...
| Colonisation | `Main` Colonise menu, project forms, and `PlotBuildCommodities` | Raven projects and construction state | Opt-in Raven project loading/selection, set/clear primary, creation, bootstrap-safe live contribution/depot/completion/docking synchronization, untracked construction-site lookup, live depot progress, cargo planning, Market guidance, linked Fleet Carrier cargo/sync, and a passive shopping overlay with the legacy Market-after-docking, construction-site, right-panel, and Squadron-bank music rules implemented; ambiguous multi-client ship cargo is cleared and cannot enter plans or Raven publishing |
...
| `FormMyProjects` | Colonisation / Projects | Implemented with opt-in Raven loading, hidden-project selection, set/clear primary project, bootstrap-safe live contribution/depot/completion/docking synchronization, untracked construction-site lookup, aggregate cargo planning, current-ship trip estimates, refresh/save, and Raven build link; the new mutation controls await the final visual pass |
