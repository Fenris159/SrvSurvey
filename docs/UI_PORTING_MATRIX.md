...
| Colonisation | `Main` Colonise menu, project forms, and `PlotBuildCommodities` | Raven projects and construction state | Opt-in Raven project loading/selection, set/clear primary, creation, bootstrap-safe live contribution/depot/completion/docking synchronization, untracked construction-site lookup, live depot progress, cargo planning, Market guidance, active-commander API-key validation, corruption-safe current Fleet Carrier publish/link plus fresh-market and live buy/sell/transfer linked-carrier cargo sync, and a passive shopping overlay with the legacy Market-after-docking, construction-site, right-panel, Squadron-bank music, and rebuy/main-menu dock-reset rules implemented; rejected/mismatched credentials leave the stored profile unchanged, registration preserves existing Raven cargo with a null payload before reconciling a fresh matching market, stale-dock/SRV/bootstrap/malformed deltas cannot publish, and ambiguous multi-client ship cargo is cleared and cannot enter plans or Raven publishing |
