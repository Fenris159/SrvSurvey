...
| `FormPlayComms` / `FormPlayComms2` | Quest communications | Lossless legacy local loading/saves, Raven service contracts, the cancellable Lua chapter runtime, source-safe context, lifecycle composition, and live desktop journal/profile wiring are implemented; player workspace and compact quest indicator remain |
