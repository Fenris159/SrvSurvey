...
| `FormPlayComms` / `FormPlayComms2` | Quest communications | Lossless legacy local loading/saves, Raven service contracts, the cancellable Lua chapter runtime, source-safe journal/faction/status/surface compatibility, and lifecycle composition with bootstrap suppression and development precedence are implemented; desktop journal wiring, player workspace, and compact quest indicator remain |
