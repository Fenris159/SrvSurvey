...
| Guardian | `Main` Guardian menu and survey forms | Sites, maps, beacons and Ram Tah | Reference/commander catalog, visits, exact completion, filters, custom distance origins, external/share actions, live site detection/writes, native survey maps, lossless survey/raw-POI editing, guarded master-template authoring/export, current-obelisk proximity/artifacts/scan actions, both Ram Tah missions, detached commander-centered/heading-up live map with all five sizes, legacy zoom behavior, site identification/heading prompts, ruins/structure alignment reticles, altitude fade, glide site/blueprint guidance, obelisk targeting, the original safe survey commands, current-system summary, and Ram Tah log/artifact overlays implemented; Guardian artifact cargo is cleared when multiple Elite windows make the shared file ambiguous |
| Colonisation | `Main` Colonise menu, project forms, and `PlotBuildCommodities` | Raven projects and construction state | Opt-in Raven project loading/selection, set/clear primary, creation, bootstrap-safe live contribution/depot/completion/docking synchronization, untracked construction-site lookup, live depot progress, cargo planning, Market guidance, active-commander API-key validation, corruption-safe current Fleet Carrier publish/link plus fresh-market and live buy/sell/transfer linked-carrier cargo sync, and a passive shopping overlay with the legacy Market-after-docking, construction-site, right-panel, and Squadron-bank music rules implemented; rejected/mismatched credentials leave the stored profile unchanged, registration preserves existing Raven cargo with a null payload before reconciling a fresh matching market, SRV/bootstrap/malformed deltas cannot publish, and ambiguous multi-client ship cargo is cleared and cannot enter plans or Raven publishing |
