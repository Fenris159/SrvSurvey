...
| --- | --- | --- | --- |
| Overview | `Main` commander group | Commander, game/session, system and body state | Implemented for bootstrap state with death/resurrection/main-menu and hyperspace-departure clearing of transient body and vehicle context while retaining durable commander/system identity; Windows visually checked |
...
| Exobiology | `Main` bio group, `FormPredictions`, Codex forms | Scan progress, rewards and predictions | `Main` active-sample, separation, reward, sale/death, and reset workflow, exact `PlotBioSystem` species/variant predictions with distinct global-regional/commander/commander-regional firsts from the validated imported cache, standalone system/body predictions, `FormShowCodex`, full commander/region `FormCodexBingo`, `PlotPriorScans`, consolidated `PlotGrounded`/`PlotTrackers`, and the compact `PlotMiniTrack` replacement are implemented; human-settlement priority is wired into the shared coordinator |
| Travel | `Main` Travel menu, journey/route forms | Ground target, system notes, journeys and routes | Ground-target editor, clipboard/current actions, persistence, live `.target here/on/off` and `@` commands with coordinate-preserving visibility changes, live guidance and passive overlay, system notes, Commander Journeys, followed-route workspace, imports, journal progression, and Galaxy Map guidance implemented |
| Search | `Main` Search menu, sphere/boxel/nearest forms | Spatial, boxel, and biological searches | Spherical center lookup, radius, enable/disable, live distance, and compatible persistence implemented; Boxel activation, hierarchy, source merging, generated and imported/provisioned hand-authored name/ID64 decoding with restart-safe persistence, completion, navigation, clipboard, and full-area audit implemented; their combined `PlotSphericalSearch` Galaxy Map guidance is implemented; nearest Canonn-signal and Spansh missing-variant searches plus result actions implemented |
| Guardian | `Main` Guardian menu and survey forms | Sites, maps, beacons and Ram Tah | Reference/commander catalog, visits, exact completion, filters, custom distance origins, external/share actions, live site detection/writes, native survey maps, lossless survey/raw-POI editing, guarded master-template authoring/export, current-obelisk proximity/artifacts/scan actions, both Ram Tah missions, detached commander-centered/heading-up live map with all five sizes, legacy zoom behavior, site identification/heading prompts, ruins/structure alignment reticles, altitude fade, glide site/blueprint guidance, obelisk targeting, the original safe survey commands, current-system summary, Ram Tah log/artifact overlays, death/resurrection/main-menu live-site reset, and authoritative-snapshot plus immediate journal-delta artifact cargo implemented; Guardian artifact cargo is cleared when multiple Elite windows make the shared file ambiguous |
| Colonisation | `Main` Colonise menu, project forms, and `PlotBuildCommodities` | Raven projects and construction state | Opt-in Raven project loading/selection, set/clear primary, creation, bootstrap-safe live contribution/depot/completion/docking and beacon-architect synchronization, delayed post-permission commander-workspace refresh, untracked construction-site lookup, completed-site MarketID/name repair with authenticated targeted PATCH and a persistent 50-success repeat guard, live depot progress, shared journal-plus-file cargo projection, Market guidance, active-commander API-key validation, corruption-safe current Fleet Carrier publish/link plus fresh-market and live buy/sell/transfer linked-carrier cargo sync, and a passive shopping overlay with the legacy Market-after-docking, construction-site, right-panel, Squadron-bank music, and rebuy/main-menu/jump dock-reset rules implemented; projected cargo refreshes plans without causing extra Raven ship publications, rejected/mismatched credentials leave the stored profile unchanged, beacon replay or missing key/system context cannot publish, registration preserves existing Raven cargo with a null payload before reconciling a fresh matching market, stale-dock/SRV/bootstrap/malformed deltas cannot publish, ambiguous/no-match site repairs remain retryable, and ambiguous multi-client ship cargo is cleared and cannot re-enter plans or Raven publishing until a fresh file write |
...
| Settings | `FormSettings`, `FormSetKeyChord`, `FormAdjustOverlay` | Themes, paths, overlays, input and privacy | Raven themes, checksum-verified legacy profile import with empty/unreadable-source filtering, live-monitor shutdown, concurrent-write rollback, immediate non-destructive preference translation, and read-only activation of matching imported system/body exploration history with journal-authoritative merging, startup plus manual versioned reference-data refresh without requiring application releases, unknown-field-preserving screenshot/notification/Guardian setting saves, lossless live overlay position/opacity editing, cross-platform verified BMP-to-PNG screenshot conversion, persisted next-jump/system-survey/prior-scan/surface-radar/combat/Guardian/human-settlement/station-information/notification/multi-game-label preferences, validated Raven service override shared by Colonisation/Quest/Green Gas Giant clients, migrated and enforced external-system/external-biology/EDDN/Green-Gas-Giant privacy choices, bootstrap-safe current-schema EDDN publication for all 16 original supported event contracts, default-off live settlement-geometry publication consent, all 30 editable keyboard/controller bindings, opt-in SharpHook keyboard capture, SDL controller discovery/polling, and the original Raven API-key/GGG help launchers implemented; all 133 tagged legacy options are migrated or deliberately superseded by the native Avalonia overlay model |
| Guides | Repository README, original wiki, parity matrices, biology-criteria notes, and implemented behavior | Searchable in-app help and overlay icon reference | Implemented as navigation item 11 with 12 task categories, full-text workflow/glossary search, current Avalonia replacements for legacy Windows-only steps, migration/update/privacy safety guidance, and 52 code-rendered icon entries covering text glyphs, biology PIPs, surface radar, route, Guardian, settlement, quest, and conflict-zone symbols; visually checked in Orange dark with theme-matched selection/search styling |
...
imported fields, provider failures are isolated, stale responses cannot cross a
system transition, and downloaded genus confirmations remain behind the
separate biology-consent switch. This is independent of the startup GitHub
reference-catalog updater and does not alter its versioned delivery behavior.
...
| `FormBuilder` | Human settlements / Template authoring | Implemented as a collapsed developer workspace with live commander-offset capture, compatible `@@`/`!!`/`..`/`//` clipboard and diagnostic measurements, runtime cockpit calibration, manual and shield-toggle polygon points, renderer-compatible circles, multi-path buildings, named points, terminals, secure doors with relative rotation, floor/security metadata, draft preview/undo/discard, and explicit checksum-verified atomic catalog export with concurrent-write refusal and byte-identical backup; final visual testing remains deferred |
...
| `FormGroundTarget` | Travel / Ground target | Implemented in Travel with typed, current, clipboard, clear, settlement-origin, and guidance actions plus live `.target`/`@`/`!` command compatibility and a passive Raven-themed `PlotTrackTarget` replacement; editor visually checked, overlay visual QA deferred to the final UI pass |
...
| `FormMyProjects` | Colonisation / Projects | Implemented with opt-in Raven loading, hidden-project selection, set/clear primary project, bootstrap-safe live contribution/depot/completion/docking synchronization, untracked construction-site lookup, aggregate cargo planning, explicit current-carrier publish/link with bounded legacy display-name recovery and existing-cargo preservation, current-ship trip estimates, refresh/save, and Raven build link; the new mutation controls await the final visual pass |
...
| `FormPostProcess` | Diagnostics / Journal tools | Implemented with commander/date-filtered and cancellable historical statistics, recent-active-journal safety, modern/legacy filename support, Trailblazers comparison, read-only Green Gas Giant candidate discovery plus privacy-and-confirmation-gated isolated publication, an explicitly confirmed atomic Commander Codex merge, byte-preserving species/atmosphere aggregation, and explicitly confirmed Odyssey system/body reconstruction with lossless unknown-field merging, serialized live writes, checksum-verified backups, atomic activation, and tested rollback; analysis and reconstruction never perform implicit network writes |
...
| `FormSettings` | Settings pages | Raven themes, checksum-backed profile import with immediate guarded preference translation, next-jump/system-survey/Canonn prior-scan/radar/combat/Guardian/notification preferences, migrated external-biology/EDDN/Green-Gas-Giant privacy choices, opt-in live-only current-schema EDDN and Green Gas Giant candidate publication plus newly inferred settlement-geometry publication, live overlay adjustment, verified screenshot conversion/banner/source-deletion controls, global keyboard/controller input, Raven API-key acquisition, and Green Gas Giant documentation actions implemented; malformed legacy settings are retained byte-for-byte and leave current Avalonia settings unchanged; 126 tagged controls map directly, while the seven remaining implementation switches are intentionally replaced by native separate/click-through overlays or consumed from imported overlay/Journey settings |
...
| `FormStartNewCmdr` / `FormMultiFloatie` | Multiple commanders/windows | Implemented in the Overview commander card with live/legacy profile discovery, malformed-file isolation, current-identity exclusion, argument-safe additional-process launch, legacy `-fid` and new `--frontier-id` journal isolation, Windows/X11 next-Elite-window focus from the card or `Alt+Ctrl+W`, and a five-second-inventory passive Commander label with focus/minimize/suppression/click-through safety and the migrated hide preference; the same inventory immediately clears and suppresses shared `Cargo.json` data across dock logs, Guardian, Colonisation, and Quests until a new single-client write; visual and live multi-client QA are deferred to the final pass |
...
Wayland disablement. The unscaled `FormMultiFloatie` replacement additionally
restores the independent multi-Elite Commander cue and window cycling without
misclassifying it as the six-second `PlotFloatie` notification queue. The
detached Guardian window is a partial consolidation of
...
The legacy publication payload is also restored behind default-off consent;
only newly calculated live geometry can submit, while imported, downloaded,
repeated, and bootstrap geometry never does.
...
persistence, live Canonn contract/parser/privacy/merge/publication behavior, settings,
