...
| EDDN publication | `EddnPublisher`, `EddnSessionPublisher`, `EddnOutbox` | Application-owned durable delivery, immutable Commander-session headers and context, fixed production schemas, per-message persistence, fair retry scheduling, companion validation, privacy/bootstrap gates and bounded payloads |
