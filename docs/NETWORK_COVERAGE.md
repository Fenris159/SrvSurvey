...
| Inara Community Goal read | `InaraCommunityGoalClient`, `InaraCommunityGoalEnricher` | Generic application identity only, no commander fields, 15-minute disk cache, bounded response, stale-cache fallback, conservative title/location/expiry matching and Frontier-first field precedence |
