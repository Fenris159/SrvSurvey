...
Issues and specs for this repo live in `Fenris159/SrvSurvey` GitHub Issues. Use the `gh` CLI for all operations.
...
- **Child ticket**: a GitHub sub-issue linked to the map. Where sub-issues are unavailable, use a task list and add `Part of #<map>` to the child.
- **Blocking**: use GitHub’s native issue dependencies. Where unavailable, add a `Blocked by: #<n>` line.
- **Frontier query**: list open children, exclude assigned or blocked tickets, and select the first in map order.
