...
public sealed class GuardianSiteMapControlTests
{
    ...
    {
        Assert.Equal(new Point(40, 20), GuardianSiteMapControl.GetCommanderHeadingEnd(new Point(40, 50), radius: 15));
    }
    ...
    public void FixedMapRotatesOnlyCommanderGlyph()
    {
        Assert.Equal(
            90,
            GuardianSiteMapControl.ResolveMapRotationHeading(commanderHeading: 90, rotateMapWithCommander: true)
        );
        Assert.Equal(
            0,
            GuardianSiteMapControl.ResolveCommanderGlyphHeading(commanderHeading: 90, rotateMapWithCommander: true)
        );
        Assert.Equal(
            0,
            GuardianSiteMapControl.ResolveMapRotationHeading(commanderHeading: 90, rotateMapWithCommander: false)
        );
        Assert.Equal(
            90,
            GuardianSiteMapControl.ResolveCommanderGlyphHeading(commanderHeading: 90, rotateMapWithCommander: false)
        );
        Assert.Equal(
            new Point(70, 50),
            GuardianSiteMapControl.GetCommanderHeadingEnd(new Point(50, 50), radius: 10, heading: 90)
        );
    }
    ...
    {
        var proximity = new GuardianSiteProximitySnapshot(12, 5, -8, 5, -8, null, null);
    ...
    public void RejectsInvalidManualScale()
    {
        ...
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            GuardianSiteMapControl.TransformMapPoint(0, 0, proximity, 0, new Point(10, 10), double.NaN)
        );
    }
    ...
    public void UnknownSurveyPointsRetainLegacySurveyMarker(GuardianPoiType type)
    {
        Assert.True(GuardianSiteMapControl.RequiresSurveyMarker(Point("P1", type, GuardianPoiStatus.Unknown)));
    }
    ...
    {
        Assert.False(GuardianSiteMapControl.RequiresSurveyMarker(Point("P1", type, GuardianPoiStatus.Unknown)));
    }
    ...
    public void RecordedSurveyPointsDoNotUseSurveyMarker(GuardianPoiStatus status)
    {
        Assert.False(GuardianSiteMapControl.RequiresSurveyMarker(Point("P1", GuardianPoiType.Orb, status)));
    }

    [Fact]
    public void SurveyMarkerRetainsLegacySiteSpecificSizingAndPalette()
    {
        Assert.Equal((17.8d, 16.8d), GuardianSiteMapControl.GetSurveyMarkerRadii(GuardianPoiType.Relic, isRuins: true));
        Assert.Equal((13d, 12d), GuardianSiteMapControl.GetSurveyMarkerRadii(GuardianPoiType.Totem, isRuins: true));
        Assert.Equal((13d, 12d), GuardianSiteMapControl.GetSurveyMarkerRadii(GuardianPoiType.Relic, isRuins: false));
        Assert.Equal((10d, 9d), GuardianSiteMapControl.GetSurveyMarkerRadii(GuardianPoiType.Totem, isRuins: false));
        Assert.Equal(Color.FromArgb(160, 72, 61, 139), GuardianSurveyMarkerDrawing.HaloColor);
        Assert.Equal(Color.FromArgb(96, 0, 255, 255), GuardianSurveyMarkerDrawing.RingColor);
    }
    ...
    public void SurveyMarkerCreatesVisibleDotsAroundTheWholeRing()
    {
        ...
        Assert.All(
            dots,
            dot =>
                Assert.Equal(18, Math.Sqrt(Math.Pow(dot.X - center.X, 2) + Math.Pow(dot.Y - center.Y, 2)), precision: 9)
        );
    ...
    public void ArtifactPaletteMatchesLegacyGuardianMap()
    {
        ...
        {
            [GuardianPoiType.Orb] = (Color.FromRgb(255, 127, 39), Color.FromRgb(147, 58, 0)),
            [GuardianPoiType.Casket] = (Color.FromRgb(34, 177, 76), Color.FromRgb(17, 87, 38)),
            [GuardianPoiType.Tablet] = (Color.FromRgb(153, 217, 234), Color.FromRgb(33, 135, 160)),
            [GuardianPoiType.Totem] = (Color.FromRgb(63, 72, 204), Color.FromRgb(29, 34, 105)),
    ...
    public void StructureGlyphStatusStylesMatchLegacyGuardianMap()
    {
        ...
        Assert.Equal(GuardianLegacyStrokePattern.Dash, component.Pattern);
    ...
    public void LegacyGuardianGlyphsRetainGeometryAndScreenRotation()
    {
        ...
        Assert.Equal(
            157.5,
            GuardianLegacyMapDrawing.GetGlyphRotation(obelisk, projection, commanderHeading: 40),
            precision: 9
        );
        Assert.Equal(
            350,
            GuardianLegacyMapDrawing.GetGlyphRotation(pylon, projection, commanderHeading: 40),
            precision: 9
        );
        Assert.Equal(
            305,
            GuardianLegacyMapDrawing.GetGlyphRotation(component, projection, commanderHeading: 40),
            precision: 9
        );
        Assert.Equal(
            280,
            GuardianLegacyMapDrawing.GetGlyphRotation(relic, projection, commanderHeading: 40),
            precision: 9
        );
        ...
        Assert.Equal(8, GuardianLegacyMapDrawing.CreateGlyphPoints(GuardianPoiType.Component, new Point(), 0).Count);
    }
    ...
    public void GuardianMarkersScaleWithTheMapLikeLegacyRenderer()
    {
        ...
        Assert.Equal(8, GuardianLegacyMapDrawing.GetPuddleRadius(ruins, Point("P1", GuardianPoiType.Orb)));
    }
    ...
    [InlineData(12, 12)]
    [InlineData(20, 15)]
    public void InteractiveViewportZoomUsesBoundedLegacyRange(double requested, double expected)
    {
        Assert.Equal(expected, GuardianSiteMapControl.NormalizeViewportZoom(requested));
    }
    ...
    public void InteractiveViewportPanKeepsPartOfTheMapReachable()
    {
        Assert.Equal(
            default,
            GuardianSiteMapControl.ClampViewportOffset(new Vector(100, -100), new Size(720, 640), zoom: 1)
        );
        Assert.Equal(
            new Vector(360, -320),
            GuardianSiteMapControl.ClampViewportOffset(new Vector(900, -900), new Size(720, 640), zoom: 2)
        );
    }
    ...
    public void OverlappingCommanderDoesNotBlockPointSelectionFeedback()
    {
        var control = new GuardianSiteMapControl
        {
            Projection = new GuardianSiteMapProjection(
                "Lacrosse",
                [RenderPoint("P1", GuardianPoiType.Orb, 0, 0, GuardianPoiStatus.Present)],
                [],
                1
            ),
            CommanderMapPosition = new GuardianSiteProximitySnapshot(0, 0, 0, 0, 0, null, null),
            MapScale = 4,
        ...
        {
            window.Show();
            window.MouseMove(new Point(360, 320), RawInputModifiers.None);
            ...
            window.MouseUp(new Point(40, 40), MouseButton.Left, RawInputModifiers.None);
    ...
    public void ComponentMaterialDotsStayScreenAlignedLikeLegacyRenderer()
    {
        ...
        Assert.All(
            dots,
            dot =>
                Assert.Equal(8, Math.Sqrt(Math.Pow(dot.X - center.X, 2) + Math.Pow(dot.Y - center.Y, 2)), precision: 9)
        );
        ...
        Assert.Equal(Colors.Lime, GuardianLegacyMapDrawing.GetComponentMaterialColor(GuardianComponentMaterial.Cell));
        Assert.Equal(
            Colors.Cyan,
            GuardianLegacyMapDrawing.GetComponentMaterialColor(GuardianComponentMaterial.Conduit)
        );
        Assert.Equal(
            Colors.OrangeRed,
            GuardianLegacyMapDrawing.GetComponentMaterialColor(GuardianComponentMaterial.Tech)
        );
    }
    ...
    public void ActiveObeliskEffectsUseLegacyMissionScanPrecedence()
    {
        Assert.Equal(
            ...
            GuardianLegacyMapDrawing.GetActiveObeliskEffectColor(
                Point(
                    "A01",
                    GuardianPoiType.Obelisk,
                    isActiveObelisk: true,
        ...
        Assert.Equal(
            ...
            GuardianLegacyMapDrawing.GetActiveObeliskEffectColor(
                Point("A01", GuardianPoiType.Obelisk, isActiveObelisk: true, isScannedObelisk: true)
            )
        );
        Assert.Equal(
            Colors.LightGray,
            GuardianLegacyMapDrawing.GetActiveObeliskEffectColor(
                Point("A01", GuardianPoiType.Obelisk, isActiveObelisk: true)
            )
        );
    }
    ...
    public void CompleteGuardianSurveyProjectionRendersEveryLegacyMapLayer()
    {
        var points = new GuardianProjectedPoint[]
        {
            ...
            RenderPoint("UNKNOWN-PANEL", GuardianPoiType.DestructiblePanel, -21, -17, GuardianPoiStatus.Unknown),
        ...
        var nearestPoi = new GuardianPointOfInterest("PYLON", GuardianPoiType.Pylon, 0, 18, 0);
        var control = new GuardianSiteMapControl
        {
    ...
    public void PackagedLegacyMapArtworkIsAvailableForMappedSiteTypes()
    {
        ...
        Assert.Equal(new Size(1024, 1024), GuardianMapImageCatalog.Find(beta)?.Size);
    }
    ...
    public void LocalDraftBackgroundImageCanBePreviewed()
    {
        ...
        try
        {
            ...
            var projection = new GuardianSiteMapProjection("Draft", [], [], 1, false, BackgroundImage: path);

            Assert.Equal(new Size(1, 1), GuardianMapImageCatalog.Find(projection)?.Size);
        }
        finally
        {
            File.Delete(path);
    ...
    public void ExternalLegendRendersWithoutDrawingTheMapSurface()
    {
        var control = new GuardianSiteMapControl
        {
            Projection = new GuardianSiteMapProjection(
                "Lacrosse",
                [
                    RenderPoint("P1", GuardianPoiType.Pylon, 0, 0, GuardianPoiStatus.Present),
                    RenderPoint("C1", GuardianPoiType.Component, 1, 1, GuardianPoiStatus.Present),
                ],
                [],
                1
            ),
        ...
        Assert.True(
            Render(
                control,
                new Size(280, 320),
                Environment.GetEnvironmentVariable("SRVSURVEY_GUARDIAN_LEGEND_RENDER_OUTPUT")
            )
        );
    }
    ...
    private static bool Render(GuardianSiteMapControl control, Size? requestedSize = null, string? outputPath = null)
    {
        ...
        try
        {
            ...
            return frame?.PixelSize == new PixelSize((int)size.Width, (int)size.Height);
        }
        finally
        {
            window.Close();
