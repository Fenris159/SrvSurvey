...
public sealed class ApplicationUpdateInstanceWarningPresentationTests
{
    ...
    public void WarningExplainsTheInstanceCountAndClosureBeforeDownload()
    {
        ...
        try
        {
            ...
            var text = dialog
                ...
                .ToArray();
            Assert.Contains("3 SrvSurvey instances are currently running.", text);
    ...
    {
        Assert.Throws<ArgumentOutOfRangeException>(() => new MultipleApplicationInstancesDialog(0));
    }

    [AvaloniaFact]
    public void WarningExplainsWhenWindowsCouldNotVerifyAMatchingProcess()
    {
        var dialog = new MultipleApplicationInstancesDialog(otherInstanceCount: 2, unverifiedInstanceCount: 1);
        try
        {
            dialog.Show();
            Assert.NotNull(dialog.CaptureRenderedFrame());
            ...
            Assert.Contains(
                text,
                value => value!.Contains("prevented verification of 1 matching process", StringComparison.Ordinal)
            );
            Assert.Contains(
                text,
