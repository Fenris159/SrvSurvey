...
public sealed class OverlayThemeSettingsPresentationTests : IDisposable
{
    ...
    public void SharedSettingsTemplateRendersCompactSingleRowColorEditors()
    {
        ...
        try
        {
            ...
            Assert.Equal(overlayTheme.Categories.Single(category => category.IsExpanded).Colors.Count, rows.Length);
            Assert.All(
                rows,
                row =>
                {
                    ...
                    Assert.True(colorOrigin.Value.X + colorPicker.Bounds.Width <= row.Bounds.Width);
                    Assert.InRange(thumb.Bounds.Width, 1, 16);
                    Assert.InRange(thumb.Bounds.Height, 1, 16);
                    Assert.True(thumbOrigin.Value.Y >= 0);
                    Assert.True(thumbOrigin.Value.Y + thumb.Bounds.Height <= opacity.Bounds.Height);
