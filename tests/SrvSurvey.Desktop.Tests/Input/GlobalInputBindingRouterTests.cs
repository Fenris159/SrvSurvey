...
public sealed class GlobalInputBindingRouterTests
{
    ...
    public void ResolvesConfiguredChordAfterCanonicalization()
    {
        ...
        {
            Bindings = GlobalInputSettings.Default.Bindings.ToDictionary(entry => entry.Key, entry => entry.Value),
        };
        var bindings = settings.Bindings.ToDictionary();
        bindings[GlobalInputAction.CopyNextBoxel] = "ctrl alt x";
        var router = new GlobalInputBindingRouter(settings with { Bindings = bindings });

        Assert.True(router.TryResolve("ALT CTRL X", out GlobalInputAction action));
    ...
    public void FirstCatalogActionWinsDuplicateChord()
    {
        ...
        var router = new GlobalInputBindingRouter(GlobalInputSettings.Default with { Bindings = bindings });

        Assert.True(router.TryResolve("ALT X", out GlobalInputAction action));
    ...
    public void ResolvesNamedKeysWithoutCaseSensitivity()
    {
        ...
        Assert.True(router.TryResolve("CTRL Oemcomma", out GlobalInputAction action));
        Assert.Equal(GlobalInputAction.ToggleAllVisibility, action);
    }
}
