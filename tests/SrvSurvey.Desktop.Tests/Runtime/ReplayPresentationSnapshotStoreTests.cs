...
public sealed class ReplayPresentationSnapshotStoreTests
{
    ...
    public void CaptureAndApplyRoundTripEveryAnchorAndPortableSetting()
    {
        ...
        Assert.Equal(
            sourcePlacements,
            applied
                .Placements.Where(entry => names.Contains(entry.Key, StringComparer.Ordinal))
    ...
    public void CaptureAndApplyRejectInvalidPortablePresentation()
    {
        ...
        var unknownOverlay = CreateSession(
            ...
            Snapshot(
                ...
                {
                    ["UnknownOverlay"] = new(ReplayHorizontalAnchor.Left, 0, ReplayVerticalAnchor.Top, 0, null, null),
                }
            )
        );
        ...
        var invalidPlacementScale = CreateSession(
            ...
            Snapshot(
                ...
                new Dictionary<string, ReplayOverlayPlacement>
                {
                    ["PlotBioStatus"] = new(ReplayHorizontalAnchor.Left, 0, ReplayVerticalAnchor.Top, 0, null, 99),
                }
            )
        );
        Assert.Throws<InvalidDataException>(() => ReplayPresentationSnapshotStore.Apply(invalidPlacementScale));
        ...
        ReplayPresentationSnapshotStore.Apply(
            CreateSession(Path.Combine(temp.Path, "empty-session"), presentationSnapshot: null)
        );
        ReplayPresentationSnapshotStore.Apply(
            CreateSession(
                Path.Combine(temp.Path, "empty-snapshot-session"),
                Snapshot(0, new Dictionary<string, ReplayOverlayPlacement>())
            )
        );
    }
    ...
    {
        return new ReplayPresentationSnapshot(1920, 1080, scaleIndex, null, new Dictionary<string, bool>(), placements);
    }
    ...
    private static DiagnosticReplaySession CreateSession(string root, ReplayPresentationSnapshot? presentationSnapshot)
    {
