...
public sealed class MiningBarDetectorTests
{
    ...
    public void AlternatingIceBarsCannotSlideTheGridIntoRigThree()
    {
        ...
        foreach (var active in new[] { 3, 2, 1, 3 })
        {
            ...
            Assert.True(
                previous.Slots[0] == ((active & 1) != 0 ? MiningBarState.Present : MiningBarState.Absent),
    ...
    private static MiningDetectionSettings IceSettings() =>
        new()
        {
            CircleWidth = 94d / 420,
            RotationDegrees = -14,
            ...
            [
                new(84d / 420, 77d / 210),
                new(217d / 420, 62d / 210),
                new(341d / 420, 45d / 210),
                new(86d / 420, 165d / 210),
                new(218d / 420, 149d / 210),
                new(342d / 420, 131d / 210),
            ],
        };
    ...
    private static MiningDetectionSettings EmptyNightVisionSettings() =>
        new()
        {
            CircleWidth = 94d / 420,
            RotationDegrees = -14,
            ...
            [
                new(87d / 420, 75d / 230),
                new(220d / 420, 59d / 230),
                new(344d / 420, 43d / 230),
                new(87d / 420, 162d / 230),
                new(220d / 420, 148d / 230),
                new(344d / 420, 130d / 230),
            ],
        };
    ...
    public void HudMovementHoldsAllChangesUntilItSettlesThenRestartsEmptyDelay()
    {
        var clock = new TestClock();
        var confirmation = new MiningBarConfirmation(clock);
        static MiningBarAnalysis Frame(double y, bool empty = false) =>
            new(
                [
                    empty ? MiningBarState.Absent : MiningBarState.Present,
    ...
    public void MovementAndReturningRigOneDoNotRenumberRemainingRigs()
    {
        ...
        foreach (var (frame, count) in new[] { (20, 1), (40, 2), (60, 2), (100, 3) })
        {
            ...
            Assert.True(
                previous.Slots.Take(count).All(state => state == MiningBarState.Present),
    ...
    public void ColorGroupsRecognizeBothBarsAfterDriving()
    {
        var settings = new MiningDetectionSettings
        {
            ...
            Markers =
            [
                new(123d / 400, 102d / 220),
                new(189d / 400, 96d / 220),
                new(250d / 400, 90d / 220),
                new(123d / 400, 148d / 220),
                new(189d / 400, 139d / 220),
    ...
    public void MissingCircleIsUncertainRatherThanAnAbsentRig()
    {
        ...
        Assert.All(
            MiningBarDetector.Analyze(source, new()).Slots,
    ...
    public void LiveObserverFrameDoesNotMistakeTheSecondRimForABar(double diameter, double aspect)
    {
        ...
        Assert.True(
            result.Slots.Skip(1).All(state => state != MiningBarState.Present),
            $"{string.Join(',', result.Slots)}; scores {string.Join(',', result.BarScores)}"
        );
    }
    ...
    public void LatestDeploymentScreenshotRecognizesTheActiveSlot(double aspect, double rotation, double gap)
    {
        ...
        var settings = new MiningDetectionSettings
        {
            ...
            Markers =
            [
                new(117d / 500, 120d / 286),
                new(246d / 500, 108d / 286),
                new(373d / 500, 96d / 286),
                new(117d / 500, 208d / 286),
                new(246d / 500, 191d / 286),
    ...
    public void ReportedHudRecognizesTheVisibleBar(double rotation, bool recolor)
    {
        ...
        var settings = new MiningDetectionSettings
        {
            ...
            Markers =
            [
                new(143d / 506, 116d / 260),
                new(274d / 506, 104d / 260),
                new(399d / 506, 90d / 260),
                new(143d / 506, 206d / 260),
                new(274d / 506, 191d / 260),
    ...
    public void RecordedBarsAreRecognizedDespiteMovement(int frame, int active)
    {
        ...
        for (var slot = 0; slot < 6; slot++)
        {
            ...
            {
                Assert.True(
                    result.Slots[slot] == MiningBarState.Present,
                    $"Frame {frame}, slot {slot + 1}: {string.Join(',', result.Slots)}; scores {debug}"
                );
            }
            else
            {
                Assert.NotEqual(MiningBarState.Present, result.Slots[slot]);
            }
        }
    }
    ...
    private static MiningDetectionSettings CalibratedSettings()
    {
        var settings = new MiningDetectionSettings
        {
            CircleWidth = .11,
            MotionMargin = .25,
            Markers = [new(.445, .47), new(.595, .43), new(.74, .39), new(.445, .67), new(.595, .64), new(.74, .60)],
        };
        return settings;
    }
        ...
        settings = settings with
        {
            ...
            Markers = settings
                .Markers.Select(p => new MiningDetectionPoint(
                    ...
                    ((p.X * 400 - 200) * s + (p.Y * 200 - 100) * c + 100) / 200
                ))
                .ToArray(),
        };
    ...
    public void UnreadableOrOutOfRangeHudIsUnknown(int frame)
    {
        ...
        Assert.All(
            MiningBarDetector.Analyze(new CapturedPixelBuffer(400, 200, new byte[400 * 200 * 4]), settings).Slots,
            state => Assert.Equal(MiningBarState.Unknown, state)
        );
    }
    ...
    public void CalibrationBoundsFollowViewportAndCancelDoesNotPersist()
    {
        ...
        Assert.Equal(new PixelRect(-960, 1080, 768, 432), settings.GetBounds(new PixelRect(-1920, 0, 3840, 2160)));
