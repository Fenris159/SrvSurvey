...
public sealed class FirstFootfallInferenceServiceTests
{
    ...
    public void ColorDetectorUsesLegacyStrictToleranceBounds()
    {
        ...
        Assert.Equal(0.5, FirstFootfallColorDetector.GetMatchRatio(source, preferences));
    }

    [Fact]
    public async Task DetectionUsesLegacyWatchAreaAndStopsAboveThreshold()
    {
        var tracker = new StubWindowTracker(
            new GameWindowSnapshot((nint)1, 42, new PixelRect(100, 200, 1920, 1080), true, true)
        );
        var capture = new StubScreenCapture(
            CreateBuffer(
    ...
    {
        var tracker = new StubWindowTracker(
            new GameWindowSnapshot((nint)1, 42, new PixelRect(0, 0, 1920, 1080), true, false)
        );
        ...
        Assert.Equal(FirstFootfallInferenceOutcome.GameNotForeground, result.Outcome);
    ...
    private sealed class StubWindowTracker(GameWindowSnapshot snapshot) : IGameWindowTracker
    {
    ...
    private sealed class StubScreenCapture(CapturedPixelBuffer buffer) : IGameScreenCapture
    {
