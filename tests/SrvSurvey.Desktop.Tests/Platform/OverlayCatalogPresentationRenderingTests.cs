...
public sealed class OverlayCatalogPresentationRenderingTests
{
    ...
    public void EveryEditorOverlayPresentationRendersAtItsExpectedSize()
    {
        ...
        if (!string.IsNullOrWhiteSpace(outputDirectory))
        {
            File.WriteAllLines(Path.Combine(outputDirectory, "dimensions.csv"), dimensions);
        }
    ...
    public void FssInformationRemeasuresAtMaximumPerRoleTypographyScale()
    {
        ...
        try
        {
            OverlayThemeResources.Apply(preview);
            preview.ApplyRuntimePresentationTheme();
            preview.ConfigureTypography(new OverlayTypographyScale(100, 100, 100, 100, 100, 100, 100));
            preview.Show();
    ...
    public void EveryPanelRemeasuresVisibleTextAtMaximumTypographyScale()
    {
        ...
        var maximum = new OverlayTypographyScale(100, 100, 100, 100, 100, 100, 100);
        string? output = Environment.GetEnvironmentVariable("SRVSURVEY_OVERLAY_RENDER_OUTPUT");
        if (!string.IsNullOrWhiteSpace(output))
        {
            Directory.CreateDirectory(output);
