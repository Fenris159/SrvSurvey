...
public sealed class CodexImageCacheTests : IDisposable
{
    ...
    public async Task RejectsInvalidMissingAndOversizedImagesWithoutPartialFiles()
    {
        using var client = new HttpClient(
            new StubHandler(request =>
            {
                ...
                response.Content.Headers.ContentLength = CodexImageCache.MaximumImageBytes + 1;
                return response;
            })
        );
        using var cache = new CodexImageCache(temporaryDirectory, client);
    ...
    public async Task PreDownloadReportsDownloadedCachedLocalAndUnavailableImages()
    {
        ...
        await File.WriteAllBytesAsync(Path.Combine(floraDirectory, "local-three.png"), [3]);
        using var client = new HttpClient(
            new StubHandler(request =>
        ...
        Assert.Equal(new CodexImagePreDownloadResult(4, 1, 1, 1, 1), result);
        Assert.Equal([1], await File.ReadAllBytesAsync(Path.Combine(cacheDirectory, "1.png")));
