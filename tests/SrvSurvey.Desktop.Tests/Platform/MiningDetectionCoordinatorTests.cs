...
public sealed class MiningDetectionCoordinatorTests : IDisposable
{
    ...
    public async Task FocusAndPanelGatesAreRecheckedAfterCapture(
        bool foreground,
        bool calibrating,
        int focus,
        bool loseFocusDuringCapture,
    ...
    )
    {
        ...
        if (calibrating)
        {
            mining.Detection.StartCalibrationTest();
        }
        ...
        var capture = new FakeCapture(() =>
        {
            ...
            {
                Avalonia
                    .Threading.Dispatcher.UIThread.InvokeAsync(async () =>
            ...
            if (loseFocusDuringCapture)
            {
                tracker.Snapshot = tracker.Snapshot with { IsForeground = false };
            }
        });
        using (var coordinator = new MiningDetectionCoordinator(mining, tracker, capture))
        {
            ...
            {
                Assert.StartsWith("HUD not located", mining.Detection.StatusText);
            }
