...
public sealed class FrontierOAuthCallbackTests
{
    ...
    {
        var callback = FrontierOAuthCallback.Parse("\"srvsurvey://frontier-auth?code=abc%20123&state=state-value\"");

        Assert.NotNull(callback);
        Assert.Equal("abc 123", callback.Code);
        Assert.Equal("state-value", callback.State);
        Assert.Null(FrontierOAuthCallback.Parse("srvsurvey://frontier-auth.evil.example?code=abc&state=state-value"));
        Assert.Null(FrontierOAuthCallback.Parse("srvsurvey://frontier-auth/path?code=abc&state=state-value"));
        Assert.Null(FrontierOAuthCallback.Parse("https://frontier-auth?code=abc&state=state-value"));
    }
    ...
    public void StateComparisonIsExact(string left, string right, bool expected)
    {
