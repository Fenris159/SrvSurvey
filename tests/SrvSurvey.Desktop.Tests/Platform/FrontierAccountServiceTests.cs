...
public sealed class FrontierAccountServiceTests
{
    ...
    public async Task CallbackUsesPkceAndStoresTokensWithoutClientSecret()
    {
        ...
        await service.HandleCallbackAsync(
            new FrontierOAuthCallback("authorization-code", "expected-state", string.Empty, string.Empty)
        );
    ...
    public async Task CallbackRejectsMismatchedStateBeforeNetworkRequest()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(() =>
            service.HandleCallbackAsync(new FrontierOAuthCallback("code", "wrong", string.Empty, string.Empty))
        );
        ...
        try
        {
            ...
            using var service = CreateService(
                ...
                request =>
                {
                    requests.Add(request.RequestUri!.AbsolutePath);
                    return request.RequestUri.AbsolutePath == "/profile"
                        ? Json(HttpStatusCode.OK, "{\"commander\":{\"name\":\"Fenris\",\"rank\":{}},\"ships\":[]}")
                        : Json(HttpStatusCode.NoContent, string.Empty);
                },
                root,
                () => now
    ...
    public async Task CarrierEnvelopeMetadataIsCachedWithoutInventingCarrier()
    {
        ...
        try
        {
            ...
            Assert.Contains(
                cached.CarrierEndpointData!,
                point => point.Path == "fleetcarrier.accountMetadata.available"
            );
        }
        finally
        {
    ...
    public async Task FailedRefreshAttemptIsAlsoThrottled()
    {
        ...
        try
        {
            ...
            using var service = CreateService(
                ...
                {
                    requestCount++;
                    return Json(HttpStatusCode.InternalServerError, "{\"message\":\"try later\"}");
                },
                root,
                () => now
            );
    ...
    public async Task SwitchingCommanderUsesIndependentCredentialsAndCaches()
    {
        ...
        try
        {
            ...
            using var service = new FrontierAccountService(
                ...
                store,
                frontierId => new FrontierProfileCacheStore(Path.Combine(root, frontierId + ".json")),
                new FrontierAccountServiceOptions(
                    UtcNow: () => now,
                    OpenBrowser: (_, _) => Task.CompletedTask,
                    RegisterProtocol: _ => Task.CompletedTask
    ...
    public async Task MiskeyedCapiIdAliasIsRemovedWithoutMergingAnotherCommander()
    {
        ...
        try
        {
            ...
            var activeCache = new FrontierProfileCacheStore(Path.Combine(root, "F472567.json"));
            var aliasCache = new FrontierProfileCacheStore(Path.Combine(root, "F739749.json"));
            var secondCache = new FrontierProfileCacheStore(Path.Combine(root, "F831234.json"));
            await activeCache.SaveAsync(
                FrontierCapiSnapshotParser.Parse(
                    "{\"commander\":{\"id\":739749,\"name\":\"Fenris Nihilus\",\"rank\":{}},\"ships\":[]}",
                    null,
                    now.AddMinutes(-5)
    ...
    public async Task BlankCommanderNameDoesNotMatchOrMigrateMiskeyedAlias()
    {
        ...
        try
        {
            ...
            await new FrontierProfileCacheStore(Path.Combine(root, "F739749.json")).SaveAsync(snapshot);
            using var service = new FrontierAccountService(
                new HttpClient(new StubHandler(_ => Json(HttpStatusCode.OK, "{}"))),
                store,
                frontierId => new FrontierProfileCacheStore(Path.Combine(root, frontierId + ".json")),
    ...
    public async Task MiskeyedCapiIdCredentialMovesToJournalFidWhenItIsTheOnlyCopy()
    {
        ...
        try
        {
            ...
            await new FrontierProfileCacheStore(Path.Combine(root, "F739749.json")).SaveAsync(
                FrontierCapiSnapshotParser.Parse(
                    "{\"commander\":{\"id\":739749,\"name\":\"Fenris Nihilus\",\"rank\":{}},\"ships\":[]}",
                    null,
                    now
        ...
        try
        {
            ...
            using var service = new FrontierAccountService(
                ...
                frontierId => new FrontierProfileCacheStore(
                    Path.Combine(root, "frontier-profile-cache", frontierId + ".json")
                ),
                new FrontierAccountServiceOptions(
    ...
    public async Task WindowsCredentialStoreEncryptsTokensAtRest()
    {
        ...
        try
        {
            ...
            {
                Accounts = new Dictionary<string, FrontierAccountCredential> { ["F123"] = account },
            };
    ...
    private static FrontierCredentialDocument LinkedCredential(DateTimeOffset now)
    {
    ...
    private static FrontierAccountCredential ScopedCredential(string accessToken, DateTimeOffset now) =>
        new()
        {
    ...
    private sealed class StubInaraCommunityGoalClient(InaraCommunityGoalsResult result) : IInaraCommunityGoalClient
    {
        public int RequestCount { get; private set; }

        public Task<InaraCommunityGoalsResult> GetRecentAsync(CancellationToken cancellationToken = default)
        {
            RequestCount++;
    ...
    private sealed class MemoryCredentialStore : IFrontierCredentialStore
    {
        ...
        public async Task<IAsyncDisposable> AcquireLeaseAsync(CancellationToken cancellationToken = default)
        {
