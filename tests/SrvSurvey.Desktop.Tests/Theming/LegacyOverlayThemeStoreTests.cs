...
public sealed class LegacyOverlayThemeStoreTests : IDisposable
{
    ...
    public void LegacyFormatsReferencesNullsAndMissingEntriesAreSupported()
    {
        ...
        Assert.Equal(Color.FromArgb(255, 0, 0, 0), theme.GetColor("guardian.background"));
        Assert.Equal(Color.FromArgb(255, 255, 255, 0), theme.GetColor("guardian.header"));
        Assert.Equal(Color.FromArgb(255, 255, 111, 0), theme.GetColor("guardian.primary"));
        Assert.Equal(Color.FromArgb(255, 95, 48, 3), theme.GetColor("guardian.primaryDark"));
        ...
        Assert.Equal(Color.FromArgb(255, 20, 20, 20), theme.GetColor("guardian.surface"));
    ...
    public void SaveRoundTripsCustomGuardianPrimary()
    {
        ...
        Assert.Equal(Color.FromArgb(255, 84, 223, 237), loaded.GetColor("guardian.secondary"));
    }
    ...
    public void SaveRoundTripsAllColorsAndCreatesVerifiedBackup()
    {
        ...
        Assert.All(colors, entry => Assert.Equal(entry.Value, loaded.Colors[entry.Key]));
    ...
    public void LegacyCeruleanGoldBiologyPaletteIsUpgradedInMemory()
    {
        ...
        Assert.Equal("Cerulean Gold", OverlayThemePresetCatalog.FindMatching(theme.Colors)?.Name);
    }
    ...
    public void OriginalDefaultBiologyPaletteGainsLegacyPipLayersAndEdges()
    {
        ...
        Assert.Equal(defaults["bio.predictionPotential"], theme.GetColor("bio.predictionPotential"));
        Assert.Equal(defaults["bio.predictionEdge"], theme.GetColor("bio.predictionEdge"));
        Assert.Equal(defaults["bio.predictionSegmentEdge"], theme.GetColor("bio.predictionSegmentEdge"));
        Assert.Equal(defaults["bio.goldPotentialSegmentEdge"], theme.GetColor("bio.goldPotentialSegmentEdge"));
        Assert.Equal(defaults["bio.confirmedDimPotential"], theme.GetColor("bio.confirmedDimPotential"));
        Assert.Equal(defaults["bio.goldPotential"], theme.GetColor("bio.goldPotential"));
        Assert.Equal(OverlayThemePresetCatalog.DefaultName, OverlayThemePresetCatalog.FindMatching(theme.Colors)?.Name);
    }
