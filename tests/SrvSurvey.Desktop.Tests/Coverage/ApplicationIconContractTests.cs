...
public sealed class ApplicationIconContractTests
{
    ...
    public void DesktopProjectEmbedsTheAvaloniaIconInTheExecutable()
    {
        ...
        var applicationIcon = project.Descendants("ApplicationIcon").Single();
    ...
    {
        var root = FindRepositoryRoot();
        var iconPath = Path.Combine(root, "src", "SrvSurvey.Desktop", "Assets", "logo.ico");
        ...
        var entries = Enumerable.Range(0, count).Select(_ => ReadEntry(reader)).ToArray();
    ...
    {
        var assets = Path.Combine(FindRepositoryRoot(), "src", "SrvSurvey.Desktop", "Assets");
        ...
        foreach (var (x, y) in new[] { (128, 28), (48, 128), (208, 128), (128, 220) })
        {
    ...
    {
        var root = FindRepositoryRoot();
        var sourcePath = Path.Combine(root, "src", "SrvSurvey.Desktop", "Assets", HighResolutionIconFileName);
