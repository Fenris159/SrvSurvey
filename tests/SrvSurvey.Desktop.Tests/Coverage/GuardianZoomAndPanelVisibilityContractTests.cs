...
public sealed class GuardianZoomAndPanelVisibilityContractTests
{
    ...
    public void GuardianZoomUsesTwoCircularOrbsInAnInteractiveChildWindow()
    {
        ...
        Assert.Contains(buttons, button => button.Attribute("Command")?.Value == "{Binding ZoomInCommand}");
        Assert.Contains(buttons, button => button.Attribute("Command")?.Value == "{Binding ZoomOutCommand}");
    ...
    public void GuardianMapShowsLandedShipChevronAtBottomLeft()
    {
        ...
        Assert.Equal("{Binding Guardian.ShipRelativeBearingDegrees}", chevron.Attribute("BearingDegrees")?.Value);
        Assert.Equal("{Binding Guardian.IsShipNavigationFar}", chevron.Attribute("IsFar")?.Value);
        Assert.Contains(
            shipIndicator.Descendants(),
            element => element.Attribute("Text")?.Value == "{Binding Guardian.ShipNavigationDistanceText}"
        );
    }
    ...
    public void GuardianFiregroupChoicesUseHighContrastSelectedState()
    {
        ...
        var selectedBindings = presentation
            ...
            .Count(attribute =>
                attribute.Name.LocalName == "Classes.selected"
                && attribute.Value.StartsWith("{Binding Guardian.IsGuardianChoice", StringComparison.Ordinal)
            );
        ...
        Assert.Contains(
            ...
            setter =>
                setter.Attribute("Property")?.Value == "BorderThickness" && setter.Attribute("Value")?.Value == "2"
        );
        Assert.Contains(
            selectedChoice.Elements(),
            setter =>
                setter.Attribute("Property")?.Value == "Background"
                && setter.Attribute("Value")?.Value == "{DynamicResource RavenGuardianSecondaryBrush}"
        );
        Assert.Contains(
            selectedText.Elements(),
            setter =>
                setter.Attribute("Property")?.Value == "Foreground"
                && setter.Attribute("Value")?.Value == "{DynamicResource RavenGuardianBackgroundBrush}"
        );
        Assert.Contains(
            selectedText.Elements(),
            setter => setter.Attribute("Property")?.Value == "FontWeight" && setter.Attribute("Value")?.Value == "Bold"
        );
    }
    ...
    private static string FindRepositoryRoot()
    {
        ...
        while (directory is not null && !File.Exists(Path.Combine(directory.FullName, "SrvSurvey.slnx")))
        {
