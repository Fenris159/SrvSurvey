...
public sealed class GuardianViewMarkupTests
{
    ...
    public void DistanceOriginActionsLeaveTheAutocompleteFullWidth()
    {
        ...
        Assert.DoesNotContain(
            ...
            element =>
                ...
                && (element == originSection || originSection.Descendants().Contains(element))
                && element.Descendants().Contains(entry)
                && element.Descendants().Any(candidate => candidate.Name.LocalName == "Button")
        );
    }
    ...
    public void SurveyMapKeepsContextCardsBesideMapInRequestedOrder()
    {
        ...
        Assert.Equal("{Binding Guardian.SelectedMapPointName}", map.Attribute("HighlightedPointName")?.Value);
        Assert.Equal("{Binding Guardian.SelectedMapCommanderPosition}", map.Attribute("CommanderMapPosition")?.Value);
        Assert.Equal("{Binding Guardian.SelectedMapCommanderPosition}", map.Attribute("Proximity")?.Value);
        Assert.Equal("{Binding Guardian.ActiveMapRelativeHeading}", map.Attribute("CommanderHeading")?.Value);
        Assert.Equal("False", map.Attribute("RotateMapWithCommander")?.Value);
        Assert.Equal("{Binding Guardian.SelectedMapTargetPointName}", map.Attribute("TargetPointName")?.Value);
        ...
        Assert.Contains(
            ...
            element =>
                element
                    .Attribute("Text")
                    ?.Value.Contains("Gradient wedges mark active obelisks", StringComparison.Ordinal) == true
        );
        Assert.Equal("{Binding Guardian.SurveyEditor.IsMapSummaryVisible}", selectedMap.Attribute("IsVisible")?.Value);
        Assert.Equal(
        ...
        var selectedContent = selectedPoint
            ...
            .Single(element =>
                ...
                && element.Attribute("Content")?.Value == "{Binding Guardian.SurveyEditor.SelectedPoint}"
            );
        Assert.Equal(
    ...
    public void SharedMapAndPerSiteSurveyEditorsExplainSeparateSaveScopes()
    {
        ...
        Assert.Contains(
            ...
            element => element.Attribute("Text")?.Value == "{Binding Guardian.TemplateAuthoring.DraftDescription}"
        );
        Assert.Contains(
            draftTools.Descendants(),
            element => element.Attribute("Text")?.Value == "{Binding Guardian.TemplateAuthoring.SaveLocationText}"
        );
        Assert.Contains(
            draftTools.Descendants(),
            element => element.Attribute("Content")?.Value == "Save map changes..."
        );
        Assert.DoesNotContain(
            draftTools.Descendants(),
            element => element.Attribute("Content")?.Value == "Apply metadata to draft"
        );
        Assert.Contains(catalogDetails, draftTools.Descendants());
        Assert.Contains(catalogDetails.Descendants(), element => element.Attribute("Text")?.Value == "BODY");
        Assert.Contains(catalogDetails.Descendants(), element => element.Attribute("Text")?.Value == "DISTANCE LY");
        Assert.Contains(
            catalogDetails.Descendants(),
            element => element.Attribute("Text")?.Value == "ARRIVAL DISTANCE LS"
        );
        Assert.Contains(
            catalogDetails.Descendants(),
            element => element.Attribute("Content")?.Value == "Save site details"
        );
        Assert.DoesNotContain(
            catalogDetails.Descendants(),
            element => element.Attribute("Text")?.Value is "GALACTIC X" or "GALACTIC Y" or "GALACTIC Z"
        );
        Assert.Contains(surveyEditor.Descendants(), element => element.Attribute("Text")?.Value == "This site survey");
        Assert.Contains(
            surveyEditor.Descendants(),
            element =>
                element.Attribute("Text")?.Value?.Contains("belong only to the selected site", StringComparison.Ordinal)
                == true
        );
    }
    ...
    public void SurveyEditorExposesRepairAndPrecisionAuthoringFields()
    {
        ...
        Assert.DoesNotContain(
            ...
            attribute => attribute.Value.Contains("SelectedActiveObelisk.", StringComparison.Ordinal)
        );
        ...
        Assert.Contains(
            ...
            element =>
                ...
                && element.Attribute("Text")?.Value == "{Binding Guardian.TemplateAuthoring.PointName, Mode=TwoWay}"
        );
        Assert.Contains(
            templatePointFields.Descendants(),
            element =>
    ...
    public void SurveyMapProvidesBottomZoomBarForInteractiveViewport()
    {
        ...
        Assert.Contains("ElementName=GuardianSurveyMap", slider.Attribute("Value")?.Value, StringComparison.Ordinal);
        ...
        Assert.Equal("{StaticResource question_circle_regular}", orientationIcon.Attribute("Data")?.Value);
        Assert.DoesNotContain(document.Descendants(), element => GetName(element) == "GuardianSurveyMapOrientation");
    }
    ...
    public void OperationalEditorsSpanRowsBelowTheMap()
    {
        ...
        Assert.Equal("{Binding Guardian.TemplateAuthoring.IsAuthoring}", mapDraftTools.Attribute("IsVisible")?.Value);
    ...
    public void ExternalLegendUsesOneRenderedCardWithRoomForAllStates()
    {
        ...
        Assert.Equal("guardian-map-legend", expander.Attribute("Classes")?.Value);
        Assert.Equal("{Binding IsMonochromeTheme}", expander.Attribute("Classes.monochrome")?.Value);
        ...
        Assert.Equal("OuterBorderEdge", expanderContentSetters["BackgroundSizing"]);
        ...
        Assert.DoesNotContain(legend.Descendants(), element => element.Name.LocalName is "Border" or "Grid");
    }
    ...
    public void SitesTableKeepsHeaderFixedAndColumnsAlignedWhileRowsScroll()
    {
        ...
        Assert.Equal(
            "Disabled",
            rows.Attributes()
                .Single(attribute => attribute.Name.LocalName == "ScrollViewer.HorizontalScrollBarVisibility")
                .Value
        );
        Assert.Equal(
            "Auto",
            rows.Attributes()
                .Single(attribute => attribute.Name.LocalName == "ScrollViewer.VerticalScrollBarVisibility")
                .Value
        );
        ...
        Assert.Equal(header.Attribute("ColumnDefinitions")?.Value, rowGrid.Attribute("ColumnDefinitions")?.Value);
        Assert.Equal(header.Attribute("ColumnSpacing")?.Value, rowGrid.Attribute("ColumnSpacing")?.Value);
        Assert.Contains(
            "table-rows",
            (rows.Attribute("Classes")?.Value ?? string.Empty).Split(' ', StringSplitOptions.RemoveEmptyEntries)
        );
    }
    ...
    public void SitesTableSortHeadersMatchRouteManagerPresentation()
    {
        ...
        var sortHeaders = document
            ...
            .Where(element =>
                ...
                && element.Attribute("Command")?.Value == "{Binding Guardian.SortSitesCommand}"
            )
            .ToArray();

        Assert.Equal(expectedParameters.Length, sortHeaders.Length);
        Assert.Equal(expectedParameters, sortHeaders.Select(header => header.Attribute("CommandParameter")?.Value));
        Assert.All(
            sortHeaders,
            header =>
            {
                Assert.Contains(
                    "link",
                    (header.Attribute("Classes")?.Value ?? string.Empty).Split(
                        ' ',
    ...
    private static XDocument LoadGuardianView() =>
        XDocument.Load(Path.Combine(FindRepositoryRoot(), "src", "SrvSurvey.Desktop", "Views", "GuardianView.axaml"));

    private static XElement FindNamedElement(XDocument document, string name) =>
        document.Descendants().Single(element => GetName(element) == name);
