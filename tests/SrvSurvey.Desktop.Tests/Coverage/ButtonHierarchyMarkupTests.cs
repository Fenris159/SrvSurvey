...
public sealed class ButtonHierarchyMarkupTests
{
    ...
    public void SharedButtonStylesKeepEmphasisAndControlFillsSeparate()
    {
        ...
        Assert.Contains("Button.link\">\n    <Setter Property=\"Background\" Value=\"Transparent\"", styles);
        Assert.Contains("<Setter Property=\"Foreground\" Value=\"{DynamicResource RavenAccentBrush}\" />", styles);
    }
    ...
    private static string Describe(XElement button) =>
        button.Attribute("Content")?.Value ?? button.Attribute("Command")?.Value ?? "unnamed button";
