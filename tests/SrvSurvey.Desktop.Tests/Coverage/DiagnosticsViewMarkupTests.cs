...
public sealed class DiagnosticsViewMarkupTests
{
    ...
    public void JournalHistoryIsSeparateFromTheQuestInspector()
    {
        ...
        Assert.Contains(
            ...
            element =>
                ...
                && element
                    .Attributes()
                    .Any(attribute =>
                        attribute.Name.LocalName == "Name" && attribute.Value == "JournalHistoryEventList"
                    )
        );
        Assert.Contains(
            document.Descendants(),
            element =>
                element.Name.LocalName == "Button" && element.Attribute("Content")?.Value == "Export replay package"
        );
        Assert.Contains(
            document.Descendants(),
            element =>
                element.Name.LocalName == "TextBox"
                && element.Attribute("Text")?.Value == "{Binding JournalHistory.SearchText, Mode=TwoWay}"
        );
        Assert.Contains(
            document.Descendants(),
        ...
        Assert.Contains(
            ...
            element =>
                ...
                && element.Attribute("SelectedDate")?.Value == "{Binding JournalHistory.RangeFromDate, Mode=TwoWay}"
        );
        Assert.Contains(
            document.Descendants(),
            element =>
                element.Name.LocalName == "TimePicker"
                && element.Attribute("SelectedTime")?.Value == "{Binding JournalHistory.RangeFromTime, Mode=TwoWay}"
        );
        Assert.Contains(
            document.Descendants(),
            element =>
                element.Name.LocalName == "CalendarDatePicker"
                && element.Attribute("SelectedDate")?.Value == "{Binding JournalHistory.RangeToDate, Mode=TwoWay}"
        );
        Assert.Contains(
            document.Descendants(),
            element =>
                element.Name.LocalName == "TimePicker"
                && element.Attribute("SelectedTime")?.Value == "{Binding JournalHistory.RangeToTime, Mode=TwoWay}"
        );
        Assert.DoesNotContain(
            ...
            element =>
                ...
                && element.Attribute("Text")?.Value
                    is "{Binding JournalHistory.RangeFromText, Mode=TwoWay}"
    ...
    public void JournalHistoryDetailsDoNotDereferenceAMissingSelection()
    {
        ...
        Assert.DoesNotContain(
            textBindings,
            binding => binding.Contains("JournalHistory.SelectedEvent.", StringComparison.Ordinal)
        );
        Assert.Contains("{Binding JournalHistory.SelectedEventFileName}", textBindings);
        Assert.Contains("{Binding JournalHistory.SelectedEventCommanderName}", textBindings);
        Assert.Contains("{Binding JournalHistory.SelectedEventSystemName}", textBindings);
        Assert.Contains("{Binding JournalHistory.SelectedEventTimestamp}", textBindings);
        Assert.Contains("{Binding JournalHistory.SelectedEventRawJson}", textBindings);
    }
    ...
    public void LiveLogUsesAnIndependentNonCaretScrollSurface()
    {
        ...
        Assert.Contains(
            logScroller.Descendants(),
            element => element.Name.LocalName == "TextBlock" && element.Attribute("Text")?.Value == logBinding
        );
        Assert.DoesNotContain(
            document.Descendants(),
            element => element.Name.LocalName == "TextBox" && element.Attribute("Text")?.Value == logBinding
        );
    }
    ...
    public void JournalInspectorUsesStableVerticalRowsAndOwnsWheelScrolling()
    {
        ...
        Assert.Contains(
            ...
            element =>
                element.Name.LocalName == "ItemsPanelTemplate"
        ...
        Assert.All(
            ...
            element =>
                Assert.Equal(
                    "False",
                    FindAttribute(element, "IsScrollChainingEnabled", "ScrollViewer.IsScrollChainingEnabled")
                )
        );
    }

    private static XDocument LoadDiagnosticsView() =>
        XDocument.Load(
            Path.Combine(FindRepositoryRoot(), "src", "SrvSurvey.Desktop", "Views", "DiagnosticsView.axaml")
        );

    private static string? FindAttribute(XElement element, params string[] names) =>
        element
            .Attributes()
            .FirstOrDefault(attribute => names.Contains(attribute.Name.LocalName, StringComparer.Ordinal))
            ?.Value;
