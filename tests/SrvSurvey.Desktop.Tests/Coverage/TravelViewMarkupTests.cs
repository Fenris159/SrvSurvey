...
public sealed class TravelViewMarkupTests
{
    ...
    public void TravelUsesRouteManagerSurfaceNavigationAndFleetCarrierTabs()
    {
        ...
        Assert.Equal(
            ...
            tabs.Select(tab => tab.Attribute("Header")?.Value ?? string.Empty).ToArray()
        );
        Assert.All(
            tabs,
            tab =>
    ...
    public void TravelTabsAreUnboundedAndSharedPanelsRemainBelowThem()
    {
        ...
        Assert.Contains("Commander journeys", sharedText, StringComparison.Ordinal);
    }
    ...
    public void RouteManagerOffersStableRowsFavoritesAndRequestedFileActions()
    {
        ...
        Assert.Contains("{Binding RouteManager.FavoritesFirst, Mode=TwoWay}", bindings);
        ...
        var openWorkspaceButton = routeManagerTab
            ...
            .Single(element =>
                element.Name.LocalName == "Button"
                && element.Attribute("Command")?.Value == "{Binding RouteManager.OpenWorkspaceCommand}"
            );
        ...
        var favoriteButton = routeManagerTab
            ...
            .Single(element =>
                element.Name.LocalName == "Button"
                && element.Attribute("Command")?.Value == "{Binding ToggleFavoriteCommand}"
            );
        ...
        Assert.Contains(
            ...
            element => element.Attribute("IsVisible")?.Value == "{Binding RouteManager.IsRenameVisible}"
        );
    }
    ...
    public void FleetCarrierTabUsesIndependentManagerWorkspaceAndFileActions()
    {
        ...
        Assert.Contains("{Binding FleetCarrierRouteManager.Routes}", bindings);
        Assert.Contains("{Binding FleetCarrierRouteManager.ToggleAutoCopyCommand}", bindings);
        Assert.Contains("{Binding FleetCarrierRouteManager.AutoCopy, Mode=OneWay}", bindings);
        Assert.Contains("{Binding FleetCarrierRoute.NextHopName}", bindings);
        Assert.Contains("{Binding FleetCarrierRoute.CarrierJumpCountdownTitle}", bindings);
        Assert.Contains("{Binding FleetCarrierRoute.CarrierJumpCountdownValue}", bindings);
        Assert.Contains("{Binding FleetCarrierRoute.CarrierJumpPhaseLabel}", bindings);
        ...
        Assert.Contains(
            document.Descendants(),
            element => element.Attribute("IsVisible")?.Value == "{Binding FleetCarrierRouteManager.IsNotesVisible}"
        );
        Assert.Contains(
            document.Descendants(),
            element => element.Attribute("IsVisible")?.Value == "{Binding FleetCarrierRouteManager.IsRenameVisible}"
        );
    }

    private static XDocument LoadTravelView() =>
        XDocument.Load(Path.Combine(FindRepositoryRoot(), "src", "SrvSurvey.Desktop", "Views", "TravelView.axaml"));

    private static XDocument LoadRavenStyles() =>
        XDocument.Load(Path.Combine(FindRepositoryRoot(), "src", "SrvSurvey.Desktop", "Styles", "RavenStyles.axaml"));

    private static Dictionary<string, string> ReadSetters(XElement style) =>
        style
            .Elements()
    ...
    {
        return element.Parent?.Elements().TakeWhile(candidate => candidate != element).Count() ?? -1;
    }
