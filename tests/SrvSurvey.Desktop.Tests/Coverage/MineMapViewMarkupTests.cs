...
public sealed class MineMapViewMarkupTests
{
    ...
    public void WorkspaceProvidesCatalogInstructionsAndGuardianStyleMapNavigation()
    {
        ...
        var headers = document.Descendants(avalonia + "TabItem")
            .Select(item => item.Attribute("Header")?.Value ?? string.Empty)
            .ToArray();

        Assert.Equal(
            ["Surface Maps", "Survey map", "Hotspot List", "Surface Hunt", "Instructions"],
            headers);
        ...
        Assert.Contains(document.Descendants(avalonia + "ItemsControl"), control =>
            control.Attribute("ItemsSource")?.Value == "{Binding HotspotRows}");
        Assert.Contains(document.Descendants(avalonia + "ItemsControl"), control =>
            control.Attribute("ItemsSource")?.Value == "{Binding SurfaceHuntRows}");
        Assert.Contains(document.Descendants(avalonia + "TextBlock"), text =>
            text.Attribute("Text")?.Value == "OVERLAY");
        Assert.Contains(document.Descendants(avalonia + "CheckBox"), checkBox =>
            checkBox.Attribute("IsChecked")?.Value
                == "{Binding IsInOverlay, Mode=TwoWay}");
        Assert.Contains(document.Descendants(avalonia + "Button"), button =>
            button.Attribute("Content")?.Value
                == "For Rig tracking and full command details click here");
        Assert.Contains(document.Descendants(avalonia + "TextBlock"), text =>
            text.Attribute("Text")?.Value == "SIGNAL #");
        Assert.Contains(document.Descendants(avalonia + "TextBlock"), text =>
            text.Attribute("Text")?.Value == "FROM SOL");
        Assert.DoesNotContain(document.Descendants(avalonia + "TextBlock"), text =>
            text.Attribute("Text")?.Value == "ACTIONS");
        Assert.DoesNotContain(document.Descendants(avalonia + "MenuItem"), item =>
        ...
        Assert.Contains(document.Descendants(avalonia + "MenuItem"), item =>
            item.Attribute("Header")?.Value == "Copy system name");
        Assert.Contains(document.Descendants(avalonia + "MenuItem"), item =>
            item.Attribute("Header")?.Value == "Edit bookmark");
        Assert.Equal(
            ["1.", "2.", "3."],
            document.Descendants(avalonia + "TextBlock")
                ...
                .ToArray());
        Assert.Contains(document.Descendants(avalonia + "TextBlock"), text =>
            text.Attribute("Text")?.Value == "Examples");
        ...
        Assert.Equal(
            "{DynamicResource RavenMapGridBrush}",
            map.Attribute("GridBrush")?.Value);
        Assert.Equal(
            "{DynamicResource RavenTextBrush}",
            map.Attribute("TextBrush")?.Value);
        Assert.Equal(
            "{DynamicResource RavenSuccessBrush}",
            map.Attribute("PlayerBrush")?.Value);
        var overlay = XDocument.Load(Path.Combine(
            FindRepositoryRoot(),
            "src",
            "SrvSurvey.Desktop",
            "MineMapOverlayPresentation.axaml"));
        var overlayMap = Assert.Single(
            overlay.Descendants(),
            element => element.Name.LocalName == "MineMapControl");
        Assert.Equal(
            "{DynamicResource RavenMapGridBrush}",
            overlayMap.Attribute("GridBrush")?.Value);
        Assert.Equal(
            "{DynamicResource RavenTextBrush}",
            overlayMap.Attribute("TextBrush")?.Value);
        ...
        Assert.DoesNotContain(document.Descendants(avalonia + "Button"), button =>
            button.Attribute("Content")?.Value is "−" or "+");
        Assert.All(
            document.Descendants(avalonia + "Grid")
                .Where(grid => grid.Attribute(XName.Get("Name", "http://schemas.microsoft.com/winfx/2006/xaml"))?.Value is "SurfaceMapsHeader" or "HotspotListHeader" or "SurfaceHuntHeader"),
            header => Assert.All(header.Elements(avalonia + "Button"), button =>
            {
                Assert.NotNull(button.Attribute("CommandParameter"));
                Assert.Contains(button.Descendants(avalonia + "TextBlock"), text =>
                    text.Attribute("Text")?.Value?.Contains("WorkspaceSortIndicatorConverter", StringComparison.Ordinal) == true);
            }));
        var surfaceHuntHeader = Assert.Single(document.Descendants(avalonia + "Grid"),
            grid => grid.Attribute(XName.Get("Name", "http://schemas.microsoft.com/winfx/2006/xaml"))?.Value == "SurfaceHuntHeader");
        Assert.Equal(8, surfaceHuntHeader.Elements(avalonia + "Button").Count());
        Assert.Equal(
            ["SEARCH GROUP", "MATERIAL", "START WITH", "ALSO POSSIBLE ON", "GEOLOGY TO LOOK FOR", "SPECIAL CLUE", "AVG GALACTIC PRICE", "PEAK SELL SNAPSHOT"],
            surfaceHuntHeader.Descendants(avalonia + "TextBlock")
                .Select(text => text.Attribute("Text")?.Value ?? string.Empty)
                .Where(text => !text.Contains("WorkspaceSortIndicatorConverter", StringComparison.Ordinal))
                .ToArray());
        var cardTitles = document.Descendants(avalonia + "TextBlock")
            .Where(text => text.Attribute("Classes")?.Value?.Contains("card-title", StringComparison.Ordinal) == true)
            .Select(text => text.Attribute("Text")?.Value ?? string.Empty)
            .ToArray();
        Assert.True(
            Array.IndexOf(cardTitles, "Selected map")
            < Array.IndexOf(cardTitles, "Marker visibility"));
        ...
        {
            Assert.Contains(document.Descendants(avalonia + "TextBlock"), text =>
                text.Attribute("Text")?.Value == binding);
        }
    }
    ...
    public void SharedBookmarkWorkspacePresentsSurfaceMapDetailsInCompactColumns()
    {
        ...
        Assert.Contains(document.Descendants(avalonia + "TextBlock"), text =>
            text.Attribute("Text")?.Value?.Contains("saved surface mining maps", StringComparison.OrdinalIgnoreCase) == true);
        Assert.Contains(document.Descendants(avalonia + "TextBlock"), text =>
            text.Attribute("Text")?.Value == "DETAILS");
        Assert.Contains(document.Descendants(avalonia + "TextBlock"), text =>
            text.Attribute("Text")?.Value == "NOTES");
        Assert.Contains(document.Descendants(avalonia + "Border"), border =>
            border.Attribute("IsVisible")?.Value == "{Binding IsSurfaceMiningMap}");
        Assert.Contains(document.Descendants(avalonia + "NumericUpDown"), input =>
            input.Attribute("Value")?.Value == "{Binding SurfaceSignal}");
        Assert.Contains(document.Descendants(avalonia + "TextBox"), input =>
            input.Attribute("Text")?.Value == "{Binding Notes}");
        Assert.Equal(5, document.Descendants(avalonia + "CheckBox").Count(checkBox =>
            checkBox.Attribute("Content")?.Value is "Mining" or "Surface Mining" or "Location" or "POI" or "Other"));
        Assert.Equal(6, document.Descendants(avalonia + "Grid")
            .Single(grid => grid.Attribute(XName.Get("Name", "http://schemas.microsoft.com/winfx/2006/xaml"))?.Value == "BookmarksHeader")
            .Elements(avalonia + "Button").Count());
        var rows = Assert.Single(document.Descendants(avalonia + "ListBox"), list =>
            list.Attribute(XName.Get("Name", "http://schemas.microsoft.com/winfx/2006/xaml"))?.Value == "BookmarkRows");
        Assert.Contains(rows.Descendants(avalonia + "Grid"), grid =>
            grid.Attribute("PointerPressed")?.Value == "BookmarkRow_PointerPressed"
            && grid.Attribute("Tag")?.Value == "{Binding Id}");
        Assert.Contains(rows.Descendants(avalonia + "TextBlock"), text =>
        ...
        Assert.Contains(document.Descendants(avalonia + "TextBlock"), text =>
            text.Attribute("Text")?.Value == "BODY");
        Assert.Contains(document.Descendants(avalonia + "TextBlock"), text =>
            text.Attribute("Text")?.Value == "RING");
        Assert.Contains(document.Descendants(avalonia + "TextBox"), input =>
            input.Attribute("Text")?.Value == "{Binding Ring}");
        Assert.Contains(document.Descendants(avalonia + "ColumnDefinition"), column =>
            column.Attribute("SharedSizeGroup") is not null);
        var header = document.Descendants(avalonia + "Grid")
            .Single(grid => grid.Attribute(XName.Get("Name", "http://schemas.microsoft.com/winfx/2006/xaml"))?.Value == "BookmarksHeader");
        Assert.All(header.Elements(avalonia + "Button"), button => Assert.Contains(
            button.Descendants(avalonia + "TextBlock"),
            text => text.Attribute("Text")?.Value?.Contains("WorkspaceSortIndicatorConverter", StringComparison.Ordinal) == true));
    ...
    public void LiveOverlayEmbedsAndHostsTheSameMinusPlusControls()
    {
        ...
        var controls = XDocument.Load(Path.Combine(
            ...
            "src",
            "SrvSurvey.Desktop",
            "MineMapZoomOverlayPresentation.axaml"));
        XNamespace avalonia = "https://github.com/avaloniaui";

        Assert.Single(
            presentation.Descendants(),
            element => element.Name.LocalName == "MineMapZoomOverlayPresentation");
        Assert.Contains(presentation.Descendants().Where(element =>
            element.Name.LocalName == "MineMapControl"), map =>
        ...
        var workspace = XDocument.Load(Path.Combine(
            ...
            "Views",
            "MineMapView.axaml"));
        Assert.Contains(workspace.Descendants().Where(element =>
            element.Name.LocalName == "MineMapControl"), map =>
            map.Attribute("PlayerLocation")?.Value == "{Binding PlayerLocation}"
            && map.Attribute("PlayerHeading")?.Value == "{Binding PlayerHeading}"
            && map.Attribute("PlayerBrush")?.Value == "{DynamicResource RavenSuccessBrush}");
        Assert.Equal(
            ["−", "+"],
            controls.Descendants(avalonia + "Button")
                .Select(button => button.Attribute("Content")?.Value
                    ?? string.Empty)
                .ToArray());
        var guardianButtons = XDocument.Load(Path.Combine(
                root,
                "src",
                "SrvSurvey.Desktop",
    ...
    {
        var root = FindRepositoryRoot();
        var reference = XDocument.Load(Path.Combine(
            root,
            "src",
            "SrvSurvey.Desktop",
            "MiningReferenceOverlayPresentation.axaml"));
        var workspace = XDocument.Load(Path.Combine(
            root,
            "src",
            "SrvSurvey.Desktop",
        ...
        Assert.All(
            ...
            column =>
            {
                ...
                Assert.StartsWith(
                    "MiningRef",
                    column.Attribute("SharedSizeGroup")?.Value);
            });
        var toggle = Assert.Single(workspace.Descendants(avalonia + "CheckBox"),
            checkBox => checkBox.Attribute("IsChecked")?.Value
                == "{Binding IsInOverlay, Mode=TwoWay}");
    ...
    public void MiningTableHeadersAreClickableAndShareCompactContentWidths(
        string fileName,
        int expectedTables)
    {
        var document = XDocument.Load(Path.Combine(
        ...
        var headers = document.Descendants(avalonia + "Grid")
            .Where(grid => grid.Attribute(x + "Name")?.Value.EndsWith(
                "Header",
        ...
        Assert.All(headers, header =>
        {
            ...
            Assert.All(buttons, button => Assert.NotNull(
                button.Attribute("CommandParameter")));
            Assert.All(buttons, button => Assert.Contains(
                button.Descendants(avalonia + "TextBlock"),
                text => text.Attribute("Text")?.Value?.Contains("WorkspaceSortIndicatorConverter", StringComparison.Ordinal) == true));
        });
    }
