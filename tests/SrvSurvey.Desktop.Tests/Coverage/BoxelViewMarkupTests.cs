...
public sealed class BoxelViewMarkupTests
{
    ...
    public void SelectedSystemPageUsesAccentForegroundForText()
    {
        ...
        Assert.Equal("{DynamicResource RavenAccentForegroundBrush}", foreground.Attribute("Value")?.Value);
    }
    ...
    public void DedicatedPageOwnsTheWholeBoxelWorkspace()
    {
        ...
        Assert.Contains("{Binding BoxelSearch.LibrarySaveButtonText}", boxelBindings);
        ...
        Assert.Contains("Send Journal to VoxStellar for boxel surveying", boxelBindings);
        Assert.Contains("{Binding VoxStellar.JournalUploadEnabled, Mode=TwoWay}", boxelBindings);
        Assert.Contains("{Binding VoxStellar.CanChangeUploadPreference}", boxelBindings);
        ...
        Assert.Equal("{StaticResource data_pie_regular}", boxelStatsIcon.Attribute("Data")?.Value);
        Assert.Equal("30", boxelStatsIcon.Attribute("Width")?.Value);
        Assert.Equal("30", boxelStatsIcon.Attribute("Height")?.Value);
        Assert.Contains("{Binding BoxelSearch.SystemNameSuggestions}", boxelBindings);
        Assert.Contains("{Binding BoxelSearch.SelectedSystemSuggestionIndex, Mode=TwoWay}", boxelBindings);
        ...
        Assert.Contains("Next incomplete suffix in the current boxel.", boxelBindings);
        Assert.DoesNotContain("Lowest incomplete suffix in the current boxel.", boxelBindings);
        ...
        var lastSystemAvailableTextBox = boxel
            ...
            .Single(element =>
                ...
                && element
                    .Attributes()
                    .Any(attribute =>
                        attribute.Name.LocalName == "Name" && attribute.Value == "LastSystemAvailableTextBox"
                    )
            );
        Assert.Equal("LastSystemAvailable_LostFocus", lastSystemAvailableTextBox.Attribute("LostFocus")?.Value);
        ...
        Assert.Contains("{Binding ShowNextIncompleteHighlight}", boxelBindings);
        Assert.Contains("{Binding ShowCurrentNextHighlight}", boxelBindings);
        ...
        Assert.Contains("{Binding BoxelSearch.ShowOnlyDeferred, Mode=TwoWay}", boxelBindings);
        Assert.Contains(boxel.Descendants(), element => element.Name.LocalName == "BoxelSystemActionMenu");
        Assert.Contains("{Binding BoxelSearch.PreviousSystemPageCommand}", boxelBindings);
        Assert.Contains("{Binding BoxelSearch.NextJumpPageCommand}", boxelBindings);
        Assert.Contains("{Binding BoxelSearch.NextSystemPageCommand}", boxelBindings);
        ...
        var systemPageFlyout = boxel
            ...
            .Single(element =>
                ...
                && element.Descendants().Any(descendant => descendant.Attribute("Classes")?.Value == "system-pages")
            );
        ...
        Assert.Contains("{Binding BoxelSearch.CanNavigateSearchTree}", boxelBindings);
        ...
        Assert.Contains("{Binding BoxelSearch.BreadcrumbBoxels}", boxelBindings);
        Assert.Contains("{Binding BoxelSearch.CurrentHierarchyBoxelProgressLabel}", boxelBindings);
        ...
        Assert.DoesNotContain(
            boxelBindings,
            value =>
                value.Contains("BoxelSearch.ParentBoxel.", StringComparison.Ordinal)
        ...
        Assert.Equal("Center", centeredHierarchyControls.Attribute("HorizontalAlignment")?.Value);
        Assert.Equal(
            3,
            centeredHierarchyControls.Elements().Count(element => element.Name.LocalName is "Button" or "Border")
        );
        Assert.Contains("Treat systems whose Spansh body data predates the start date as complete", boxelBindings);
        Assert.Contains(
        ...
        Assert.Equal(
            ...
            boxel
                ...
                .Count(element =>
                    element.Name.LocalName == "Grid"
                    && element.Attribute("ColumnDefinitions")?.Value == "2.05*,0.75*,1.3*,1.3*,84,110"
                )
        );
        Assert.DoesNotContain(
            boxel.Descendants(),
            element => element.Attribute("ColumnDefinitions")?.Value == "2.2*,0.8*,1.15*,1.15*,110,110"
        );
        Assert.DoesNotContain(searchBindings, binding => binding.Contains("BoxelSearch", StringComparison.Ordinal));
    }
    ...
    public void SystemActionMenuProvidesFourThemedRadialActions()
    {
        ...
        Assert.Contains("Button.radial-launcher.engaged Canvas.radial-glyph", values);
        Assert.Contains("Button.radial-launcher.engaged Path.slice-top-right", values);
        Assert.Contains("Button.radial-launcher.engaged Path.radial-slice", values);
        Assert.Contains("Button.radial-launcher.engaged Ellipse.radial-center", values);
        ...
        Assert.Contains("Button.radial-option:pointerover /template/ Path.option-chrome", values);
        ...
        var launcher = menu.Descendants()
            .Single(element =>
                ...
                && element
                    .Attributes()
                    .Any(attribute => attribute.Name.LocalName == "Name" && attribute.Value == "Launcher")
            );
        ...
        Assert.Contains(
            ...
            element => element.Attribute("Property")?.Value == "Opacity" && element.Attribute("Value")?.Value == "1"
        );
        Assert.Contains(
            disabledOptionStyle.Elements(),
            element =>
        ...
        Assert.Contains(
            disabledShadeStyle.Elements(),
            element => element.Attribute("Property")?.Value == "Opacity" && element.Attribute("Value")?.Value == "0.45"
        );
        Assert.DoesNotContain(
            menu.Descendants(),
        ...
        var menuHitSurface = menu.Descendants()
            .Single(element =>
                element
                    .Attributes()
                    .Any(attribute => attribute.Name.LocalName == "Name" && attribute.Value == "MenuHitSurface")
            );
        Assert.Equal("Menu_PointerWheelChanged", menuHitSurface.Attribute("PointerWheelChanged")?.Value);
    }
    ...
    public void ExpectedSystemsInformationUsesOneLocalizedExampleTemplate()
    {
        ...
        Assert.Contains(
            values,
            value => value.Contains("The number produced is an estimate.", StringComparison.Ordinal)
        );
        Assert.DoesNotContain(values, value => value.Contains("Then add 1 to that number", StringComparison.Ordinal));
        Assert.Contains(
            values,
            value =>
                value.Contains("SrvSurvey includes system 0 when it calculates the total.", StringComparison.Ordinal)
        );
        ...
        Assert.Contains("LocalizationCatalog.Translate(ExampleTemplate)", codeBehind, StringComparison.Ordinal);
        ...
        Assert.DoesNotContain(
            ...
            element =>
                element.Name.LocalName == "Button"
                && element.Attribute("Classes")?.Value.Contains("link", StringComparison.Ordinal) == true
        );
        Assert.DoesNotContain(values, value => value.Contains("Underline", StringComparison.Ordinal));
    }
    ...
    public void VoxStellarInformationExplainsConsentDataAndLicensing()
    {
        ...
        Assert.Contains(
            values,
            value => value.Contains("commander name and the complete JSON data", StringComparison.Ordinal)
        );
        Assert.Contains(values, value => value.Contains("does not submit this data to EDDN", StringComparison.Ordinal));
        Assert.Contains(
            values,
            value => value.Contains("worldwide, non-exclusive, royalty-free license", StringComparison.Ordinal)
        );
        Assert.Contains(values, value => value.Contains("Copyright © 2023 Sven Ziereis", StringComparison.Ordinal));
    ...
    public void BoxelStatisticsCanRebuildBeforeAnyBoxelIsSelected()
    {
        ...
        Assert.Contains(
            header.Elements(),
            element => element.Attribute("Command")?.Value == "{Binding RebuildCommand}"
        );
    }
    ...
    public void BoxelStatisticsOffersDedicatedAverageExplanation()
    {
        ...
        Assert.Contains("Body count for that type \u00f7 systems recorded", dialogValues);
        Assert.Contains(dialogValues, value => value.Contains("Selected boxel uses only", StringComparison.Ordinal));
        Assert.Contains(
            dialogValues,
            value => value.Contains("Entire saved search combines", StringComparison.Ordinal)
        );
        Assert.Contains(
            dialogValues,
            value => value.Contains("Nav Beacon setting affects only", StringComparison.Ordinal)
        );
    }
    ...
    public void BoxelStatisticsExplainsExactFilteringSettingsAndNativeExport()
    {
        ...
        Assert.DoesNotContain(
            values,
            value => value.Contains("Unvisited children still show", StringComparison.Ordinal)
        );
        Assert.Contains("Count Nav Beacon scans as FSS complete (statistics only)", values);
        Assert.Contains(
            values,
            value => value.Contains("does not affect boxel-search completion", StringComparison.Ordinal)
        );
        Assert.Contains(values, value => value.Contains("Settings apply immediately", StringComparison.Ordinal));
        ...
        var entireSearchScope = scopePanel
            ...
            .Single(element =>
                ...
                && element.Attribute("Content")?.Value == "{Binding EntireSavedSearchScopeText}"
            );
        Assert.Equal("{Binding CanShowSearchRollup}", entireSearchScope.Attribute("IsEnabled")?.Value);
        Assert.Contains("{Binding MinSystemsForAverages, Mode=TwoWay}", values);
        Assert.Contains("{Binding MinSystemsForExport, Mode=TwoWay}", values);
        Assert.Equal(
            2,
            window
                .Descendants()
    ...
    public void BoxelOverlaySettingsContainOnlyAccurateBoxelControls()
    {
        ...
        Assert.Contains("{Binding Notifications.CurrentBoxelSearchStatus, Mode=TwoWay}", values);
        Assert.Contains("{Binding Notifications.ShowNextBoxelToSearch, Mode=TwoWay}", values);
        Assert.Contains(values, value => value.Contains("mutually exclusive", StringComparison.Ordinal));
    }

    private static XDocument LoadView(string fileName) =>
        XDocument.Load(Path.Combine(FindRepositoryRoot(), "src", "SrvSurvey.Desktop", "Views", fileName));

    private static XDocument LoadStyles() =>
        XDocument.Load(Path.Combine(FindRepositoryRoot(), "src", "SrvSurvey.Desktop", "Styles", "RavenStyles.axaml"));
