    ...
    public async Task WorkspaceBuildsExactRowsFocusesCurrentBodyAndOpensLinks()
    {
        ...
        survey.ApplyUpdate(
            [
                ...
                Parse(
                    """{"event":"FSSBodySignals","SystemAddress":42,"BodyName":"Test 2","BodyID":2,"Signals":[{"Type":"$SAA_SignalType_Biological;","Count":1}],"Genuses":[{"Genus":"$Codex_Ent_Bacterial_Genus_Name;","Genus_Localised":"Bacterium"}]}"""
                ),
                Parse("""{"event":"Disembark","Body":"Test 1","BodyID":1,"OnPlanet":true,"OnStation":false}"""),
            ],
            new EliteStatus { GuiFocus = GuiFocus.SystemMap, BodyName = "Test 1" }
        );
        ...
        currentBody = Assert.Single(viewModel.Bodies, body => body.BodyId == 1);
        ...
        Assert.False(Assert.Single(viewModel.Bodies, body => body.BodyId == 2).IsExpanded);
        Assert.True(new BiologyPredictionsSettingsStore(settingsPath).Load().CurrentBodyOnly);
