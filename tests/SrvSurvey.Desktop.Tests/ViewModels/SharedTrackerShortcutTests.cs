...
{
    private readonly string root = Path.Combine(Path.GetTempPath(), $"SrvSurvey-shared-trackers-{Guid.NewGuid():N}");
    ...
    )
    {
        ...
        viewModel.SystemSurvey.ApplyUpdate(
            ...
            nextParkedSrvType: parkedType
        );
        var session = new SurfaceSurveySessionContext(
            "F123",
            "Test Cmdr",
            "Test System",
            42,
            ...
            BodyId: 7,
            BodyName: "Test System 1",
            BodyRadiusMeters: 1000
        );
        await viewModel.SurfaceSurvey.ApplyUpdateAsync(session, [], status, ExobiologySnapshot.Empty);
        await viewModel.Mining.ApplyUpdateAsync(
            session,
            viewModel.SystemSurvey.Snapshot,
            status,
            srvType,
            parkedSrvType: parkedType
        );

        Assert.Equal(expectedHandled, await viewModel.ToggleTrackerOrMiningRigAsync(number));
        Assert.Equal(aboardRhino && expectedHandled ? 1 : 0, viewModel.Mining.Rigs.Count(rig => rig.IsSet));
        Assert.Equal(
            !aboardRhino && !parkedRhino && expectedHandled ? 1 : 0,
    ...
    public async Task LiveChatClearsBothStoresButReplayedChatDoesNotClearSavedRigs()
    {
        ...
        await File.WriteAllTextAsync(
            ...
            $$"""
            {"timestamp":"2026-09-05T12:00:04Z","event":"Status","Flags":{{(long)(
                StatusFlags.InSrv | StatusFlags.HasLatLong
            )}},"Latitude":0,"Longitude":0,"PlanetRadius":1000,"BodyName":"Test System 1"}
            """
        );
        using (
