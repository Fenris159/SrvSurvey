...
public sealed class NotificationViewModelTests : IDisposable
{
    ...
    public void JournalNotificationsUseLiveStateAndNeverReplayBootstrapMessages()
    {
        ...
        Assert.Contains(
            viewModel.Messages,
            message => message.Text == "Collected: 3x Pattern Epsilon Obelisk Data, new total 10"
        );
        Assert.Contains(viewModel.Messages, message => message.Text == "Deliver Bertrandite: 176 units remaining");
    ...
    public void OverlayInteractionReportsBothModeTransitions()
    {
        ...
        Assert.Collection(
            viewModel.Messages,
            message => Assert.Equal("Overlay mouse interaction enabled", message.Text),
            message => Assert.Equal("Overlay mouse interaction disabled", message.Text)
        );
    }
    ...
    public void MaterialTradesAndTechnologyBrokerKeepPickupTotalsAccurate()
    {
        ...
        Assert.Contains(viewModel.Messages, message => message.Text == "Collected: 1x iron, new total 11");
        Assert.Contains(viewModel.Messages, message => message.Text == "Collected: 1x selenium, new total 3");
        Assert.Contains(
            viewModel.Messages,
            message => message.Text == "Collected: 1x Pattern Epsilon Obelisk Data, new total 3"
        );
    }
    ...
    public void MalformedMaterialTradeDoesNotPartiallyChangeInventory()
    {
        ...
        Assert.Equal("Collected: 1x iron, new total 21", Assert.Single(viewModel.Messages).Text);
    }
    ...
    public void BoxelScreenshotUploadAndBannerMessagesMatchLegacyWording()
    {
        ...
        viewModel.ReportScreenshotResult(
            new ScreenshotProcessingResult(
                [new ScreenshotConversion("source.bmp", Path.Combine("target", "shot.png"), false, null)],
                []
            ),
        ...
        Assert.Contains(viewModel.Messages, message => message.Text == "Current boxel 100% searched.");
        Assert.Contains(viewModel.Messages, message => message.Text == "Next boxel to search: Synuefe AA-A b1-3");
        Assert.Contains(viewModel.Messages, message => message.Text == "Saved 'shot.png' with no banner");
        Assert.Contains(viewModel.Messages, message => message.Text == "Congrats, GGG #1 GGG uploaded!");
        Assert.Contains(viewModel.Messages, message => message.Text == "Adding embedded banner to future screenshots");
    }
    ...
    private NotificationViewModel CreateViewModel(TimeProvider timeProvider)
    {
        ...
        return new NotificationViewModel(
            new NotificationSettingsStore(Path.Combine(temporaryDirectory, "ui-settings.json")),
            timeProvider
        );
    }
