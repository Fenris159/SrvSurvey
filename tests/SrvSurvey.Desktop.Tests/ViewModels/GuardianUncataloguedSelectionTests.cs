...
public sealed class GuardianUncataloguedSelectionTests
{
    ...
    public async Task DefaultDistanceUsesSolEvenWhileCommanderIsAtSelectedSite()
    {
        ...
        try
        {
            ...
            var survey = new GuardianCommanderSiteSurvey(
                ...
                "Ancient Ruins (1)",
                "Drew",
                DateTimeOffset.Parse("2026-08-27T12:00:00Z", global::System.Globalization.CultureInfo.InvariantCulture),
                DateTimeOffset.Parse("2026-08-27T12:00:00Z", global::System.Globalization.CultureInfo.InvariantCulture),
                "Beta",
                1,
                9000000000004,
            ...
            await new GuardianCommanderSurveyStore(root).SaveAsync("F123", isOdyssey: true, survey);
    ...
    public async Task InitialSiteTypeRevealsAndOpensActiveUncataloguedSurvey()
    {
        ...
        try
        {
            ...
            await viewModel.ApplyJournalEventsAsync(
                [
                    Parse(
                        """{"event":"Location","StarSystem":"Diagnostic Uncatalogued","SystemAddress":9000000000001}"""
                    ),
                    Parse(
                        """{"event":"ApproachSettlement","Name":"$Ancient:#index=1;","Name_Localised":"Ancient Ruins (1)","SystemAddress":9000000000001,"BodyID":7,"BodyName":"Diagnostic Uncatalogued A 1","Latitude":10,"Longitude":20}"""
                    ),
                ],
                "Drew"
            );
            ...
            await viewModel.ApplyJournalEventsAsync([Parse("""{"event":"SendText","Message":"b"}""")], "Drew");

            Assert.Equal(string.Empty, viewModel.FilterText);
            Assert.Equal(9000000000001, viewModel.SelectedSite?.Reference.SystemAddress);
    ...
    public async Task SavingHeadingPreservesSelectionAndEnablesLiveMapAuthoring()
    {
        ...
        try
        {
            ...
            await viewModel.ApplyJournalEventsAsync(
                [
                    Parse(
                        """{"event":"Location","StarSystem":"Diagnostic Uncatalogued","SystemAddress":9000000000002}"""
                    ),
                    Parse(
                        """{"event":"ApproachSettlement","Name":"$Ancient:#index=1;","Name_Localised":"Ancient Ruins (1)","SystemAddress":9000000000002,"BodyID":7,"BodyName":"Diagnostic Uncatalogued A 1","Latitude":10,"Longitude":20}"""
                    ),
            ...
            Assert.Equal(9000000000002, viewModel.SelectedSite?.Reference.SystemAddress);
    ...
    public async Task NewLocalSiteUsesCurrentSystemPositionAndBodyArrivalDistance()
    {
        ...
        try
        {
            var viewModel = new GuardianViewModel(root);
            await viewModel.LoadProfileAsync("F123", isOdyssey: true);
            viewModel.UpdateCurrentSystem("Diagnostic Uncatalogued", new GalacticCoordinate(100.5, 200.25, -300.75));
            await viewModel.ApplyJournalEventsAsync(
                [
                    Parse(
                        """{"event":"Location","StarSystem":"Diagnostic Uncatalogued","SystemAddress":9000000000003,"StarPos":[9,8,7]}"""
                    ),
                    Parse(
                        """{"event":"Scan","ScanType":"Detailed","StarSystem":"Diagnostic Uncatalogued","SystemAddress":9000000000003,"BodyID":7,"BodyName":"Diagnostic Uncatalogued A 1","DistanceFromArrivalLS":1234.5}"""
                    ),
                    Parse(
                        """{"event":"ApproachSettlement","Name":"$Ancient:#index=1;","Name_Localised":"Ancient Ruins (1)","SystemAddress":9000000000003,"BodyID":7,"BodyName":"Diagnostic Uncatalogued A 1","Latitude":10,"Longitude":20}"""
                    ),
                ],
                "Drew"
            );
        ...
        try
        {
            ...
            await viewModel.ApplyJournalEventsAsync(
                [
                    ...
                    Parse(
                        """{"event":"ApproachSettlement","Name":"$Ancient:#index=1;","Name_Localised":"Ancient Ruins (1)","SystemAddress":9000000000011,"BodyID":7,"BodyName":"Local One A 1","Latitude":10,"Longitude":20}"""
                    ),
                    Parse("""{"event":"Location","StarSystem":"Local Two","SystemAddress":9000000000012}"""),
                    Parse(
                        """{"event":"ApproachSettlement","Name":"$Ancient:#index=1;","Name_Localised":"Ancient Ruins (1)","SystemAddress":9000000000012,"BodyID":8,"BodyName":"Local Two B 2","Latitude":11,"Longitude":21}"""
                    ),
                ],
                "Drew"
            );
