...
public sealed class CodexImageSettingsStoreTests : IDisposable
{
    ...
    public async Task LoadSavePreserveUnknownSettingsAndUsePortableDefault()
    {
        ...
        Assert.Equal(new CodexImagePreferences(Path.GetFullPath(defaultCache), null, false), store.Load());
        ...
        var saved = Assert.IsType<JsonObject>(JsonNode.Parse(await File.ReadAllTextAsync(path)));
        Assert.True(saved["FutureRoot"]?.GetValue<bool>());
        Assert.Equal(42, saved["CodexImages"]?["FutureSetting"]?.GetValue<int>());
    }
