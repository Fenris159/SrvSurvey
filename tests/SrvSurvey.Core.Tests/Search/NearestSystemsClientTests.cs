...
public sealed class NearestSystemsClientTests
{
    ...
    public async Task CanonnSearchEnrichesNearestSystemsWithBiologyNotes()
    {
        ...
        {
            if (request.RequestUri!.AbsolutePath.EndsWith("/nearest/codex", StringComparison.Ordinal))
            {
        ...
        Assert.Equal("No bio signals in system", result.Rows[1].Notes);
        Assert.All(result.Rows, row => Assert.Equal(NearestSystemSource.Canonn, row.Source));
    }
    ...
    public async Task SpanshVariantSearchUsesLegacyShapeAndLimitsUniqueSystems()
    {
        ...
        Assert.Equal(1, root.GetProperty("reference_coords").GetProperty("x").GetDouble());
    }
    ...
    private static string Body(string name, string system, long address, double distance, string variant)
    {
        return $$"""
