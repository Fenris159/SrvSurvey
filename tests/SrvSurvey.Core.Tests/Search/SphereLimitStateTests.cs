...
public sealed class SphereLimitStateTests
{
    ...
    public void EnablePersistsCenterAndUsesEuclideanDistance()
    {
        ...
        Assert.Equal(new SphereLimitSnapshot(true, "Center", center.Position, 100), state.CreateSnapshot());
    }
    ...
    {
        var state = new SphereLimitState();
        var center = new StarSystemReference("Sol", 10477373803, new GalacticCoordinate(0, 0, 0));
        state.TryEnable(center, 250, out _);

        state.Disable();

        Assert.Equal(new SphereLimitSnapshot(false, "Sol", center.Position, 250), state.CreateSnapshot());
    ...
    {
        var state = new SphereLimitState(new SphereLimitSnapshot(true, null, null, -10));
