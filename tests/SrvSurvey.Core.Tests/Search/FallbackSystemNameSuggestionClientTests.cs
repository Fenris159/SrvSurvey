...
public sealed class FallbackSystemNameSuggestionClientTests
{
    ...
    {
        var primary = new StubClient(_ => Task.FromResult<IReadOnlyList<SystemNameSuggestion>>([]));
        var fallback = new StubClient(_ =>
            Task.FromResult<IReadOnlyList<SystemNameSuggestion>>([
    ...
    public async Task UserCancellationDoesNotQueryFallback()
    {
        ...
        await Assert.ThrowsAnyAsync<OperationCanceledException>(() => client.SearchAsync("Sol", cancellation.Token));
        Assert.Equal(0, fallback.CallCount);
    }
    ...
    {
        var primary = new StubClient(_ => throw new InvalidOperationException("Unexpected"));
        var fallback = new StubClient(_ => Task.FromResult<IReadOnlyList<SystemNameSuggestion>>([]));
        var client = new FallbackSystemNameSuggestionClient(primary, fallback);

        await Assert.ThrowsAsync<InvalidOperationException>(() => client.SearchAsync("Sol"));

        Assert.Equal(0, fallback.CallCount);
    }

    private sealed class StubClient(Func<CancellationToken, Task<IReadOnlyList<SystemNameSuggestion>>> search)
