...
public sealed class GuardianSiteMapProjectorTests
{
    ...
    public void ProjectionAppliesCommanderMarkerOffsetWithoutMutatingTemplate()
    {
        var template = new GuardianSiteTemplate(
            ...
            [new GuardianPointOfInterest("p1", GuardianPoiType.Orb, 0, 10, 0)],
            [],
            new Dictionary<string, GuardianMapPoint> { ["A"] = new GuardianMapPoint(0, 15) }
        );
        ...
        AssertPoint(Assert.Single(projection.Groups).X, Assert.Single(projection.Groups).Y, 12, 8);
    ...
    public void SelectedGroupsFilterObelisksAndProjectLabels()
    {
        var template = new GuardianSiteTemplate(
            ...
            1,
            [
                new GuardianPointOfInterest("A01", GuardianPoiType.Obelisk, 0, 10, 0),
                new GuardianPointOfInterest("B01", GuardianPoiType.Obelisk, 90, 20, 0),
                new GuardianPointOfInterest("p1", GuardianPoiType.Orb, 180, 30, 0),
            ],
            [],
            new Dictionary<string, GuardianMapPoint>
            {
    ...
    public void ComponentModeProjectsTowerMaterialsAndDestructiblePanels()
    {
        var template = new GuardianSiteTemplate(
            ...
            new GuardianMapPoint(0, 0),
            1,
            [new GuardianPointOfInterest("c1", GuardianPoiType.Component, 0, 10, 0)],
            [new GuardianPointOfInterest("d1", GuardianPoiType.DestructiblePanel, 90, 20, 0)],
            new Dictionary<string, GuardianMapPoint>()
        );
        var survey = new GuardianSurveyData
        {
    ...
    public void ProjectsLegacyHeadingAndRamTahRenderingState()
    {
        var template = new GuardianSiteTemplate(
            ...
            1,
            [
                new GuardianPointOfInterest("t1", GuardianPoiType.Relic, 0, 10, 0),
                new GuardianPointOfInterest("t2", GuardianPoiType.Relic, 90, 20, 0),
                new GuardianPointOfInterest("A01", GuardianPoiType.Obelisk, 180, 30, 25),
                new GuardianPointOfInterest("e1", GuardianPoiType.EmptyPuddle, 270, 40, 0),
            ],
            [],
            new Dictionary<string, GuardianMapPoint>()
        );
        ...
        var projection = new GuardianSiteMapProjector().Project(
            ...
            neededRamTahLogCodes: new HashSet<string>(["H1"], StringComparer.OrdinalIgnoreCase)
        );
        ...
        Assert.Equal(GuardianPoiStatus.Empty, projection.Points.Single(point => point.Name == "e1").Status);
    }
    ...
    private static GuardianSiteTemplate CreateTemplate(IReadOnlyList<GuardianPointOfInterest> points)
    {
    ...
    private static void AssertPoint(GuardianProjectedPoint point, double x, double y)
    {
        AssertPoint(point.X, point.Y, x, y);
    }

    private static void AssertPoint(double actualX, double actualY, double expectedX, double expectedY)
    {
