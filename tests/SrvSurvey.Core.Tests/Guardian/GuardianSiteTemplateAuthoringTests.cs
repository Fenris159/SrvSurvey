...
public sealed class GuardianSiteTemplateAuthoringTests : IDisposable
{
    ...
    public void DraftEditsDoNotMutateSourceTemplate()
    {
        ...
        session.AddPoint(new GuardianPointOfInterest("p2", GuardianPoiType.Orb, 45, 20, 0));
        session.AddPoint(new GuardianPointOfInterest("d1", GuardianPoiType.DestructiblePanel, 90, 30, 10));
        session.UpdatePoint("p1", new GuardianPointOfInterest("p1", GuardianPoiType.Tablet, 180, 40, 0));
        ...
        Assert.Equal(new GuardianMapPoint(90, 50), session.Template.ObeliskGroupNameLocations["B"]);
    }
