...
public sealed class GuardianSurveyShareServiceTests : IDisposable
{
    ...
    public void DiscoveryReasonsMatchLegacyComparisonRules()
    {
        ...
        var survey = Survey(
            ...
            new GuardianSurveyData
            {
                ...
                RawPointsOfInterest = [new GuardianPointOfInterest("New", GuardianPoiType.Pylon, 1, 2, 3)],
            }
        );
    ...
    public async Task CompleteSurveyRoundTripsIntoLegacyShareArchive()
    {
        ...
        var survey = new GuardianCommanderSiteSurvey(
            ...
            new GuardianSurveyData
            {
                ...
                RelicHeadings = new Dictionary<string, int> { ["t1"] = 135 },
                ...
                RawPointsOfInterest = [new GuardianPointOfInterest("x1", GuardianPoiType.Urn, 10.5, 25, 180)],
            },
        ...
        var template = new GuardianSiteTemplate(
            ...
            1,
            [
                new GuardianPointOfInterest("c1", GuardianPoiType.Component, 0, 0, 0),
                new GuardianPointOfInterest("p1", GuardianPoiType.Orb, 0, 0, 0),
                new GuardianPointOfInterest("p2", GuardianPoiType.Casket, 0, 0, 0),
                new GuardianPointOfInterest("t1", GuardianPoiType.Relic, 0, 0, 0),
            ],
            [],
        ...
        Assert.Equal("Complete field survey", root.GetProperty("notes").GetString());
        Assert.Equal(12.5, root.GetProperty("location").GetProperty("lat").GetDouble());
        Assert.Equal(-34.25, root.GetProperty("location").GetProperty("long").GetDouble());
        Assert.Equal(6.5, root.GetProperty("mapMarkerOffset").GetProperty("x").GetDouble());
        Assert.Equal(-3.25, root.GetProperty("mapMarkerOffset").GetProperty("y").GetDouble());
        ...
        Assert.Equal("A01!-ca,or-H1-", root.GetProperty("activeObelisks")[0].GetString());
        Assert.Equal(135, root.GetProperty("relicHeadings").GetProperty("t1").GetInt32());
        Assert.Equal("urn", root.GetProperty("rawPoi")[0].GetProperty("type").GetString());
        Assert.Equal(
            ["c1,cell,conduit,tech", "d1,tech"],
            root.GetProperty("components").EnumerateArray().Select(item => item.GetString()!).ToArray()
        );
    }
    ...
    {
        return Path.Combine(temporaryDirectory, "guardian", FrontierId, fileName);
    }
    ...
    private static GuardianCommanderSiteSurvey Survey(string path, GuardianSurveyData data)
    {
