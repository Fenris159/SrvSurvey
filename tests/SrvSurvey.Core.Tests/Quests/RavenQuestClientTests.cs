...
public sealed class RavenQuestClientTests
{
    ...
    public async Task LoadsPublishedDefinitionsWithOptionalApiKey()
    {
        var client = Create(
            new StubHandler(request =>
            {
                ...
                Assert.Equal("/root/api/quest/published", request.RequestUri!.AbsolutePath);
    ...
    public async Task LoadsSpecificDefinitionUsingEscapedInvariantIdentity()
    {
        var client = Create(
            ...
            {
                Assert.Equal(HttpMethod.Get, request.Method);
                Assert.Equal("/root/api/quest/Publisher%20Name/quest%2Fone/1.5/", request.RequestUri!.AbsolutePath);
                return Json(
                    """
    ...
    public async Task LoadsCommanderProgressWithoutDroppingFutureFields()
    {
        var client = Create(
            new StubHandler(request =>
            {
                ...
                Assert.Equal("/root/api/quest/cmdr/load/active", request.RequestUri!.AbsolutePath);
                Assert.Equal("secret-key", Assert.Single(request.Headers.GetValues("rcc-key")));
                return Json(
                    """
                    [{
        ...
        Assert.Equal(7, quest.ExtensionData["futureState"].GetProperty("value").GetInt32());
    }
    ...
    public async Task SavesCommanderProgressWithLegacyPropertyNamesAndApiKey()
    {
        ...
        var client = Create(
            ...
            {
                Assert.Equal(HttpMethod.Post, request.Method);
                Assert.Equal("/root/api/quest/cmdr/save/Raven/sample", request.RequestUri!.AbsolutePath);
                Assert.Equal("secret-key", Assert.Single(request.Headers.GetValues("rcc-key")));
        ...
        var quest = new RavenCommanderQuest
        {
            ...
            Variables = new Dictionary<string, JsonElement> { ["counter"] = JsonSerializer.SerializeToElement(42) },
        ...
        Assert.Equal(42, root.GetProperty("vars").GetProperty("counter").GetInt32());
    ...
    public async Task CatalogStatusAndActivationUseLegacyEndpoints()
    {
        ...
        Assert.Equal(
            [(HttpMethod.Get, "/root/api/quest/cmdr"), (HttpMethod.Put, "/root/api/quest/cmdr/Raven/sample")],
            requests
        );
    }
    ...
    public async Task MutationAndChapterEndpointsMatchLegacyContract()
    {
        ...
        var client = Create(
            ...
            {
                requests.Add((request.Method, request.RequestUri!.AbsolutePath));
                return request.RequestUri.AbsolutePath.Contains("/chapter/", StringComparison.Ordinal)
    ...
    public async Task PublishSendsDefinitionAndPreservesLegacyDurationCasing()
    {
        ...
        var client = Create(
            ...
            {
                Assert.Equal(HttpMethod.Post, request.Method);
                Assert.Equal("/root/api/quest/publish", request.RequestUri!.AbsolutePath);
                Assert.Equal("secret-key", Assert.Single(request.Headers.GetValues("rcc-key")));
                body = await request.Content!.ReadAsStringAsync();
                return new HttpResponseMessage(HttpStatusCode.OK) { ReasonPhrase = "Published" };
            })
        );
        ...
        Assert.Equal("Extended", JsonDocument.Parse(body!).RootElement.GetProperty("duration").GetString());
    }
    ...
    public async Task LegacyUnavailableResponsesRemainNonFatalForReadSurfaces()
    {
        ...
        Assert.Null(await client.GetQuestChapterAsync(new RavenQuestReference("Raven", "sample", 1), "start"));
    }
    ...
    {
        return new RavenQuestClient(new HttpClient(handler), new Uri("https://example.test/root/"));
    }
