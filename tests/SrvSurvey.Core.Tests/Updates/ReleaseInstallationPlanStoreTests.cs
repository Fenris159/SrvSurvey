...
public sealed class ReleaseInstallationPlanStoreTests : IDisposable
{
    ...
    public async Task CreateAndLoadRoundTripsBoundedHandoffPlan()
    {
        ...
        Assert.Equal(created.ParentProcessStartTimeUtcTicks, loaded.ParentProcessStartTimeUtcTicks);
        Assert.Equal(created.HealthToken, loaded.HealthToken);
        IReadOnlyList<string> noArguments = Array.Empty<string>();
        Assert.Equal(
            created.Preparation with
            {
                StartupArguments = noArguments,
            },
            loaded.Preparation with
            {
                StartupArguments = noArguments,
            }
        );
        Assert.Equal(created.Preparation.StartupArguments, loaded.Preparation.StartupArguments);
        ...
        Assert.Empty(Directory.GetFiles(Path.GetDirectoryName(created.PlanPath)!, "*.tmp"));
    }
    ...
    public async Task OutcomeMustMatchPlanAndIsAtomicallyReplaced()
    {
        ...
        Assert.Equal("RolledBack", document.RootElement.GetProperty("status").GetString());
        Assert.Equal("health timeout", document.RootElement.GetProperty("Error").GetString());
        Assert.Empty(Directory.GetFiles(Path.GetDirectoryName(plan.OutcomePath)!, "*.tmp"));
    }
