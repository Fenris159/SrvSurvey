...
public sealed class GreenGasGiantClientTests
{
    ...
    public async Task PublishesLegacyCompatiblePayload()
    {
        ...
        var client = new GreenGasGiantClient(new HttpClient(handler), new Uri("https://example.test/root/"));
        ...
        Assert.Equal(
            [1.5, -2, 3],
            root.GetProperty("starPos").EnumerateArray().Select(value => value.GetDouble()).ToArray()
        );
        Assert.Equal("{\"event\":\"Scan\"}", root.GetProperty("json").GetString());
    }
