...
public sealed class CargoInventoryStateTests
{
    ...
    public void JournalDeltasMaintainAUnifiedShipInventory()
    {
        ...
        state.Reset(Snapshot("Ship", new CargoItem("gold", "Gold", 2, 1), new CargoItem("water", "Water", 3, 0)));

        Assert.True(state.Apply(Event("CollectCargo", "\"Type\":\"silver\",\"Type_Localised\":\"Silver\"")));
        Assert.True(state.Apply(Event("EjectCargo", "\"Type\":\"gold\",\"Count\":1")));
        Assert.True(state.Apply(Event("MarketBuy", "\"Type\":\"water\",\"Count\":2")));
        Assert.True(state.Apply(Event("MarketSell", "\"Type\":\"water\",\"Count\":1")));
        Assert.True(
            state.Apply(
                Event("ColonisationContribution", "\"Contributions\":[{\"Name\":\"$Water_name;\",\"Amount\":2}]")
            )
        );
        Assert.True(
            state.Apply(
        ...
        Assert.Equal(1, Assert.Single(snapshot.Inventory, item => item.Name == "gold").Stolen);
    }
    ...
    {
        var state = new CargoInventoryState();
        state.Reset(Snapshot("SRV", new CargoItem("ancientrelic", null, 2, 0)));
    ...
    {
        var state = new CargoInventoryState();
        state.Apply(Event("CollectCargo", "\"Type\":\"gold\",\"Type_Localised\":\"Gold\""));
    ...
    {
        var state = new CargoInventoryState();
        state.Reset(Snapshot("Ship", new CargoItem("gold", "Gold", 1, 0)));
    ...
    {
        var state = new CargoInventoryState();
        state.Reset(Snapshot("Ship", new CargoItem("steel", "Steel", 50, 0)));
        ...
        Assert.True(
            state.Apply(
                Event("CargoTransfer", "\"Transfers\":[{\"Type\":\"steel\",\"Count\":10,\"Direction\":\"tocarrier\"}]")
            )
        );
        ...
        state.Reset(Snapshot("Ship", new CargoItem("steel", "Steel", 40, 0)));
        Assert.True(state.HasPreservedSnapshot);
        Assert.Equal(40, state.CreateSnapshot()!.GetCount("steel"));
    ...
    {
        var state = new CargoInventoryState();
        state.Reset(Snapshot("Ship", new CargoItem("water", "Water", 3, 0)));
        state.CaptureBeforeSnapshot();
        Assert.True(state.HasPreservedSnapshot);
