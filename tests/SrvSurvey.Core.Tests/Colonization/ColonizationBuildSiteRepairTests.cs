...
public sealed class ColonizationBuildSiteRepairTests
{
    ...
    public void RecognizesPlayerColonyMarketIdPrefixes(long marketId, bool expected)
    {
        Assert.Equal(expected, ColonizationBuildSiteRepair.IsPlayerColonyMarketId(marketId));
    }

    [Theory]
    [InlineData("FleetCarrier", "N4W-T0Z", false)]
    [InlineData("SpaceConstructionDepot", "Orbital Construction Site: Dampier Gateway", false)]
    [InlineData(null, "Planetary Construction Site: Example Base", false)]
    [InlineData("SurfaceStation", "Some ColonisationShip Pad", true)]
    public void SkipsUnsafeDockContexts(string? stationType, string stationName, bool isConstructionShip)
    {
        Assert.True(ColonizationBuildSiteRepair.ShouldSkipDockContext(stationType, stationName, isConstructionShip));
    }

    [Fact]
    public void AllowsCompletedPlayerStationDockContext()
    {
        Assert.False(ColonizationBuildSiteRepair.ShouldSkipDockContext("Dodec", "Gold Enterprise"));
    }

    [Theory]
    [InlineData("Orbital Construction Site: Dampier Gateway", "Dampier Gateway")]
    [InlineData("$EXT_PANEL_Colonisation;Saez Synthetics Facility", "Saez Synthetics Facility")]
    public void NormalizesJournalStationNames(string input, string expected)
    {
        Assert.Equal(expected, ColonizationBuildSiteRepair.NormalizeDockStationName(input));
    }
    ...
    public void RepairsMissingOrStaleMarketIdByUniqueCompletedName()
    {
        ...
        Assert.Equal(ColonizationBuildSiteRepairField.MarketId, missingPlan?.Field);
        Assert.Equal(4_310_999_999, missingPlan?.CreatePatch().MarketId);
        Assert.Equal(ColonizationBuildSiteRepairField.MarketId, stalePlan?.Field);
    }
    ...
    public void RejectsAmbiguousNamesEvenWhenOnlyOneRowIsEligible()
    {
        ...
        Assert.Null(ColonizationBuildSiteRepair.CreatePlan(sites, "Twin Hub", 4_310_999_999));
    }
    ...
    public void RejectsAmbiguousMarketIdNameRepair()
    {
        ...
        Assert.Null(ColonizationBuildSiteRepair.CreatePlan(sites, "Dampier Gateway", 4_310_999_999));
    }
    ...
    private static ColonizationSystemSite CompleteSite(string id, string name, long? marketId)
    {
