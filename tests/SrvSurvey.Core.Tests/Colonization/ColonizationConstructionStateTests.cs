...
public sealed class ColonizationConstructionStateTests
{
    ...
    public void TracksSquadronBankMusicContext()
    {
        ...
        Assert.True(state.Apply(Event("Music", "\"MusicTrack\":\"Squadrons\"")));

        Assert.True(state.CreateSnapshot().IsSquadronBankOpen);

        Assert.True(state.Apply(Event("Music", "\"MusicTrack\":\"DockingComputer\"")));
    ...
    public void TracksConstructionDockAndDepotRequirements()
    {
        ...
        Assert.Equal(DateTimeOffset.Parse("2026-07-24T12:00:00Z"), snapshot.CurrentDock.Timestamp);
    ...
    public void SessionExitClearsDockingContext(string eventName, string properties)
    {
        var state = new ColonizationConstructionState();
        state.Apply(
            Event(
    ...
    public void TracksClaimAndBeaconDeployment()
    {
        ...
        Assert.True(state.Apply(Event("ColonisationBeaconDeployed", string.Empty)));

        Assert.Equal("North America Sector PI-T c3-4", state.LastClaim?.SystemName);
    ...
    {
        Assert.Equal(expected, ColonizationConstructionState.NormalizeCommodityName(value));
    }
