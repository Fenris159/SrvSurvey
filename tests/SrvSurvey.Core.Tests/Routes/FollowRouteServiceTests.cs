...
public sealed class FollowRouteServiceTests : IDisposable
{
    ...
    {
        (FollowRouteService? service, FollowRouteDocument? route) = CreateServiceAndRoute(isActive: false);

        FollowRouteDocument updated = await service.SetActiveAsync(route, true, 1);
    ...
    public async Task EmptyAndCompletedRoutesCannotBeActivated()
    {
        ...
        (FollowRouteService _, FollowRouteDocument? complete) = CreateServiceAndRoute(
            isActive: false,
            lastReachedIndex: 2
        );

        FollowRouteDocument activatedEmpty = await service.SetActiveAsync(empty, true);
        FollowRouteDocument activatedComplete = await service.SetActiveAsync(complete, true);
    ...
    {
        (FollowRouteService? service, FollowRouteDocument? route) = CreateServiceAndRoute();

        FollowRouteArrivalResult second = await service.ApplyArrivalAsync(route, "second", 2);
        FollowRouteArrivalResult final = await service.ApplyArrivalAsync(
            second.Route,
    ...
    {
        (FollowRouteService? service, FollowRouteDocument? route) = CreateServiceAndRoute(lastReachedIndex: -1);

        FollowRouteArrivalResult outOfOrder = await service.ApplyArrivalAsync(route, "Second", 2);
        FollowRouteArrivalResult final = await service.ApplyArrivalAsync(route, "Third", 3);
    ...
    public async Task ArrivalMatchesNamesWithoutCaseAndIgnoresUnknownSystems()
    {
        ...
        FollowRouteArrivalResult unknown = await service.ApplyArrivalAsync(route, "Unknown", 99);
        FollowRouteArrivalResult first = await service.ApplyArrivalAsync(route, "sOl", null);
    ...
    public async Task ManualFinalProgressDisablesRouteAndUncheckingCanResume()
    {
        ...
        FollowRouteDocument complete = await service.SetProgressAsync(route, 99);
        FollowRouteDocument resumed = await service.ReplaceAsync(complete, complete.Hops, 0, true, complete.AutoCopy);
    ...
    {
        FollowRouteHop sol = Hop("Sol", 1, new GalacticCoordinate(0, 0, 0));
        FollowRouteHop other = Hop("Other", 2, new GalacticCoordinate(3, 4, 0));
        FollowRouteHop unresolved = Hop("Unknown", null, null);
