...
public sealed class GuardianBubbleLocatorTests
{
    ...
    {
        Assert.True(GuardianBubbleLocator.IsWithinKnownBubble(new GalacticCoordinate(x, y, z)));
    }
    ...
    {
        Assert.True(
            GuardianBubbleLocator.IsWithinKnownBubble(
                new GalacticCoordinate(1099.21875 + 749.99, -146.6875, -133.59375)
            )
        );
        Assert.False(
            GuardianBubbleLocator.IsWithinKnownBubble(new GalacticCoordinate(1099.21875 + 750, -146.6875, -133.59375))
        );
    }
}
