...
public sealed class HumanSiteMapProjectorTests
{
    ...
    public void EmbeddedTemplatesProjectCompleteFiniteMapGeometry()
    {
        ...
        Assert.Equal(398, projections.Sum(projection => projection.SecureDoors.Count));
        Assert.Equal(592, projections.Sum(projection => projection.NamedPoints.Count));
        Assert.Equal(143, projections.Sum(projection => projection.DataTerminals.Count));
        Assert.Empty(projections.SelectMany(projection => projection.ConflictZonePoints));
        Assert.Equal(3, projections.Sum(projection => projection.SkippedImplausiblePoints));
        Assert.All(
            projections,
            projection =>
            {
                ...
                Assert.All(
                    projection
                        .LandingPads.Concat(projection.SecureDoors)
    ...
    public void DisplayOptionsMatchLegacyPoiTogglesAndWarState()
    {
        ...
        Assert.DoesNotContain(hidden.NamedPoints, point => point.Name is "Medkit" or "Battery");
        Assert.Empty(hidden.DataTerminals);
        Assert.Equal(template.ConflictZonePoints.Count, hidden.ConflictZonePoints.Count);
    }
    ...
    public void ConvertsGdiLineAndCubicPathTypesWithoutSystemDrawing()
    {
        ...
        Assert.Equal(new HumanSiteMapPoint(15, 0), path.Segments[2].First);
        Assert.Equal(new HumanSiteMapPoint(10, 10), path.Segments[2].Third);
    }
    ...
    public void RejectsIncompleteCubicPath()
    {
        ...
        Assert.Throws<InvalidDataException>(() => new HumanSiteMapProjector().Project(template));
    }

    private static HumanSiteTemplate CreateTemplate(HumanSiteBuildingPath path)
    {
        return new HumanSiteTemplate(
            HumanSiteEconomy.Agriculture,
            1,
            "Test",
            [new HumanSiteLandingPad(new HumanSiteMapPoint(0, 0), 0, 0, 0, HumanSiteLandingPadSize.Small)],
            [],
            [],
            [],
            [],
