...
public sealed class EddnPublisherTests
{
    ...
    )
    {
        ...
        {
            Event(
                """{"event":"Fileheader","gameversion":"4.1.2.3","build":"r123/r0 ","Odyssey":true,"Horizons":true}"""
            ),
        };
    ...
    public async Task BootstrapBuildsContextAndLiveSchemasPublishThroughLiveGateway()
    {
        ...
        Assert.Equal(
            [1.5, -2, 3],
            message.GetProperty("StarPos").EnumerateArray().Select(value => value.GetDouble()).ToArray()
        );
        Assert.True(message.GetProperty("horizons").GetBoolean());
        Assert.True(message.GetProperty("odyssey").GetBoolean());
        Assert.False(message.GetProperty("Signals")[0].TryGetProperty("Type_Localised", out _));
    }
    ...
    public async Task JumpFlushesSignalBatchAgainstSourceSystem()
    {
        ...
        Assert.Equal("Test B", jumpPayload.RootElement.GetProperty("message").GetProperty("StarSystem").GetString());
    }
        ...
        try
        {
            ...
            await publisher.ApplyAsync(
                new EddnApplyRequest
                {
                    ...
                    [
                        Event(
                            """{"timestamp":"2026-07-25T12:00:00Z","event":"Fileheader","gameversion":"4.1.2.3","build":"r123/r0","Odyssey":true}"""
                        ),
                        Event(
                            """{"timestamp":"2026-07-25T12:00:01Z","event":"LoadGame","Commander":"Test Cmdr","Horizons":true,"Odyssey":true}"""
                        ),
    ...
    public async Task MulticrewAndSharedCompanionInputsAreSuppressedIndependently()
    {
        ...
        var crew = await publisher.ApplyAsync(
            new EddnApplyRequest
            {
                ...
                [
                    ...
                    Event(
                        """{"timestamp":"2026-07-25T12:01:01Z","event":"DockingGranted","MarketID":1,"StationName":"Port","LandingPad":2}"""
                    ),
                ],
                Status = null,
                Enabled = true,
        ...
        Assert.Contains(
            shared.Warnings,
            warning => warning.Contains("shared companion files are suppressed", StringComparison.Ordinal)
        );
    ...
    public async Task OperationalSuspensionPreservesQueueAndBlocksNewMessages()
    {
        ...
        var queued = await publisher.ApplyAsync(
            new EddnApplyRequest
            {
                ...
                [
                    Event(
                        """{"timestamp":"2026-07-25T12:01:00Z","event":"DockingGranted","MarketID":1,"StationName":"First Port","LandingPad":2}"""
                    ),
                ],
                Status = null,
                Enabled = true,
        ...
        var blocked = await publisher.ApplyAsync(
            new EddnApplyRequest
            {
                ...
                [
                    Event(
                        """{"timestamp":"2026-07-25T12:02:00Z","event":"DockingGranted","MarketID":2,"StationName":"Second Port","LandingPad":3}"""
                    ),
                ],
                Status = null,
                Enabled = true,
        ...
        Assert.Contains(
            blocked.Warnings,
            warning => warning.Contains("multiple Elite windows", StringComparison.Ordinal)
        );
        Assert.Equal(1, publisher.PendingCount);
        Assert.Empty(requests);
    ...
    public async Task QueuedMessagesRetainTheirCapturedCommanderAcrossSessionSwitch()
    {
        ...
        await publisher.ApplyAsync(
            new EddnApplyRequest
            {
                ...
                [
                    Event(
                        """{"timestamp":"2026-07-25T12:00:00Z","event":"LoadGame","Commander":"First Cmdr","FID":"F1","Odyssey":true}"""
                    ),
                    Event(
                        """{"timestamp":"2026-07-25T12:00:01Z","event":"DockingGranted","MarketID":1,"StationName":"First Port","LandingPad":2}"""
                    ),
                ],
                Enabled = true,
                AllowPublishing = true,
                ...
                GameVersion = "4.1",
                GameBuild = "r1",
                JournalPath = Path.Combine(Path.GetTempPath(), "Journal.2026-07-25T120000.01.log"),
            }
        );
        await publisher.ApplyAsync(
            new EddnApplyRequest
            {
                ...
                [
                    Event(
                        """{"timestamp":"2026-07-25T12:01:00Z","event":"LoadGame","Commander":"Second Cmdr","FID":"F2","Odyssey":true}"""
                    ),
                    Event(
                        """{"timestamp":"2026-07-25T12:01:01Z","event":"DockingGranted","MarketID":2,"StationName":"Second Port","LandingPad":3}"""
                    ),
                ],
                Enabled = true,
                AllowPublishing = true,
                ...
                GameVersion = "4.1",
                GameBuild = "r2",
                JournalPath = Path.Combine(Path.GetTempPath(), "Journal.2026-07-25T120100.01.log"),
            }
        );
        ...
        Assert.Equal(
            ...
            requests
                .Select(request => JsonDocument.Parse(request.Content))
                .Select(payload => payload.RootElement.GetProperty("header").GetProperty("uploaderID").GetString())
                .OfType<string>()
                .ToArray()
        );
    }
    ...
    public async Task ContinuedJournalPartPreservesCommanderIdentityAndContext()
    {
        ...
        await publisher.ApplyAsync(
            new EddnApplyRequest
            {
                ...
                [
                    Event(
                        """{"timestamp":"2026-07-25T12:00:00Z","event":"Fileheader","part":1,"gameversion":"4.1","build":"r1","Odyssey":true}"""
                    ),
                    Event(
                        """{"timestamp":"2026-07-25T12:00:01Z","event":"LoadGame","Commander":"Test Cmdr","Odyssey":true}"""
                    ),
                    Event(
        ...
        await publisher.ApplyAsync(
            new EddnApplyRequest
            {
                ...
                [
                    Event(
                        """{"timestamp":"2026-07-25T12:10:00Z","event":"Fileheader","part":2,"gameversion":"4.1","build":"r2","Odyssey":true}"""
                    ),
        ...
        var result = await publisher.ApplyAsync(
            new EddnApplyRequest
            {
                ...
                [
                    Event(
                        """{"timestamp":"2026-07-25T12:11:00Z","event":"FSSBodySignals","SystemAddress":123,"BodyName":"Test A 1","BodyID":4,"Signals":[{"Type":"$SAA_SignalType_Biological;","Count":2}]}"""
                    ),
                ],
                Status = null,
                Enabled = true,
    ...
    public async Task UnrelatedJournalFileRequiresFreshCommanderIdentity(int part)
    {
        ...
        await publisher.ApplyAsync(
            new EddnApplyRequest
            {
                ...
                [
                    Event(
                        """{"timestamp":"2026-07-25T12:00:00Z","event":"Fileheader","part":1,"gameversion":"4.1","build":"r1"}"""
                    ),
                    Event(
                        """{"timestamp":"2026-07-25T12:00:01Z","event":"LoadGame","Commander":"Test Cmdr","Odyssey":true}"""
                    ),
        ...
        await publisher.ApplyAsync(
            new EddnApplyRequest
            {
                ...
                [
                    Event(
                        $$"""{"timestamp":"2026-07-25T12:10:00Z","event":"Fileheader","part":{{part}},"gameversion":"4.1","build":"r2"}"""
                    ),
                ],
                Status = null,
                Enabled = true,
                AllowPublishing = false,
                JournalPath = Path.Combine(Path.GetTempPath(), "Journal.2026-07-25T121000.01.log"),
            }
        );
        ...
        var result = await publisher.ApplyAsync(
            new EddnApplyRequest
            {
                ...
                [
                    Event(
                        """{"timestamp":"2026-07-25T12:11:00Z","event":"DockingGranted","MarketID":1,"StationName":"Port","LandingPad":2}"""
                    ),
                ],
                Status = null,
                Enabled = true,
        ...
        try
        {
            ...
            await publisher.ApplyAsync(
                new EddnApplyRequest
                {
                    ...
                    [
                        ...
                        Event(
                            """{"timestamp":"2026-07-25T12:00:01Z","event":"LoadGame","Commander":"First Cmdr","Odyssey":true}"""
                        ),
                        Event(
                            """{"timestamp":"2026-07-25T12:00:02Z","event":"Location","StarSystem":"Test A","SystemAddress":123,"StarPos":[1,2,3]}"""
                        ),
            ...
            await publisher.ApplyAsync(
                new EddnApplyRequest
                {
                    ...
                    [
                        Event(
                            """{"timestamp":"2026-07-25T12:02:00Z","event":"Fileheader","gameversion":"4.1","build":"r2"}"""
                        ),
                    ],
                    Status = null,
                    Enabled = true,
    ...
    private static async Task BootstrapAsync(EddnPublisher publisher, params JournalEventEnvelope[] additionalEvents)
    {
        await publisher.ApplyAsync(
            new EddnApplyRequest
            {
    ...
    )
    {
        ...
        return new EddnPublisher(
            ...
            Path.Combine(Path.GetTempPath(), "SrvSurvey-EddnPublisherTests-" + Guid.NewGuid().ToString("N") + ".json"),
