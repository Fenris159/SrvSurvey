...
public sealed class OverlayExceptionsViewModel
{
    ...
    public void UpdateBoardedVehicle(JournalSessionState journal, EliteStatus? status)
    {
        ...
        if (next == boarded) return;
        boarded = next;
        Apply();
    }

    private void Apply() => registry.SetVehicleExcludedPlotters(
        OverlayLayoutCatalog.Supported.Where(panel => panel.SettingsCategories
            .Any(category => !ForCategory(category).Allows(boarded))).Select(panel => panel.Name));
}
...
public sealed class OverlayExceptionCategoryViewModel : WorkspaceObservable
{
    ...
    public OverlayExceptionCategoryViewModel(OverlaySettingsCategory category, OverlayVehicleSettingsStore store, Action apply)
    {
        Category = category;
        this.store = store;
        this.apply = apply;
    ...
    {
        batching = true;
        try { foreach (var entry in Entries) entry.IsAllowed = allowed; }
...
public sealed class OverlayExceptionEntryViewModel(OverlayVehicleDefinition definition, bool allowed, Action save) : WorkspaceObservable
{
    private bool isAllowed = allowed;
    public OverlayVehicleDefinition Definition { get; } = definition;
    public string Name => Definition.Name;
    public bool IsAllowed { get => isAllowed; set { if (Set(ref isAllowed, value)) save(); } }
}
