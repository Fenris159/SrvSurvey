...
) : WorkspaceObservable, IDisposable
{
    ...
    public string Reference
    {
        ...
        set { Set(ref options, options with { Reference = value ?? "" }); }
    }
    public string Mineral
    {
        get => options.Mineral;
        set { Set(ref options, options with { Mineral = value ?? "Platinum" }); }
    }
    public string RingType
    {
        get => options.RingType;
        set { Set(ref options, options with { RingType = value ?? "All" }); }
    }
    public string Commodity
    {
        get => options.Commodity;
        set { Set(ref options, options with { Commodity = value ?? "Platinum" }); }
    }
    public double Radius
    {
        get => options.Radius;
        set { Set(ref options, options with { Radius = double.IsFinite(value) ? Math.Clamp(value, 1, 500) : 100 }); }
    }
    ...
    {
        get => objective;
        set => Set(ref objective, value);
    }
    public string PledgedPower
    {
        get => pledgedPower;
        set => Set(ref pledgedPower, value ?? "");
    }
    ...
    private bool MatchesObjective(MiningSystemResult system) =>
        Objective switch
        {
            "Reinforce" => !Same(system.PowerState, ExpansionState)
                && system.Power.Length > 0
                && Same(system.Power, PledgedPower),
            "Undermine" => !Same(system.PowerState, ExpansionState)
                && system.Power.Length > 0
                && !Same(system.Power, PledgedPower),
