                ...
                [
                    ...
                    Section(
                        ...
                        [
                            ...
                            "Drag the frame over the six HUD circles. Resize it from the lower-right corner, leaving room for cockpit movement; resizing changes the capture area without stretching or moving the guides inside it. Drag each numbered red dot onto its matching circle centre. Keep the game's small labels unobstructed and inside the frame.",
                            "Use Size−/Size+ for circle diameter, Height−/Height+ for oval proportions, and R−/R+ for outline rotation. Gap−/Gap+ move the cyan bar curves closer to or farther below the circles without changing the outlines. Align them with the middle of the deployed rig's segmented bar. The curves show the nominal sampling position; detection searches a small distance around them and rejects matches on the circle rim. Each circle centre can be positioned independently to match the sloping cockpit HUD.",
                            "Search−/Search+ adjust the movement search allowance in pixels. The readout and optional Search bounds outline show its extent; this does not move or rotate the guides. Keep the search area inside the capture frame. Changing size, height, rotation or alignment requires learning the labels again; changing Search or Gap alone retains them.",
                            "While stationary and looking forward, select Test to hide the guides and previews, then return focus to Elite to learn the small labels next to your circles. Check the six slot readings before driving. A question mark means uncertain, not detected; the controls display the reason when learning or capture is blocked. An ellipsis means a change is awaiting confirmation. BAR means confirmed present; empty means confirmed absent. A bar already present establishes a baseline, so it can read BAR without reporting a new appearance. Later confirmed appearances and disappearances are reported after three agreeing readings.",
                        ...
                        [
                            ...
                            "Detection checks the shape of bright colored bars in any hue. Gray, white, black and dark pixels do not count as a deployment bar, and visible circle rims are excluded. A clear colored bar can be recognized even when its circle is difficult to read. Low brightness, changed HUD geometry, head look, or obscuring windows can prevent recognition.",
