...
public static class GuideCatalog
{
                ...
                [
                    Section(
                        ...
                        [
                            ...
                            "The translucent dotted PIP border follows the legacy state color independently from the segment fills. Border colors and opacity are configurable in Overlay appearance.",
    ...
    private static IReadOnlyList<GuideIconViewModel> CreateIconGlossary()
    {
        ...
        [
            ...
            Icon(GuideIconKind.BiologyRewardKnown, "", "Confirmed reward PIPs", "An unhatched PIP means the organism and its reward are confirmed. Each filled segment clears one reward threshold; black empty slots show thresholds the reward does not reach. Its translucent dotted border is independently themeable.", "Bio signals and biology system overlays", "bars pips confirmed solid filled empty border"),
            Icon(GuideIconKind.BiologyRewardPredicted, "", "Predicted reward PIPs", "Diagonal hatching marks a predicted organism or reward. Solid prediction segments show the dependable lower band; the darker possible segment shows how high the reward range may extend. The dotted group frame spans the body's reported biological signal count. Additional PIPs outside that frame are alternative genus candidates, not additive rewards.", "Bio signals and biology predictions", "bars pips estimate range hatched potential alternative genus overflow dotted frame"),
