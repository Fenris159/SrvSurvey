...
{
    public static readonly Uri IssuesUri = new("https://github.com/Fenris159/SrvSurvey/issues");
    public static readonly Uri DiscordUri = new("https://discord.gg/QZsMu2SkSA");
    private const string NewIssueAddress = "https://github.com/Fenris159/SrvSurvey/issues/new";
    ...
    )
    {
        ...
        RecentLogs = string.Join(Environment.NewLine, applicationLog?.Entries.TakeLast(20) ?? []);
        JournalPath =
            !string.IsNullOrWhiteSpace(journalPath) && File.Exists(journalPath) ? Path.GetFullPath(journalPath) : null;
    }
    ...
    public Uri BuildIssueUri(DateTimeOffset? timestamp = null)
    {
        ...
        {
            ["title"] = $"{exception.GetType().Name} \"{exception.Message}\" at " + (timestamp ?? DateTimeOffset.Now),
            ["what-happened"] = Steps,
            ["version"] = version,
    ...
    {
        return CopyAsync(clipboardWriter, ErrorDetails, "Error details copied to the clipboard.");
    }
    ...
    {
        return CopyAsync(clipboardWriter, JournalPath ?? string.Empty, "The journal path was copied to the clipboard.");
    }
    ...
    public async Task<bool> OpenJournalAsync(Func<FileInfo, Task<bool>> fileLauncher)
    {
        ...
        catch (Exception launchException) when (IsExpectedPlatformException(launchException))
        {
            StatusMessage = "The current journal file could not be opened: " + launchException.Message;
            return false;
        }
    }

    private async Task CopyAsync(Func<string, Task> clipboardWriter, string text, string successMessage)
    {
        ...
        catch (Exception copyException) when (IsExpectedPlatformException(copyException))
        {
            StatusMessage = "The text could not be copied: " + copyException.Message;
        }
    }
