...
public sealed class FiregroupsWorkspaceViewModel : WorkspaceObservable
{
    ...
    private FiregroupShip? liveShip,
        editorShip;
    ...
    public string ConfigurationName
    {
        ...
        set => Set(ref configurationName, value);
    }
    ...
    public string ShipSummary =>
        editorShip?.Display ?? "Waiting for a Loadout event. Board your ship to identify its equipped modules.";
    ...
    public string ModuleStatus =>
        HasEquippedModules
            ? "Equipped hardpoints, utilities, Surface Scanner and internal limpet controllers are listed with the ship's built-in scanners. Shield cell banks, shield boosters, point defence, power distributors, module reinforcement and cargo racks are excluded. Slots distinguish duplicate modules."
    ...
    public string LiveSummary =>
        ActiveProfile is { } profile
            ? $"Active: {profile.Name} · {liveShip?.Display}"
            : "No saved configuration selected for the current ship.";
    public FiregroupProfile? ActiveProfile =>
        liveShip is null
            ? null
            : document.Profiles.FirstOrDefault(p =>
                p.Ship.Key == liveShip.Key && document.ActiveProfiles.GetValueOrDefault(liveShip.Key) == p.Id
            );
    ...
    public void Apply(JournalMonitorUpdate update, JournalSessionState journal, EliteStatus? currentStatus)
    {
        ...
        {
            ChangeCommander(nextCommander);
        }

        latestStatus = currentStatus;
        boarded = OverlayVehicleCatalog.Resolve(journal, currentStatus);
        if (commander is null || !storageAvailable)
        {
            NotifyLive();
            return;
        }
        ...
        {
            liveShip = nextShip;
            if (editorShip?.Key == nextShip.Key)
            {
                editorShip = nextShip;
                RefreshOptions();
                NotifyEditor();
    ...
    private FiregroupDocument? UpdateLoadouts(List<FiregroupShip> loadouts)
    {
        ...
        return ships.SequenceEqual(document.Ships) ? null : document with { Ships = ships };
    }
    ...
    private void ChangeCommander(string? nextCommander)
    {
        ...
        if (commander is not null)
        {
            try
            {
                document = store.Load(commander);
                storageAvailable = true;
            }
            catch (Exception ex) when (IsStorageError(ex))
            {
                Status = "Firegroups could not be loaded: " + ex.Message;
            }
        }
        RefreshSaved();
        ClearEditor();
    ...
    private List<FiregroupShip> ReadLoadouts(IReadOnlyList<JournalEventEnvelope> entries, string? previousCommander)
    {
        ...
        foreach (var entry in entries)
        {
            ...
            if (
                entry.EventName == "Loadout"
                && eventCommander == commander
                && FiregroupLoadout.Parse(entry.Payload) is { } ship
            )
            {
                loadouts.Add(ship);
            }
    ...
    public bool Restore(string expectedCommander, string json)
    {
        if (commander != expectedCommander || !storageAvailable)
        {
            Status = "Commander changed; Firegroups backup was not applied.";
            return false;
        }
        try
        {
        ...
        {
            Status = "Firegroups restore failed: " + ex.Message;
            return false;
        }
    }
    ...
    {
        if (document.LegacyImported || liveShip is null || commander is null)
        {
            return;
        }
        ...
        try
        {
            ...
            if (legacy.Count > 0)
            {
                var profile = new FiregroupProfile(
                    ...
                    liveShip,
                    legacy
                        .Select(g => new FiregroupAssignment(
                            g.Group,
                            LegacyModules(g.Primary),
                            LegacyModules(g.Secondary)
                        ))
                        .ToArray()
                );
                document.Profiles.Add(profile);
                document.ActiveProfiles[liveShip.Key] = profile.Id;
            }
            document.LegacyImported = true;
            Persist(
                document,
                legacy.Count > 0
                    ? "Previous firegroups imported. Select equipped modules to update their assignments."
                    : "Ship loadout ready."
            );
            RefreshSaved();
        }
        ...
        {
            Status = "Previous firegroups could not be imported: " + ex.Message;
        }
    }

    private static FiregroupModule[] LegacyModules(string value) =>
        string.IsNullOrWhiteSpace(value) ? [] : [new("", "", value)];
    ...
    private void OpenEditor(FiregroupShip? ship, FiregroupProfile? profile)
    {
        ...
        {
            ResetRows(groups.GetValueOrDefault(groupNumber));
        }
    ...
    {
        if (!CommitCurrent(requireModules: false))
        {
            return;
        }

        groupNumber = number;
        ResetRows(groups.GetValueOrDefault(number));
        Changed(nameof(GroupLetter));
        RefreshPreview();
    }
    ...
    private void AddGroup()
    {
        if (!CanEdit)
        {
            Status = "Wait for a commander and ship Loadout before adding groups.";
            return;
        }
        if (!CommitCurrent(requireModules: true))
        {
            return;
        }
        ...
        groupNumber = next;
        ResetRows(groups.GetValueOrDefault(next));
        Changed(nameof(GroupLetter));
        RefreshPreview();
    }
    ...
    private void RemoveGroup()
    {
        ...
        ResetRows(null);
        RefreshPreview();
        Status = $"Group {GroupLetter} removed from the draft; save to keep the change.";
    }

    private FiregroupAssignment Capture() =>
        new(
            groupNumber,
            Primary.Select(r => r.SelectedModule).OfType<FiregroupModule>().ToArray(),
            Secondary.Select(r => r.SelectedModule).OfType<FiregroupModule>().ToArray()
        );
    ...
    private bool CommitCurrent(bool requireModules)
    {
        ...
        if (current.Primary.Count + current.Secondary.Count == 0)
        {
            if (requireModules)
            {
                Status = "Choose at least one primary or secondary module.";
                return false;
            }
            groups.Remove(groupNumber);
            return true;
        ...
        if (
            new[] { current.Primary, current.Secondary }.Any(side =>
                side.DistinctBy(m => (m.Slot, m.Symbol)).Count() != side.Count
            )
        )
        {
            Status =
                "Each equipped module can appear only once per trigger. Use its slot to distinguish duplicate modules.";
            return false;
        }
        groups[groupNumber] = current;
        RefreshPreview();
        return true;
    }
    ...
    {
        var row = new FiregroupSelectionRow(
            editorShip?.Modules ?? [],
            module,
            () =>
            {
        ...
        row.RemoveCommand = new WorkspaceCommand(() =>
        {
            target.Remove(row);
            RefreshPreview();
        });
    ...
    private void Save()
    {
        if (!CanEdit || commander is null || editorShip is null)
        {
            Status = "Connect a commander and read a ship Loadout before saving.";
            return;
        }
        if (string.IsNullOrWhiteSpace(ConfigurationName))
        {
            Status = "Enter a firegroup configuration name before saving.";
            return;
        }
        if (!CommitCurrent(requireModules: false))
        {
        ...
        {
            Status = "That name is already saved for this ship. Select it below to edit, or choose a different name.";
            return;
        }
        var profile = new FiregroupProfile(
        ...
        if (!Persist(candidate, $"Saved {name} for {editorShip.Display}."))
        {
            return;
        }
    ...
    private void Remove()
    {
        ...
        {
            Status = "Select a saved configuration to remove.";
            return;
        }
        if (SelectedSavedProfile is { } row)
        {
            Remove(row.Profile);
        }
    }
    ...
    private void Remove(FiregroupProfile target)
    {
        ...
        if (!document.Profiles.Any(profile => ReferenceEquals(profile, target)))
        {
            return;
        }
        ...
        if (!Persist(candidate, $"Removed {target.Name}."))
        {
            return;
        }
    ...
    private bool Persist(FiregroupDocument candidate, string message)
    {
        ...
        try
        {
            store.Save(commander, candidate);
            Status = message;
            return true;
        }
        catch (Exception ex) when (IsStorageError(ex))
        {
            Status = "Firegroups could not be saved: " + ex.Message;
            return false;
        }
    }
    ...
    {
        Changed(nameof(ShipSummary));
        Changed(nameof(CanEdit));
        Changed(nameof(HasEquippedModules));
        Changed(nameof(ModuleStatus));
        Changed(nameof(GroupLetter));
    }

    private void NotifyLive()
    {
        Changed(nameof(ActiveProfile));
        Changed(nameof(ActiveGroup));
        Changed(nameof(ActiveGroupNumber));
        Changed(nameof(ShouldShow));
        Changed(nameof(LiveSummary));
    }
...
public sealed class FiregroupSelectionRow : WorkspaceObservable
{
    ...
    public string Warning
    {
        get
        {
            ...
            if (FiregroupLoadout.IsExcluded(selected))
            {
                return $"{selected.Name} is excluded from Firegroups; choose a replacement or remove this row.";
            }
...
{
    public static FiregroupTreeNode From(FiregroupAssignment group) =>
        new(
            group.Label,
            [
