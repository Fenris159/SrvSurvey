...
public sealed class HumanSiteTemplateAuthoringViewModel : INotifyPropertyChanged
{
    ...
    private bool isBusy;
    private string statusMessage = "Approach an aligned settlement to author its template.";
    ...
    )
    {
        ...
        this.previewChanged = previewChanged ?? throw new ArgumentNullException(nameof(previewChanged));
        ...
        endPolygonCommand = new DelegateCommand(
            EndPolygon,
            () => CanCapturePoint && session?.IsCapturingPolygon == true && session.PendingPolygonPoints.Count > 0
        );
        cancelPolygonCommand = new DelegateCommand(CancelPolygon, () => session?.IsCapturingPolygon == true && !IsBusy);
        ...
        addNamedPointCommand = new DelegateCommand(
            AddNamedPoint,
            () => CanCapturePoint && !string.IsNullOrWhiteSpace(NamedPointName)
        );
        addDataTerminalCommand = new DelegateCommand(AddDataTerminal, () => CanCapturePoint);
        addSecureDoorCommand = new DelegateCommand(AddSecureDoor, () => CanCapturePoint);
        ...
        requestDiscardCommand = new DelegateCommand(RequestDiscard, () => IsAuthoring && !IsBusy);
        confirmDiscardCommand = new DelegateCommand(ConfirmDiscard, () => IsDiscardConfirmationPending && !IsBusy);
    ...
    public HumanSiteTemplate? PreviewTemplate => session?.CreatePreviewTemplate(BuildingName);

    public bool CanStart => activeTemplate is not null && !IsAuthoring && !IsBusy;

    public bool IsAuthoring => session is not null;

    public bool CanCapturePoint => session is not null && currentOffset is not null && !IsBusy;

    public bool IsCapturingPolygon => session?.IsCapturingPolygon == true;

    public bool HasPendingBuilding => session?.HasPendingBuilding == true;

    public int PendingPolygonPointCount => session?.PendingPolygonPoints.Count ?? 0;

    public int PendingBuildingPathCount => session?.PendingBuildingPaths.Count ?? 0;

    public int BuildingCount => session?.Template.Buildings.Count ?? activeTemplate?.Buildings.Count ?? 0;

    public int NamedPointCount => session?.Template.NamedPoints.Count ?? activeTemplate?.NamedPoints.Count ?? 0;

    public int DataTerminalCount => session?.Template.DataTerminals.Count ?? activeTemplate?.DataTerminals.Count ?? 0;

    public int SecureDoorCount => session?.Template.SecureDoors.Count ?? activeTemplate?.SecureDoors.Count ?? 0;

    public string TemplateTitle =>
        activeTemplate is null
            ? "No aligned settlement template"
            : $"{activeTemplate.Economy} #{activeTemplate.SubType} · " + activeTemplate.Name;

    public string CurrentOffsetText =>
        currentOffset is { } offset
    ...
    public async Task ExportAsync(string path, CancellationToken cancellationToken = default)
    {
        ...
        {
            StatusMessage = "The settlement template catalog was not exported: " + exception.Message;
        }
        finally
        {
            IsBusy = false;
    ...
    private void AddNamedPoint()
    {
        ...
        session.AddNamedPoint(NamedPointName, point, SecurityLevel, Floor);
    ...
    private void AddSecureDoor()
    {
        ...
        session.AddSecureDoor(point, relativeHeading, SecurityLevel, Floor);
