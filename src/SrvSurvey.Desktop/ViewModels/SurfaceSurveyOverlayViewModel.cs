...
public sealed class SurfaceSurveyOverlayViewModel : INotifyPropertyChanged, IDisposable
{
    private string platformStatus;
    private string inputMode;
    private bool disposed;

    public SurfaceSurveyOverlayViewModel(SurfaceSurveyViewModel surfaceSurvey, OverlayPlatformCapabilities capabilities)
    {
        SurfaceSurvey = surfaceSurvey ?? throw new ArgumentNullException(nameof(surfaceSurvey));
    ...
    public double WindowWidth => GetWindowSize(SurfaceSurvey.RadarSize, SurfaceSurvey.IsTrackerOnly).Width;

    public double WindowHeight => GetWindowSize(SurfaceSurvey.RadarSize, SurfaceSurvey.IsTrackerOnly).Height;
    ...
    private static (double Width, double Height) GetWindowSize(int radarSize, bool trackerOnly)
    {
        if (trackerOnly)
        {
            return (300, 260);
    ...
    private void OnSurfaceSurveyPropertyChanged(object? sender, PropertyChangedEventArgs eventArgs)
    {
        if (
            ...
            is nameof(SurfaceSurveyViewModel.RadarSize)
