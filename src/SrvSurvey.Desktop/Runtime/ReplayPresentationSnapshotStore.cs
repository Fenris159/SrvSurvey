...
internal static class ReplayPresentationSnapshotStore
{
    ...
    public static void Apply(DiagnosticReplaySession session)
    {
        ...
        {
            throw new InvalidDataException("The replay overlay presentation uses an unsupported scale.");
        }

        var supportedNames = OverlayLayoutCatalog
            .Supported.Select(definition => definition.Name)
            .ToHashSet(StringComparer.Ordinal);
        if (
            snapshot.OverlayEnablement.Keys.Any(name => !supportedNames.Contains(name))
            || snapshot.OverlayPlacements.Keys.Any(name => !supportedNames.Contains(name))
        )
        {
            throw new InvalidDataException("The replay overlay presentation contains an unknown overlay.");
        }

        if (
            snapshot.OverlayPlacements.Values.Any(placement =>
                placement.ScaleIndex is { } scaleIndex && !OverlayScaleCatalog.IsSupported(scaleIndex)
            )
        )
        {
            throw new InvalidDataException("The replay overlay presentation uses an unsupported placement scale.");
        }
    ...
    private static ReplayHorizontalAnchor Convert(LegacyHorizontalAnchor anchor) =>
        anchor switch
        {
    ...
    private static ReplayVerticalAnchor Convert(LegacyVerticalAnchor anchor) =>
        anchor switch
        {
    ...
    private static LegacyHorizontalAnchor Convert(ReplayHorizontalAnchor anchor) =>
        anchor switch
        {
    ...
    private static LegacyVerticalAnchor Convert(ReplayVerticalAnchor anchor) =>
        anchor switch
        {
