...
#pragma warning disable CA2213
    ...
    private ColonizationCommodityOverlayCoordinator? colonizationCommodityOverlayCoordinator;
    ...
    private FleetCarrierRouteOverlayCoordinator? fleetCarrierRouteOverlayCoordinator;
    private FleetCarrierJumpCountdownCoordinator? fleetCarrierJumpCountdownCoordinator;
    ...
    private MineMapOverlayCoordinator? mineMapOverlayCoordinator;
    ...
    private BiologyPredictionsWindowCoordinator? biologyPredictionsWindowCoordinator;
    private BiologyCodexWindowCoordinator? biologyCodexWindowCoordinator;
    private BiologyCodexBingoWindowCoordinator? biologyCodexBingoWindowCoordinator;
...
#pragma warning restore CA2213
    ...
    )
    {
        ...
        applicationLog.Append(
            $"Overlay presentation: {overlayPresentation.Decision.Mode}. " + overlayPresentation.Decision.Reason
        );
        ...
        using var overlayInteractionOwnership = new MainWindowViewModelStartupResource<OverlayInteractionViewModel>(
            overlayInteraction,
            exception => applicationLog.Append("Main window startup cleanup failed: " + exception.Message)
        );
        ...
        using var mainViewModelStartup = new MainWindowViewModelStartup(
            ...
            new MainWindowFoundationInputs
            {
                ...
                CommanderPreferenceCommandLineOverride = commanderPreferenceResolution.IsCommandLineOverride,
                CommanderPreferenceInitialStatus = commanderPreferenceResolution.StatusMessage,
                FrontierProfile = diagnosticReplay is null
                    ? null
                    : new CommanderProfileViewModel(new DiagnosticReplayFrontierAccountService()),
            ...
            new MainWindowExplorationInputs
            {
                ...
                SystemBodyDataClient = new SystemBodyDataClient(externalNetworkClient),
            },
            new MainWindowTravelInputs
            {
                GameWindowSwitcher = diagnosticReplay is null ? null : new DiagnosticReplayGameWindowSwitcher(),
            },
        ...
        if (diagnosticReplay is null)
        {
            ...
            viewModel.Localization.SetRestartHandler(() => RestartApplicationAsync("Language preference changed"));
        ...
        fleetCarrierJumpCountdownCoordinator = new FleetCarrierJumpCountdownCoordinator(viewModel.FleetCarrierRoute);
        biologyPredictionsWindowCoordinator = new BiologyPredictionsWindowCoordinator(
        ...
        biologyCodexBingoWindowCoordinator = new BiologyCodexBingoWindowCoordinator(
        ...
        miningActivityOverlayCoordinator = new MiningActivityOverlayCoordinator(
            viewModel.MiningWorkspace,
        ...
        startup.Checkpoint?.Invoke(DesktopStartupCheckpoint.OverlayDependentsReady);
        if (diagnosticReplay is null)
        {
            StartGlobalInputServices(inputSettings, capabilities, viewModel, desktop);
        }
    ...
    private void HandleMainWindowOpened(object? sender, EventArgs eventArgs)
    {
        ...
        _ = journalMonitorSession.Start(
            ...
            exception => applicationLogService?.Append("Journal monitor stopped unexpectedly: " + exception)
        );
    }
    ...
    )
    {
        ...
        var installationWorkflow = new ReleaseInstallationWorkflow(
            new ReleaseInstallationWorkflowAdapters(
                ...
                new ReleaseInstallationPreparer(historyCleanup: releaseHistoryCleanup),
                new ApplicationUpdateHandoffService(),
                applicationInstanceManager,
                (scan, _, cancellationToken) =>
                    ConfirmMultipleApplicationInstancesAsync(desktop, scan, cancellationToken)
            ),
        ...
        viewModel.ReleaseUpdates.ConfigureInstallationWorkflow(installationWorkflow);
    }
    ...
    private PosixSignalRegistration? RegisterLinuxTermination()
    {
        ...
        return PosixSignalRegistration.Create(
            ...
            {
                context.Cancel = true;
                PostShutdownOnUiThread(DesktopShutdownReason.LinuxTermination);
            }
        );
    }
    ...
    private static void MigrateLegacyUiSettings(AppDataPaths appDataPaths, ApplicationLogService applicationLog)
    {
                ...
                $"Migrated {settingsMigration.MappedPreferenceCount:N0} legacy UI preferences."
                    + (
                        settingsMigration.PreviousSettingsBackupPath is null
                            ? string.Empty
                            : " Previous settings backup: " + settingsMigration.PreviousSettingsBackupPath
                    )
            );
            return;
        }
        ...
        {
            applicationLog.Append("Legacy UI settings migration was skipped: " + settingsMigration.Error);
        }
    }
    ...
    private static void MigrateLegacyOverlayLayout(AppDataPaths appDataPaths, ApplicationLogService applicationLog)
    {
        ...
        if (migration.Migrated)
        {
            applicationLog.Append(
                $"Converted {migration.NormalizedPlacementCount:N0} imported "
                    ...
                    + (migration.BackupPath is null ? string.Empty : " Previous layout backup: " + migration.BackupPath)
            );
            return;
        }
        ...
        {
            applicationLog.Append("Imported overlay placement conversion was skipped: " + migration.Error);
        }
    }

    private static void MigrateLegacyOrganicProfiles(AppDataPaths appDataPaths, ApplicationLogService applicationLog)
    {
        try
        {
        ...
        {
            applicationLog.Append(
                "Legacy organic migration was skipped without modifying its " + "source data: " + exception.Message
            );
        }
    }
    ...
    {
        Func<bool> areShortcutsActive = () => AreGlobalInputShortcutsActive(viewModel, desktop);
        ...
        globalKeyboardHookService.StatusChanged += (_, _) => PostKeyboardRuntimeStatus(inputSettings);
        ...
        inputSettings.UpdateRuntimeStatus(globalKeyboardHookService.Status);
        inputSettings.UpdateControllerRuntimeStatus(globalControllerInputService.Status);
    ...
    )
    {
        return mainWindow!.InputContext.AreShortcutsActive
            || (
                (viewModel.OverlayInteraction.IsEditing || viewModel.OverlayInteraction.IsLiveInteractionEnabled)
                && !mainWindow.IsActive
                && desktop.Windows.Any(window => window.IsActive)
            );
    }
    ...
    private async Task StopDesktopProducersAsync()
    {
        ...
        {
            await TryCleanupAsync(() => keyboardInput.DisposeAsync().AsTask());
        }
    }
    ...
    private Task DisposeDesktopDependentsAsync()
    {
        ...
        DisposeResource(ref mineMapOverlayCoordinator);
    ...
    )
    {
        if (diagnosticReplay is not null)
        {
            return new CommanderPreferenceResolution(
                ...
                StatusMessage: "Diagnostic replay is waiting for commander identity from the imported journal."
            );
        }
    ...
    private void TryAppendShutdownFailure(Exception exception)
    {
        try
        {
            applicationLogService?.Append("Desktop runtime shutdown failed: " + exception);
        }
        catch
        {
    ...
    private void TryAppendStartupFailure(Exception exception)
    {
        try
        {
            applicationLogService?.Append("Fatal desktop runtime startup error: " + exception);
        }
        catch
        {
    ...
    )
    {
        ...
        {
            applicationLog.Append("Update replacement health confirmation failed: " + exception.Message);
        }
    }
    ...
    )
    {
        try
        {
            ...
            if (!File.Exists(Path.Combine(AppContext.BaseDirectory, "release-package.json")))
            {
                return;
            }
        ...
        catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested)
        {
            ...
            when (exception is IOException or UnauthorizedAccessException or InvalidDataException)
        {
            applicationLog.Append("Update history cleanup could not run: " + exception.Message);
        }
    }
    ...
    )
    {
        ...
        applicationLog.Append(
            $"Update {updateOutcome.Version} outcome: "
                ...
                + (string.IsNullOrWhiteSpace(updateOutcome.Error) ? string.Empty : " - " + updateOutcome.Error)
        );
    }
    ...
    {
        return diagnosticReplayContext?.CreateGameWindowTracker() ?? GameWindowTracker.CreateCurrent();
    }
    ...
    private void HandleFrontierAuthorizationCallback(object? sender, EventArgs eventArgs)
    {
    ...
    private void HandleUiException(object? sender, DispatcherUnhandledExceptionEventArgs eventArgs)
    {
    ...
    )
    {
        if (
            ...
            is nameof(SystemSurveyViewModel.IsFssInfoForced)
    ...
    private void ApplyOverlaySuppression()
    {
        ...
        mineMapOverlayCoordinator?.SetSuppressed(suppress);
    ...
    private void HandleOverlayBehaviorChanged(object? sender, System.ComponentModel.PropertyChangedEventArgs eventArgs)
    {
        if (
            eventArgs.PropertyName
            is nameof(OverlayBehaviorViewModel.ShouldSuppressForSuit)
    ...
    private async Task<bool> ExecuteGlobalInputActionAsync(GlobalInputAction action)
    {
        ...
        {
            GlobalInputAction.MapZoomIn => guardianOverlayCoordinator?.AdjustZoom(zoomIn: true) == true
                || humanSiteOverlayCoordinator?.AdjustZoom(zoomIn: true) == true
                || systemSurveyOverlayCoordinator?.AdjustSurfaceZoom(zoomIn: true) == true,
            GlobalInputAction.MapZoomOut => guardianOverlayCoordinator?.AdjustZoom(zoomIn: false) == true
                || humanSiteOverlayCoordinator?.AdjustZoom(zoomIn: false) == true
                || systemSurveyOverlayCoordinator?.AdjustSurfaceZoom(zoomIn: false) == true,
            GlobalInputAction.MapZoomAuto => guardianOverlayCoordinator?.ResetZoom() == true
            ...
            GlobalInputAction.ShowStationInfo => viewModel.StationInfo.ToggleForcedVisibility(),
            GlobalInputAction.NextWindow => viewModel.CommanderInstances.SwitchToNextGameWindow(),
            GlobalInputAction.QuestShow => ShowQuestsWindow(),
            GlobalInputAction.ShowColonyShopping => ToggleColonyShopping(),
            GlobalInputAction.ShowSystemNotes => systemNotesWindowCoordinator is not null
            ...
            GlobalInputAction.ToggleFirstFootfall => await viewModel.ToggleCurrentBodyFirstFootfallAsync(),
            GlobalInputAction.StreamOne => streamOverlayCoordinator?.Toggle() == true,
            GlobalInputAction.AdjustVr => BeginVrAdjustment(),
            GlobalInputAction.ResetVr => vrOverlayCoordinator?.ResetOrientation() == true,
    ...
    private async Task<bool> PasteGalaxyMapAsync()
    {
        ...
        return resolvedText is not null && gameTextInputService.EnterText(resolvedText).Succeeded;
    }
    ...
    private Task<bool> ToggleQuickTrackerAsync(int trackerNumber)
    {
        ...
        return viewModel.ToggleTrackerOrMiningRigAsync(trackerNumber, CancellationToken.None);
    }
