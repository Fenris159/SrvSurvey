...
public sealed class BundledAssetImageConverter : IValueConverter
{
    ...
    private readonly ConcurrentDictionary<string, object> images = new(StringComparer.Ordinal);

    public BundledAssetImageConverter()
        : this(uri => AssetLoader.Open(uri), stream => new Bitmap(stream)) { }

    internal BundledAssetImageConverter(Func<Uri, Stream> openAsset, Func<Stream, object> decodeAsset)
    {
        this.openAsset = openAsset ?? throw new ArgumentNullException(nameof(openAsset));
        this.decodeAsset = decodeAsset ?? throw new ArgumentNullException(nameof(decodeAsset));
    }
