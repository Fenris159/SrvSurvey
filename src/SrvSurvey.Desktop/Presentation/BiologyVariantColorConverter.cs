...
{
    private static readonly Dictionary<string, Color> VariantColors = new Dictionary<string, Color>(
    ...
    public static bool Supports(string? variant) =>
        !string.IsNullOrWhiteSpace(variant) && VariantColors.ContainsKey(variant.Trim());
