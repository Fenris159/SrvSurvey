...
public sealed record RouteBodyVisual(RouteBodyVisualKind Kind, string AssetPath, string AccessibleName);
...
{
    private const string AssetRoot = "avares://SrvSurvey.Desktop/Assets/Bodies/";

    public static IReadOnlyList<RouteBodyVisual> SupportedVisuals { get; } =
        Enum.GetValues<RouteBodyVisualKind>().Select(CreateVisual).ToArray();
    ...
    {
        return new RouteBodyVisual(kind, AssetRoot + GetFileName(kind), GetAccessibleName(kind));
    }

    private static readonly (string Token, RouteBodyVisualKind Kind)[] ExactTokenKinds =
    [
    ...
    private static RouteBodyVisualKind ResolveKind(string normalized)
    {
        ...
        if (
            normalized.EndsWith("star", StringComparison.Ordinal)
    ...
    private static string GetFileName(RouteBodyVisualKind kind) =>
        kind switch
        {
    ...
    private static string GetAccessibleName(RouteBodyVisualKind kind) =>
        kind switch
        {
