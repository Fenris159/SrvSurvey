...
public sealed class MiningCalibrationWindow : Window
{
    ...
    private readonly TextBlock detectionStatus = new()
    {
        FontSize = 11,
    ...
    private readonly TextBlock values = new()
    {
        FontSize = 10,
        TextWrapping = TextWrapping.Wrap,
        Margin = new Thickness(3),
    ...
    public MiningCalibrationWindow(MiningDetectionViewModel model, PixelRect viewport, double scaling)
    {
        ...
        color.Bind(
            ...
            new Binding(nameof(MiningDetectionViewModel.BarColor)) { Source = model, Mode = BindingMode.TwoWay }
        );
    ...
    private void ResizeCircles(double delta) =>
    ...
    private void Rotate(double delta) =>
        model.UpdateCalibration(model.Settings with { RotationDegrees = model.Settings.RotationDegrees + delta });

    private void ResizeHeight(double delta) =>
        model.UpdateCalibration(model.Settings with { CircleAspectRatio = model.Settings.CircleAspectRatio + delta });

    private void ResizeBarGap(double delta) =>
        model.UpdateCalibration(model.Settings with { BarGap = model.Settings.BarGap + delta });
    ...
    {
        if (positioning || !IsVisible || Bounds.Width <= 0 || Bounds.Height <= 0)
        {
            return;
        }

        model.UpdateCalibration(
            model.Settings.WithBounds(
                new PixelRect(
                    Position,
                    new PixelSize(
                        (int)Math.Round(Bounds.Width * RenderScaling),
                        (int)Math.Round(Bounds.Height * RenderScaling)
                    )
                ),
                viewport
            )
    ...
    private void OnPressed(object? sender, PointerPressedEventArgs e)
    {
        ...
        {
            ManagedOverlayWindowDragSession.Begin(this, e);
        }
    ...
    private void OnDetectionChanged(object? sender, PropertyChangedEventArgs e)
    {
        ...
        moreMovement.IsEnabled =
            settings.MotionMargin < 120d / MiningDetectionSettings.GetWorkingWidth(settings.CircleWidth) - .000001;
        ToolTip.SetTip(
            canvas,
            $"Drag dots onto circle centres. Drag empty space to move; lower-right corner to resize.\n"
    ...
    private sealed class CalibrationCanvas(MiningDetectionViewModel model) : Control
    {
        ...
        public override void Render(DrawingContext context)
        {
            ...
            context.DrawLine(
                pen,
                new Point(Bounds.Width - 15, Bounds.Height - 2),
                new Point(Bounds.Width - 2, Bounds.Height - 15)
            );
            if (!ShowGuides)
            {
                return;
            ...
            if (ShowSearchArea)
            {
                ...
                var search = new Rect(
                    ...
                    new Point(
                        markers.Max(p => p.X) * Bounds.Width + margin,
            ...
            for (var i = 0; i < 6; i++)
            {
                ...
                DrawPolyline(
                    context,
                    new Pen(Brushes.Cyan, 1.5),
                    MiningBarShape
                        .GuidePoints.Select(p =>
                            center
                            + geometry.Transform(p.X, p.Y, radius)
                            + new Vector(0, radius * model.Settings.BarGap)
                        )
                        .ToArray()
                );
            }
        ...
        protected override void OnPointerPressed(PointerPressedEventArgs e)
        {
            ...
            if (!ShowGuides || !e.GetCurrentPoint(this).Properties.IsLeftButtonPressed)
            {
                return;
            }
