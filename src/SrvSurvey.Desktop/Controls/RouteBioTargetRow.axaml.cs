...
public sealed partial class RouteBioTargetRow : UserControl
{
    public static readonly StyledProperty<bool> IsInteractiveProperty = AvaloniaProperty.Register<
        RouteBioTargetRow,
        bool
    >(nameof(IsInteractive), defaultValue: false);
    ...
    static RouteBioTargetRow()
    {
        IsInteractiveProperty.Changed.AddClassHandler<RouteBioTargetRow>(
            ...
            {
                control.CompletionCheckBox.IsHitTestVisible = eventArgs.NewValue is true;
            }
        );
    }
    ...
    private void CompletionCheckBox_Click(object? sender, RoutedEventArgs eventArgs)
    {
        if (!IsInteractive || DataContext is not RouteBioTargetItemViewModel target || sender is not CheckBox checkBox)
        {
            return;
        }

        CompletionRequested?.Invoke(this, new RouteBioCompletionRequestedEventArgs(target, checkBox.IsChecked == true));
    }
}
