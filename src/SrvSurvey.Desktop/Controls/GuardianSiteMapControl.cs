...
{
    public static readonly StyledProperty<GuardianSiteMapProjection?> ProjectionProperty = AvaloniaProperty.Register<
        GuardianSiteMapControl,
        GuardianSiteMapProjection?
    >(nameof(Projection));
    public static readonly StyledProperty<GuardianSiteProximitySnapshot?> ProximityProperty = AvaloniaProperty.Register<
        GuardianSiteMapControl,
        GuardianSiteProximitySnapshot?
    >(nameof(Proximity));
    public static readonly StyledProperty<GuardianSiteProximitySnapshot?> CommanderMapPositionProperty =
        AvaloniaProperty.Register<GuardianSiteMapControl, GuardianSiteProximitySnapshot?>(nameof(CommanderMapPosition));
    public static readonly StyledProperty<double> MapScaleProperty = AvaloniaProperty.Register<
        GuardianSiteMapControl,
        double
    >(nameof(MapScale), double.NaN);
    public static readonly StyledProperty<double> CommanderHeadingProperty = AvaloniaProperty.Register<
        GuardianSiteMapControl,
        double
    >(nameof(CommanderHeading));
    public static readonly StyledProperty<bool> RotateMapWithCommanderProperty = AvaloniaProperty.Register<
        GuardianSiteMapControl,
        bool
    >(nameof(RotateMapWithCommander), true);
    public static readonly StyledProperty<string?> TargetPointNameProperty = AvaloniaProperty.Register<
        GuardianSiteMapControl,
        string?
    >(nameof(TargetPointName));
    public static readonly StyledProperty<string?> HighlightedPointNameProperty = AvaloniaProperty.Register<
        GuardianSiteMapControl,
        string?
    >(nameof(HighlightedPointName));
    public static readonly StyledProperty<string?> SelectedPointNameProperty = AvaloniaProperty.Register<
        GuardianSiteMapControl,
        string?
    >(nameof(SelectedPointName));
    public static readonly DirectProperty<GuardianSiteMapControl, string?> HoveredPointNameProperty =
        AvaloniaProperty.RegisterDirect<GuardianSiteMapControl, string?>(
    ...
    public static readonly StyledProperty<IBrush?> MutedBrushProperty = AvaloniaProperty.Register<
        ...
        IBrush?
    >(nameof(MutedBrush));
    public static readonly StyledProperty<IBrush?> PresentBrushProperty = AvaloniaProperty.Register<
        GuardianSiteMapControl,
        IBrush?
    >(nameof(PresentBrush));
    public static readonly StyledProperty<IBrush?> AbsentBrushProperty = AvaloniaProperty.Register<
        GuardianSiteMapControl,
        IBrush?
    >(nameof(AbsentBrush));
    public static readonly StyledProperty<IBrush?> EmptyBrushProperty = AvaloniaProperty.Register<
        ...
        IBrush?
    >(nameof(EmptyBrush));
    public static readonly StyledProperty<bool> ShowLegendProperty = AvaloniaProperty.Register<
        GuardianSiteMapControl,
        bool
    >(nameof(ShowLegend), true);
    public static readonly StyledProperty<bool> IsLegendOnlyProperty = AvaloniaProperty.Register<
        GuardianSiteMapControl,
        bool
    >(nameof(IsLegendOnly));
    ...
    private double MapRotationHeading => ResolveMapRotationHeading(CommanderHeading, RotateMapWithCommander);

    private double CommanderGlyphHeading => ResolveCommanderGlyphHeading(CommanderHeading, RotateMapWithCommander);
    ...
    public string? HoveredPointName
    {
        ...
        private set => SetAndRaise(HoveredPointNameProperty, ref hoveredPointName, value);
    }
    ...
    public override void Render(DrawingContext context)
    {
        ...
        if (!IsLegendOnly)
        {
            context.DrawRectangle(MapBackground ?? Brushes.Transparent, null, bounds);
        }

        if (Projection is not { } projection || bounds.Width <= 0 || bounds.Height <= 0)
        {
            return;
        }
        ...
        foreach (var point in projection.Points)
        {
            DrawPoint(
                context,
                point,
                TransformMapPoint(point.X, point.Y, Proximity, MapRotationHeading, viewportCenter, scale),
                projection,
        ...
        foreach (var group in projection.Groups)
        {
            DrawGroup(
                context,
                group,
                TransformMapPoint(group.X, group.Y, Proximity, MapRotationHeading, viewportCenter, scale),
                scale
            );
        }
        ...
        {
            DrawCommander(context, viewportCenter, projection.IsRuins, scale, CommanderGlyphHeading);
        }
    ...
    )
    {
        ...
        context.DrawLine(
            gridPen,
            TransformMapPoint(0, -gridExtent, Proximity, MapRotationHeading, viewportCenter, scale),
            TransformMapPoint(0, gridExtent, Proximity, MapRotationHeading, viewportCenter, scale)
        );
        context.DrawLine(
            gridPen,
    ...
    public static double CalculateFittedScale(Rect bounds, GuardianSiteMapProjection projection, IImage? mapImage)
    {
        ...
        return Math.Min(horizontalRoom / Math.Max(1, maximumX), verticalRoom / Math.Max(1, maximumY));
    }
    ...
    public static IReadOnlyList<string> CreateLegendLabels(GuardianSiteMapProjection projection)
    {
        ArgumentNullException.ThrowIfNull(projection);
        return CreateLegendEntries(projection).Select(entry => entry.Label).ToArray();
    }
    ...
    {
        return double.IsFinite(zoom) ? Math.Clamp(zoom, MinimumViewportZoom, MaximumViewportZoom) : MinimumViewportZoom;
    }
    ...
    protected override void OnPointerPressed(PointerPressedEventArgs e)
    {
        ...
        if (!CanInteractWithViewport || !e.GetCurrentPoint(this).Properties.IsLeftButtonPressed)
        {
            return;
        }
    ...
    private bool CanInteractWithViewport => AllowViewportInteraction && !IsLegendOnly && Projection is not null;
    ...
    private GuardianProjectedPoint? HitTestPoint(Point pointerPosition)
    {
        ...
        return projection
            ...
            {
                Point = point,
                Screen = TransformMapPoint(point.X, point.Y, Proximity, MapRotationHeading, viewportCenter, scale),
            })
            ...
            .Where(candidate => candidate.Distance <= GetHitRadius(candidate.Point, projection, scale))
    ...
    internal static double ResolveMapRotationHeading(double commanderHeading, bool rotateMapWithCommander)
    {
        return rotateMapWithCommander && double.IsFinite(commanderHeading) ? commanderHeading : 0;
    }

    internal static double ResolveCommanderGlyphHeading(double commanderHeading, bool rotateMapWithCommander)
    {
        return !rotateMapWithCommander && double.IsFinite(commanderHeading) ? commanderHeading : 0;
    }
    ...
    private void DrawLegend(DrawingContext context, GuardianSiteMapProjection projection)
    {
        ...
        DrawLegendRows(context, projection, 16, 38, rowHeight, width - 8, fontSize: 10);
    }
    ...
    private static bool IsFullWidthLegendEntry(GuardianMapLegendEntry entry)
    {
        return entry.IsActiveObelisk
            || entry.Type is GuardianPoiType.Pylon or GuardianPoiType.Component
    ...
    {
        if (!double.IsFinite(projection.ImageScaleFactor) || projection.ImageScaleFactor <= 0)
        {
            return;
        }
        ...
        using (context.PushTransform(CreateMapTransform(Proximity, MapRotationHeading, viewportCenter, scale)))
        {
            context.DrawImage(mapImage, new Rect(mapImage.Size), destination);
        }
    }
    ...
    private void DrawMissingMapNotice(DrawingContext context, Rect bounds, string siteType)
    {
        ...
        context.DrawRectangle(MapBackground ?? Brushes.Black, new Pen(AbsentBrush ?? Brushes.Red, 1), surface, 4, 4);
        context.DrawText(text, new Point(surface.X + padding, surface.Y + padding));
    }
    ...
    )
    {
        ...
        if (entry.Kind == GuardianMapLegendKind.SiteHeading)
        {
            context.DrawLine(
                ...
                new Point(center.X - 6 * symbolScale, center.Y + 5 * symbolScale),
                new Point(center.X + 5 * symbolScale, center.Y - 6 * symbolScale)
            );
            return;
        }
        ...
        {
            context.DrawLine(
                new Pen(EmptyBrush ?? Brushes.Goldenrod, 2 * symbolScale),
                new Point(center.X - 6 * symbolScale, center.Y + 5 * symbolScale),
                new Point(center.X + 5 * symbolScale, center.Y - 6 * symbolScale)
            );
            return;
        ...
        DrawPoint(
            ...
            center,
            new GuardianSiteMapProjection("Alpha", [], [], 1, IsRuins: true, SiteHeading: 0, RelicTowerHeading: 45),
            headingLength: 0,
            markerScale: (entry.IsActiveObelisk ? 0.55 : 1) * symbolScale
        );
    }

    private FormattedText CreateLegendText(string text, FontWeight weight, double fontSize = 10)
    {
        return new FormattedText(
            LocalizationCatalog.Translate(text),
            CultureInfo.InvariantCulture,
            FlowDirection.LeftToRight,
            new Typeface("Century Gothic, Segoe UI, sans-serif", FontStyle.Normal, weight),
            fontSize,
            MutedBrush ?? Brushes.Wheat
        );
    }

    private static List<GuardianMapLegendEntry> CreateLegendEntries(GuardianSiteMapProjection projection)
    {
        var entries = new List<GuardianMapLegendEntry>
        {
            ...
            new("Active obelisk · unscanned", GuardianPoiType.Obelisk, IsActiveObelisk: true),
            new("Active obelisk · scanned", GuardianPoiType.Obelisk, IsActiveObelisk: true, IsScannedObelisk: true),
            new(
        ...
        {
            entries.Add(new GuardianMapLegendEntry("Energy pylon", GuardianPoiType.Pylon));
            entries.Add(new GuardianMapLegendEntry("Component tower", GuardianPoiType.Component));
        }

        entries.Add(
            new GuardianMapLegendEntry("Site heading", GuardianPoiType.Unknown, Kind: GuardianMapLegendKind.SiteHeading)
        );
        entries.Add(
            new GuardianMapLegendEntry(
                "Tower heading",
    ...
    )
    {
        ...
        {
            DrawActiveObeliskEffect(context, point, location, rotation, markerScale);
        }
    ...
    {
        if (point.Type != GuardianPoiType.Relic || !point.HasIndividualRelicHeading || headingLength <= 0)
        {
            return;
        }
    ...
    private static void DrawPointGlyph(PointGlyphDraw draw)
    {
        switch (draw.Point.Type)
        {
            ...
            case GuardianPoiType.BrokenObelisk:
                DrawObeliskGlyph(draw.Context, draw.Point, draw.Location, draw.Pen, draw.Rotation, draw.MarkerScale);
                break;

            case GuardianPoiType.Pylon:
                DrawPylonGlyph(draw.Context, draw.Point, draw.Location, draw.Pen, draw.Rotation, draw.MarkerScale);
                break;
            ...
            case GuardianPoiType.Component:
                ...
                DrawComponentMaterials(draw.Context, draw.Location, draw.Point.ComponentMaterials, draw.MarkerScale);
                break;

            case GuardianPoiType.DestructiblePanel:
                DrawDestructiblePanel(draw.Context, draw.Point, draw.Location, draw.Pen, draw.MarkerScale);
                break;
    ...
    )
    {
        DrawPolyline(
            context,
            GuardianLegacyMapDrawing.CreateGlyphPoints(point.Type, location, rotation, markerScale),
            pen
        );
        ...
        context.DrawLine(
            pen,
            location + GuardianLegacyMapDrawing.RotateClockwise(new Point(0.2 * markerScale, 0), rotation),
            location
                + GuardianLegacyMapDrawing.RotateClockwise(new Point(-0.5 * markerScale, -1.2 * markerScale), rotation)
        );
        context.DrawLine(
            pen,
            location + GuardianLegacyMapDrawing.RotateClockwise(new Point(0.2 * markerScale, 0), rotation),
            location
                + GuardianLegacyMapDrawing.RotateClockwise(new Point(1.5 * markerScale, -0.8 * markerScale), rotation)
        );
    }
    ...
    )
    {
        ...
        context.DrawLine(
            pen,
            location,
            location + GuardianLegacyMapDrawing.RotateClockwise(new Point(0, 3 * markerScale), rotation)
        );
    }
    ...
    )
    {
        ...
        context.DrawRectangle(
            materialColor is { } known ? new SolidColorBrush(known) : null,
            materialColor is not null ? new Pen(Brushes.Black, markerScale) : pen,
            new Rect(location.X - 2 * markerScale, location.Y - 2 * markerScale, 4 * markerScale, 4 * markerScale)
        );
    }
    ...
    )
    {
        ...
        for (var step = 0; step < 6; step++)
        {
            ...
            context.DrawGeometry(
                ...
                null,
                CreatePolygon(GuardianLegacyMapDrawing.CreateWedge(location, radius, rotation))
            );
        }
    }
    ...
    private static Pen CreatePen(GuardianLegacyPointStyle style, double markerScale)
    {
        ...
        return new Pen(new SolidColorBrush(style.Stroke), style.StrokeWidth * markerScale, dashStyle: dash);
    }
    ...
    private void DrawGroup(DrawingContext context, GuardianProjectedGroup group, Point location, double markerScale)
    {
        ...
        context.DrawText(
            text,
            new Point(location.X - text.Width / 2 + 2 * markerScale, location.Y - text.Height / 2 + 2 * markerScale)
        );
    }
