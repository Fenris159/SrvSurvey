...
public sealed class CodexImageCache : IDisposable
{
    ...
    private static readonly string[] KnownExtensions = [".jpg", ".jpeg", ".png", ".webp", ".bmp", ".gif"];
    ...
    public CodexImageCache(string cacheDirectory, HttpClient? httpClient = null, TimeSpan? downloadTimeout = null)
        : this(() => new CodexImageLocations(cacheDirectory, null), httpClient, downloadTimeout) { }
    ...
    )
    {
        ...
        try
        {
            ...
            {
                return CodexImageCacheResult.Failed(path, "No reference image is available from the source.");
            }
            ...
            {
                await CopyWithLimitAsync(source, target, operationToken).ConfigureAwait(false);
        ...
        {
            return CodexImageCacheResult.Failed(path, "The reference image download timed out.");
        }
    ...
    )
    {
        ...
        for (var index = 0; index < materialized.Length; index++)
        {
            ...
            progress?.Report(
                new CodexImagePreDownloadProgress(index + 1, materialized.Length, downloaded, cached, local, failed)
            );
        }

        return new CodexImagePreDownloadResult(materialized.Length, downloaded, cached, local, failed);
    }
    ...
    private static async Task CopyWithLimitAsync(Stream source, Stream target, CancellationToken cancellationToken)
    {
        ...
        while (true)
        {
            ...
            {
                throw new InvalidDataException("The reference image exceeds the 30 MB safety limit.");
            }
        ...
        return new CodexImageLocations(
            Path.GetFullPath(
                string.IsNullOrWhiteSpace(locations.CacheDirectory)
                    ? throw new InvalidOperationException("A Codex image cache directory is required.")
    ...
    private static string? ResolveLocalImage(string? directory, string? imageName)
    {
        if (
            directory is null
    ...
    private static string? ResolveCachedImage(string directory, long entryId, string preferredPath)
    {
...
public sealed record CodexImageLocations(string CacheDirectory, string? LocalFloraDirectory);

public sealed record CodexImageRequest(long EntryId, string ImageUrl, string? LocalImageName = null);
...
public sealed record CodexImagePreDownloadResult(int Total, int Downloaded, int Cached, int Local, int Failed);
