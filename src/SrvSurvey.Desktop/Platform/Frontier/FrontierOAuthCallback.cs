...
public sealed record FrontierOAuthCallback(string Code, string State, string Error, string ErrorDescription)
{
    ...
    public static FrontierOAuthCallback? Find(IEnumerable<string> arguments)
    {
        ...
        return arguments.Select(Parse).FirstOrDefault(callback => callback is not null);
    }
