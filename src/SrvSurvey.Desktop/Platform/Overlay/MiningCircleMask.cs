...
internal static class MiningCircleMask
{
    ...
    internal static (double X, double Y)? LocateGrid(
        IFssPixelSource image,
    ...
    private static (int Top, int Bottom, int Columns, double Total) ScoreGrid(
        IFssPixelSource image,
        (double X, double Y)[] centers,
        double radius,
        Vector[] directions,
    ...
    )
    {
        ...
        for (var i = 0; i < centers.Length; i++)
        {
            ...
            for (var scale = 0; scale <= 4; scale++)
            {
                confidence = Math.Max(
                    confidence,
                    Score(image, centers[i].X + x, centers[i].Y + y, radius * (.9 + scale * .05), directions)
                );
            }
    ...
    private static Vector[] Directions(MiningHudGeometry geometry) =>
        Enumerable.Range(0, 19).Select(i => geometry.RingPoint(-Math.PI + i * Math.PI / 18, 1)).ToArray();
    ...
    private static double Value(FssRgbPixel pixel, int channel) =>
        channel switch
        {
            0 => pixel.Red,
            1 => pixel.Green,
