...
public sealed class MiningActivityOverlayCoordinator : IDisposable
{
    ...
    public MiningActivityOverlayCoordinator(MiningWorkspaceViewModel workspace, FiregroupsWorkspaceViewModel firegroupsWorkspace, OverlayPresentationSession session)
    {
    ...
    {
        if (e.PropertyName is nameof(MiningWorkspaceViewModel.ShouldShowNotifications) or nameof(FiregroupsWorkspaceViewModel.ShouldShow)) Synchronize();
    }
