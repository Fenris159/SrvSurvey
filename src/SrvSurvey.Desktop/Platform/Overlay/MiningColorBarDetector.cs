...
{
    internal static MiningBarAnalysis Analyze(
    ...
    private sealed class Frame
    {
        ...
        internal MiningBarAnalysis RunAnalysis()
        {
            ...
            locatedGrid =
                previous?.HasAnchor == true
                    ...
                    : null;
            if (locatedGrid is { } located)
            {
                (offsetX, offsetY) = located;
            }
        ...
        private MiningBarAnalysis Unknown() =>
            new(states, offsetX, offsetY)
            {
        ...
        private int AnchorSlots()
        {
            ...
            {
                return previous?.AnchorSlots ?? 0;
            }

            return assignments.Keys.Aggregate(0, (bits, slot) => bits | (1 << slot))
                | Enumerable
                    .Range(0, 6)
                    .Where(i => states[i] == MiningBarState.Unknown)
        ...
        private void FindAssignments()
        {
            ...
            foreach (var candidate in candidates)
            {
                for (var anchor = 0; anchor < 6; anchor++)
                {
                    ...
                    if (!IsBetterAssignment(matches.Count, distance, bestCount, bestDistance))
                    {
                        continue;
                    }
        ...
        private Dictionary<int, int> MatchSlots(double dx, double dy)
        {
            ...
            for (var i = 0; i < candidates.Count; i++)
            {
                ...
                var nearest = centers
                    ...
                    .First();
                if (nearest.Distance > Math.Pow(radius * .6, 2))
                {
                    continue;
                }

                if (!matches.TryAdd(nearest.Slot, i))
                {
                    matches.Clear();
                    break;
                }
        ...
        private void UpdateOffset()
        {
            ...
            if (Math.Abs(nextX) > allowance || Math.Abs(nextY) > allowance)
            {
                assignments.Clear();
            }
            else
        ...
        private static bool HasBarFragment(
            List<Group> fragments,
            double x,
            double y,
            double radius,
        ...
        private static double ScoreGroup(
            IFssPixelSource source,
            double x,
            double y,
            double radius,
        ...
        )
        {
            ...
            foreach (var (dx, dy, scale, tilt) in ShapeVariations())
            {
                ...
                if (
                    score > best
                    && MiningBarShape.Score(source, x + dx, y + dy + radius * gap, radius * scale, geometry, tilt)
                        >= .55
                )
                {
                    best = score;
        ...
        private static IEnumerable<Group> FindGroups(ColorMask mask)
        {
            ...
            for (var y = 0; y < mask.Height; y++)
            {
                for (var x = 0; x < mask.Width; x++)
                {
                    ...
                    {
                        yield return FloodGroup(mask, visited, x, y);
                    }
                }
            }
        }
    ...
    internal static bool MatchesColor(FssRgbPixel pixel, FssRgbPixel target)
    {
        ...
        return Math.Abs((pixel.Red - minimum) / (double)chroma - (target.Red - targetMin) / (double)targetChroma) <= .3
            && Math.Abs((pixel.Green - minimum) / (double)chroma - (target.Green - targetMin) / (double)targetChroma)
                <= .3
            && Math.Abs((pixel.Blue - minimum) / (double)chroma - (target.Blue - targetMin) / (double)targetChroma)
                <= .3;
    }
}
