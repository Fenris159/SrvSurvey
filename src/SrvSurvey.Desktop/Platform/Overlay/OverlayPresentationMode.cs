...
public sealed record OverlayPresentationDecision(OverlayPresentationMode Mode, string Reason);

public static class OverlayPresentationModeSelector
{
    public const string HostOverrideVariable = "SRVSURVEY_OVERLAY_HOST";

    public static OverlayPresentationDecision DetectCurrent(OverlayPlatformCapabilities capabilities)
    {
    ...
    )
    {
        ...
        if (
            SupportsCombinedWindow(capabilities)
            && capabilities.UsesX11Compatibility
            && IsGamescope(gamescopeWaylandDisplay, gamescopeDisplay, currentDesktop)
        )
        {
            return new OverlayPresentationDecision(
                OverlayPresentationMode.CombinedWindow,
    ...
    private static bool SupportsCombinedWindow(OverlayPlatformCapabilities capabilities)
    {
        return capabilities.Host == OverlayHostKind.Windows || capabilities.UsesX11Compatibility;
    }

    private static bool IsGamescope(string? gamescopeWaylandDisplay, string? gamescopeDisplay, string? currentDesktop)
    {
        return !string.IsNullOrWhiteSpace(gamescopeWaylandDisplay)
            || !string.IsNullOrWhiteSpace(gamescopeDisplay)
            || currentDesktop?.Contains("gamescope", StringComparison.OrdinalIgnoreCase) == true;
    }

    private static bool IsMultipleWindowOverride(string? value)
    {
        return value?.ToLowerInvariant() is "multiple" or "multi" or "separate" or "windows";
    }

    private static bool IsCombinedWindowOverride(string? value)
    {
        return value?.ToLowerInvariant() is "combined" or "single" or "canvas";
    }
}
