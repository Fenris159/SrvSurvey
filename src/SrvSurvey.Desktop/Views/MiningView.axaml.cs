...
public sealed partial class MiningView : UserControl
{
    ...
    private async void AttachScreenshots_Click(object? sender, RoutedEventArgs e) => await WithFiles(async (vm, storage) =>
    {
        if (vm.SelectedSession is not { } session) { vm.Status = "Select a completed session first."; return; }
        var files = await storage.OpenFilePickerAsync(new FilePickerOpenOptions { Title = "Attach session screenshots", AllowMultiple = true, FileTypeFilter = [FilePickerFileTypes.ImageAll] });
        foreach (var file in files)
            if (file.TryGetLocalPath() is { } path && !session.Screenshots.Contains(path, StringComparer.OrdinalIgnoreCase)) session.Screenshots.Add(path);
        vm.SaveNotesCommand.Execute(null);
        vm.Status = $"{session.Screenshots.Count} screenshots attached.";
    });
    private async void ImportJournals_Click(object? sender, RoutedEventArgs e) => await WithFiles(async (vm, storage) =>
    {
        var files = await storage.OpenFilePickerAsync(new FilePickerOpenOptions { Title = "Import earlier journals for this commander", AllowMultiple = true });
        var paths = files.Select(f => f.TryGetLocalPath()).OfType<string>().ToArray();
        if (paths.Length > 0) await vm.ImportJournalsAsync(paths);
    });
    private async void Backup_Click(object? sender, RoutedEventArgs e) => await WithFiles(async (vm, storage) =>
    {
        var file = await storage.SaveFilePickerAsync(new FilePickerSaveOptions { Title = "Mining backup", SuggestedFileName = "mining-backup.zip", DefaultExtension = "zip" });
        if (file is null) return;
    ...
    private async void Restore_Click(object? sender, RoutedEventArgs e) => await WithFiles(async (vm, storage) =>
    {
        ...
        {
            if (buffer.Length + read > 256 * 1024 * 1024) throw new IOException("Mining backup exceeds 256 MB.");
            await buffer.WriteAsync(chunk.AsMemory(0, read));
        }
        var bytes = buffer.ToArray();
        if (bytes.Length > 2 && bytes[0] == 'P' && bytes[1] == 'K') await vm.RestorePackageAsync(bytes);
        else vm.Restore(System.Text.Encoding.UTF8.GetString(bytes));
    });
    private async Task WithFiles(Func<MiningWorkspaceViewModel, IStorageProvider, Task> action)
    {
        if (Model is not { } vm || TopLevel.GetTopLevel(this)?.StorageProvider is not { } storage) return;
        try { await action(vm, storage); }
        catch (Exception ex) when (ex is IOException or UnauthorizedAccessException or InvalidOperationException or NotSupportedException or System.Text.Json.JsonException or Microsoft.VisualBasic.FileIO.MalformedLineException) { vm.Status = ex.Message; }
    }
