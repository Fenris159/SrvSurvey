...
public sealed partial class PowerplayRingAcquireSharedCluster : UserControl
{
    private PowerplayRingAcquireClusterViewModel? cluster;
    private string lastActivePath = "";
    private string lastInactivePath = "";

    public PowerplayRingAcquireSharedCluster()
    {
        InitializeComponent();
    ...
    private void ChangeCluster()
    {
        ...
        cluster = DataContext as PowerplayRingAcquireClusterViewModel;
        cluster?.PropertyChanged += OnClusterChanged;
    ...
    {
        if (args.PropertyName is nameof(PowerplayRingAcquireClusterViewModel.SelectedSellSystem))
        {
            lastActivePath = "";
            lastInactivePath = "";
            UpdateLinks();
    ...
    private void UpdateLinks()
    {
        ...
        Dictionary<PowerplayRingAcquireSellNode, double> sells = AnchorPositions<PowerplayRingAcquireSellNode>(
            SellItems,
            "SellLinkAnchor"
        );
        Dictionary<PowerplayRingAcquireMiningNode, double> mining = AnchorPositions<PowerplayRingAcquireMiningNode>(
            MiningItems,
            "MiningLinkAnchor"
        );
        ...
        {
            PowerplayRingAcquireSellNode sell = cluster.SellNodes[index];
    ...
    private static void OnSellPointerPressed(object? sender, PointerPressedEventArgs args)
    {
        ...
        {
            if (current is Border { Name: "SellLinkAnchor", DataContext: PowerplayRingAcquireSellNode sell })
            {
    ...
    private bool AlignMiningToSelectedSell()
    {
        Border? selected = SellItems
            ...
            .FirstOrDefault(border =>
                border.Name == "SellLinkAnchor"
                && border.DataContext is PowerplayRingAcquireSellNode { IsSelected: true }
            );
