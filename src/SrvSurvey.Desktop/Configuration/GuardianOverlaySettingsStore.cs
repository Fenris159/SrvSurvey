...
public sealed class GuardianOverlaySettingsStore
{
    ...
    public GuardianOverlayPreferences Load()
    {
        ...
        return new GuardianOverlayPreferences(
            ...
            GetBoolean(settings, "AutoShowRamTah", defaults.AutoShowRamTah),
            GetBoolean(settings, "SuppressForActiveBuildProjects", defaults.SuppressForActiveBuildProjects),
            GetBoolean(settings, "AutoZoomNearObelisks", defaults.AutoZoomNearObelisks),
            GetBoolean(settings, "AutoZoomInSrvTurret", defaults.AutoZoomInSrvTurret),
            GetBoolean(settings, "ShowComponentMaterials", defaults.ShowComponentMaterials),
            GetInteger(settings, "OverlaySizeIndex", defaults.OverlaySizeIndex),
            GetBoolean(settings, "DisableRuinsMeasurementGrid", defaults.DisableRuinsMeasurementGrid),
            GetBoolean(settings, "DisableAerialAlignmentGrid", defaults.DisableAerialAlignmentGrid),
            GetBoolean(settings, "ShowMapNotes", defaults.ShowMapNotes),
            GetBoolean(settings, "ShowMapLegend", defaults.ShowMapLegend)
        );
    }
    ...
    public void Save(GuardianOverlayPreferences preferences)
    {
        ...
        documentStore.Update(root =>
        {
            ...
            settings["AutoShowGuardianSummary"] = preferences.AutoShowGuardianSummary;
            ...
            settings["AutoZoomNearObelisks"] = preferences.AutoZoomNearObelisks;
            settings["AutoZoomInSrvTurret"] = preferences.AutoZoomInSrvTurret;
            settings["ShowComponentMaterials"] = preferences.ShowComponentMaterials;
            settings["OverlaySizeIndex"] = Math.Clamp(preferences.OverlaySizeIndex, 0, 4);
            settings["DisableRuinsMeasurementGrid"] = preferences.DisableRuinsMeasurementGrid;
            settings["DisableAerialAlignmentGrid"] = preferences.DisableAerialAlignmentGrid;
...
{
    public static GuardianOverlayPreferences Default { get; } =
        new(
