...
public sealed class UnavailableGameTextInputService : IGameTextInputService
{
    ...
    {
        UnavailableReason = string.IsNullOrWhiteSpace(reason) ? "Game text entry is unavailable." : reason;
    }
...
internal sealed class SharpHookGameTextInputService : IGameTextInputService
{
    ...
    internal SharpHookGameTextInputService(Func<string, UioHookResult> simulateText)
    {
        this.simulateText = simulateText ?? throw new ArgumentNullException(nameof(simulateText));
    }
    ...
    public GameTextInputResult EnterText(string text)
    {
        ...
        {
            return new GameTextInputResult(false, "Galaxy Map text entry failed: " + exception.Message);
        }
    }
}
