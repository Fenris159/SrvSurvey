...
public sealed class GlobalInputBindingRouter
{
    ...
    public void Update(GlobalInputSettings settings)
    {
        ...
        foreach (GlobalInputAction action in GlobalInputActionCatalog.All.Select(definition => definition.Action))
        {
            string? configured = settings.Bindings.GetValueOrDefault(action);
            if (!InputChord.TryNormalize(configured, out string? chord))
            {
                continue;
            }
    ...
    {
        action = default;
        Dictionary<string, GlobalInputAction> routes = Volatile.Read(ref actionsByChord);
        return InputChord.TryNormalize(chord, out string? normalized) && routes.TryGetValue(normalized, out action);
    }
}
