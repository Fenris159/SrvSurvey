...
public static class HorizontalScrollInputBehavior
{
    ...
    private static void OnEnabledChanged(ScrollViewer scrollViewer, AvaloniaPropertyChangedEventArgs eventArgs)
    {
        if (eventArgs.NewValue is true)
        {
            Subscriptions.GetValue(scrollViewer, static target => new Subscription(target));
            return;
        }
    ...
    private sealed class Subscription : IDisposable
    {
        ...
        private void OnPointerWheelChanged(object? sender, PointerWheelEventArgs eventArgs)
        {
            ...
            )
            {
                ...
                {
                    ForwardVerticalInputToAncestor(eventArgs.Source, -eventArgs.Delta.Y, isWheel: true, eventArgs);
                }
                return;
            }

            if (!usesShiftFallback && scrollViewer.FlowDirection == FlowDirection.RightToLeft)
            {
        ...
        private void OnScrollGesture(object? sender, ScrollGestureEventArgs eventArgs)
        {
            if (
                Math.Abs(eventArgs.Delta.X) < double.Epsilon
                || Math.Abs(eventArgs.Delta.X) < Math.Abs(eventArgs.Delta.Y)
            )
            {
                if (Math.Abs(eventArgs.Delta.Y) >= double.Epsilon)
                {
                    ForwardVerticalInputToAncestor(eventArgs.Source, eventArgs.Delta.Y, isWheel: false, eventArgs);
                }
                return;
            }
        ...
        private static bool CanScrollHorizontally(ScrollViewer candidate) =>
            candidate.HorizontalScrollBarVisibility != ScrollBarVisibility.Disabled
