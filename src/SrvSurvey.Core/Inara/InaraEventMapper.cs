...
internal sealed class InaraEventMapper
{
        ...
        switch (name)
        {
            ...
            case "ShipyardTransfer":
                addRequired(
                    ...
                    obj(
                        (ShipTypeKey, entry[ShipTypeProperty]),
                        (ShipGameIdKey, entry[ShipIdProperty]),
                        (StarSystemNameKey, context.SystemName),
                        (StationNameKey, context.StationName),
                        ("transferTime", entry["TransferTime"])
                    ),
                    $"ship-transfer:{entry[ShipIdProperty]}"
                );
                break;
            case "StoredShips":
            ...
            case "MissionAbandoned":
                addRequired(
                    ...
                    timestamp,
                    obj((MissionGameIdKey, entry[MissionIdKey])),
                    $"mission-abandoned:{entry[MissionIdKey]}"
                );
                break;
            ...
            case "MissionFailed":
                addRequired(
                    ...
                    timestamp,
                    obj((MissionGameIdKey, entry[MissionIdKey])),
                    $"mission-failed:{entry[MissionIdKey]}"
                );
                break;
            case "Died":
            case "Interdicted":
            ...
            case "DeleteSuitLoadout":
                addRequired(
                    ...
                    timestamp,
                    obj((LoadoutGameIdKey, entry[LoadoutIdKey])),
                    $"suit:{entry[LoadoutIdKey]}"
                );
                break;
    ...
    private void updateMulticrewState(string name, JObject entry)
    {
        ...
        else if (entry["Multicrew"] is JValue { Type: JTokenType.Boolean, Value: true })
        {
            InMulticrew = true;
        }
    ...
    private bool updateMaterialDelta(string name, JObject entry)
    {
        switch (name)
        {
            ...
            case "EngineerLegacyConvert":
                if (entry.Value<bool?>("IsPreview") == true)
                {
                    return false;
                }
    ...
    )
    {
        ...
        {
            var data = new JArray(
                cargo.OrderBy(item => item.Key).Select(item => obj((ItemNameKey, item.Key), (ItemCountKey, item.Value)))
            );
        ...
        if (materialsChanged && hasMaterialsSnapshot)
        {
            var data = new JArray(
                materials
                    ...
                    .Select(item => obj((ItemNameKey, item.Key), (ItemCountKey, item.Value)))
            );
    ...
    private void mapProgress(string timestamp, JObject entry, List<InaraEvent> events)
    {
        ...
        if (values.Count > 0)
        {
            events.Add(new("setCommanderRankPilot", timestamp, values, "ranks"));
        }
    }
        ...
        foreach (var property in entry.Properties().Where(p => p.Name is not TimestampKey and not EventKey))
        {
            ...
            events.Add(
                new(
                    ...
                    timestamp,
                    obj(("rankName", normalizeRank(property.Name)), (RankValueKey, value), ("rankProgress", 0d)),
    ...
    private static void mapEngineer(string timestamp, JObject entry, List<InaraEvent> events)
    {
        ...
        addRequired(
            ...
            timestamp,
            obj(("engineerName", entry["Engineer"]), (RankValueKey, entry["Rank"]), ("rankStage", entry["Progress"])),
    ...
    private static void mapMajorFactionReputation(string timestamp, JObject entry, List<InaraEvent> events)
    {
        ...
        {
            events.Add(
                new("setCommanderReputationMajorFaction", timestamp, new JArray(reputation), "reputation:major")
            );
        }
    }
    ...
    private static void mapLocation(string timestamp, JObject entry, List<InaraEvent> events)
    {
        ...
        {
            copy(data, entry, (StarSystemBodyNameKey, BodyKey));
            data["starsystemBodyCoords"] = new JArray(
                entry[LatitudeKey]!.DeepClone(),
                entry[LongitudeKey]!.DeepClone()
            );
        }
    ...
    {
        if (entry.Value<bool?>("PlayerControlled") == false || entry.Value<bool?>("OnPlanet") == false)
        {
            return;
        }
    ...
    {
        if (entry["Factions"] is not JArray factions)
        {
            return;
        }
        ...
        var reputation = factions
            ...
            .Select(f =>
                obj(("minorfactionName", f["Name"]), ("minorfactionReputation", f.Value<double>("MyReputation") / 100d))
            )
            .ToArray();
        if (reputation.Length > 0)
        {
            events.Add(
                new("setCommanderReputationMinorFaction", timestamp, new JArray(reputation), "reputation:minor")
            );
        }
    }
    ...
    private static void mapStatistics(string timestamp, JObject entry, List<InaraEvent> events)
    {
        ...
        if (stats.Count > 0)
        {
            events.Add(new("setCommanderGameStatistics", timestamp, stats, "statistics"));
        }
    }
    ...
    private static void mapShipyard(string timestamp, JObject entry, InaraContext context, List<InaraEvent> events)
    {
        if (entry["StoreShipID"] != null)
        {
            addRequired(
                events,
                SetCommanderShipEvent,
                timestamp,
                obj(
                    (ShipTypeKey, entry["StoreOldShip"]),
                    (ShipGameIdKey, entry["StoreShipID"]),
                    (StarSystemNameKey, context.SystemName),
                    (StationNameKey, context.StationName)
                ),
                $"ship:{entry["StoreShipID"]}"
            );
        }
        ...
        if (entry["SellShipID"] != null)
        {
            addRequired(
                ...
                obj(
                    ...
                    (ShipGameIdKey, entry["SellShipID"])
                ),
                $"ship:{entry["SellShipID"]}"
            );
        }
    }
    ...
    private static void mapStoredShips(string timestamp, JObject entry, List<InaraEvent> events)
    {
        foreach (var ship in (entry["ShipsHere"] as JArray)?.OfType<JObject>() ?? [])
        {
            addRequired(
                ...
                timestamp,
                obj(
                    (ShipTypeKey, ship[ShipTypeProperty]),
                    (ShipGameIdKey, ship[ShipIdProperty]),
                    (ShipNameKey, ship["Name"]),
                    (IsHotKey, ship["Hot"]),
                    (StarSystemNameKey, entry[StarSystemProperty]),
        ...
        foreach (var ship in (entry["ShipsRemote"] as JArray)?.OfType<JObject>() ?? [])
        {
            addRequired(
                ...
                obj(
                    ...
                    (ShipNameKey, ship["Name"]),
                    (IsHotKey, ship["Hot"]),
                    (StarSystemNameKey, ship[StarSystemProperty]),
                    (MarketIdKey, ship["ShipMarketID"])
                ),
                $"ship:{ship[ShipIdProperty]}"
            );
        }
    ...
    private static void mapLoadout(string timestamp, JObject entry, InaraContext context, List<InaraEvent> events)
    {
        ...
        addRequired(
            ...
            timestamp,
            obj((ShipTypeKey, shipType), (ShipGameIdKey, shipId), ("shipLoadout", modules)),
            $"loadout:{shipId}"
        );
        ...
        var ship = obj(
            ...
            ("shipHullValue", entry["HullValue"]),
            ("shipModulesValue", entry["ModulesValue"]),
            ("shipRebuyCost", entry["Rebuy"])
        );
    ...
    private static JObject mapModule(JObject module)
    {
        ...
        if (module["Engineering"] is JObject engineering)
        {
            ...
            {
                mapped["modifiers"] = new JArray(
                    modifiers
                        .OfType<JObject>()
                        .Select(modifier =>
                            obj(
                                ("name", modifier["Label"]),
                                ("value", modifier["Value"] ?? modifier["ValueStr"]),
    ...
    private static void mapStoredModules(string timestamp, JObject entry, List<InaraEvent> events)
    {
        ...
        foreach (var item in storedModules.OrderBy(i => i.Value<int?>("StorageSlot")))
        {
            var module = obj(
                ...
                (IsHotKey, item["Hot"]),
                (StarSystemNameKey, item[StarSystemProperty]),
                (MarketIdKey, item[MarketIdProperty])
            );
    ...
    private static void mapMissionAccepted(
        string timestamp,
        JObject entry,
        InaraContext context,
        List<InaraEvent> events
    )
    {
        var data = obj(
            ...
            ("starsystemNameOrigin", context.SystemName),
            ("stationNameOrigin", context.StationName),
    ...
    private static void mapMissionCompleted(string timestamp, JObject entry, List<InaraEvent> events)
    {
        ...
        if (entry["FactionEffects"] is JArray factionEffects)
        {
            ...
            foreach (var faction in factionEffects.OfType<JObject>())
            {
                ...
                if (influence != null)
                {
                    effect["influenceGain"] = influence;
                }
        ...
        addRequired(
            ...
            timestamp,
            data,
            $"mission-completed:{entry[MissionIdKey]}"
        );
    }

    private static JArray mapRewards(JArray rewards) =>
        new(
            rewards
                .OfType<JObject>()
                .Select(item => obj((ItemNameKey, item["Name"]), (ItemCountKey, item[CountProperty])))
        );

    private static void mapCombat(
        string name,
        string timestamp,
        JObject entry,
        InaraContext context,
    ...
    )
    {
        ...
        switch (name)
        {
            case "Died":
                ...
                {
                    data["wingOpponentNames"] = new JArray(
                        killers.OfType<JObject>().Select(k => k["Name"]?.DeepClone())
                    );
                }
                else
                {
    ...
    private static JToken? opponent(JObject entry, string primary)
    {
        ...
        {
            return entry[primary]!.DeepClone();
        }

        if (entry[FactionKey] != null)
        {
            return entry[FactionKey]!.DeepClone();
        }

        if (entry[PowerProperty] != null)
        {
            return entry[PowerProperty]!.DeepClone();
        }

        if (entry.Value<bool?>("IsThargoid") == true || entry.Value<bool?>("isThargoid") == true)
        {
            return "Thargoid";
        }
    ...
    private static void mapShipLocker(string timestamp, JObject entry, List<InaraEvent> events)
    {
        ...
        events.Add(
            new(
                ...
                timestamp,
                new JArray(types.Select(type => obj(("itemType", type)))),
                "locker:reset"
            )
        );
        var data = new JArray();
        foreach (var type in types)
        {
            foreach (var item in ((JArray)entry[type]!).OfType<JObject>())
            {
                data.Add(
                    obj(
                        (ItemNameKey, item["Name"]),
                        (ItemCountKey, item[CountProperty]),
                        ("itemType", type),
        ...
        foreach (var module in (entry["Modules"] as JArray)?.OfType<JObject>() ?? [])
        {
            modules.Add(
                obj(
                    ...
                    (ItemNameKey, module["ModuleName"]),
                    ("itemClass", module["Class"]),
                    ("itemGameID", module["SuitModuleID"]),
                    (
                        EngineeringKey,
                        new JArray((module["WeaponMods"] as JArray)?.Select(mod => obj((BlueprintNameKey, mod))) ?? [])
                    )
                )
            );
        }
    ...
    private static void mapSuitModule(string timestamp, JObject entry, List<InaraEvent> events)
    {
        var module = obj(
            ...
            (
                EngineeringKey,
                new JArray((entry["WeaponMods"] as JArray)?.Select(mod => obj((BlueprintNameKey, mod))) ?? [])
            )
        );
        addRequired(
            events,
    ...
    private static void mapCommunityGoals(string timestamp, JObject entry, List<InaraEvent> events)
    {
        foreach (var goal in (entry["CurrentGoals"] as JArray)?.OfType<JObject>() ?? [])
        {
            ...
            var data = obj(
                ...
                (StationNameKey, goal["MarketName"]),
    ...
    private static void mapFriend(string timestamp, JObject entry, List<InaraEvent> events)
    {
        ...
        if (eventName != null)
        {
            addRequired(
                ...
                eventName,
                timestamp,
                obj(("commanderName", entry["Name"]), ("gamePlatform", "pc")),
                $"friend:{entry["Name"]}"
            );
        }
    }
    ...
    {
        if (context.ShipId is null or < 0 || string.IsNullOrWhiteSpace(context.ShipType))
        {
            return null;
        }

        return obj(
            (ShipTypeKey, context.ShipType),
            (ShipGameIdKey, context.ShipId),
            (ShipNameKey, context.ShipName),
            ("shipIdent", context.ShipIdent),
            ("isCurrentShip", true)
        );
    }
    ...
    private static void addShipIdentity(JObject data, InaraContext context, bool? isTaxi)
    {
        ...
        if (!string.IsNullOrWhiteSpace(context.ShipType))
        {
            data[ShipTypeKey] = context.ShipType;
        }

        if (context.ShipId is >= 0)
        {
            data[ShipGameIdKey] = context.ShipId;
        }
    }
    ...
    private static string normalizeRank(string rank) =>
        rank.Equals("Exploration", StringComparison.OrdinalIgnoreCase) ? "explore" : rank.ToLowerInvariant();

    private static string? itemName(JObject entry) =>
        entry.Value<string>("Type")
        ?? entry.Value<string>("Name")
        ?? entry.Value<string>("Material")
        ?? entry.Value<string>("Commodity");
