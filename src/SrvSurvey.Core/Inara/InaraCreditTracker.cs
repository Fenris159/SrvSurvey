...
internal sealed class InaraCreditTracker
{
    ...
    private static long? TryTradeAndServiceDelta(JObject entry, string? eventName)
    {
        return eventName switch
        {
            ...
            "RefuelAll" or "RefuelPartial" or "Repair" or "RepairAll" or "RestockVehicle" => -valueOrZero(
                entry,
                "Cost"
            ),
    ...
    public InaraEvent? CreateReport(string timestamp, bool force, bool includeAssets = false)
    {
        ...
        HasUnreportedChanges = false;
        return new InaraEvent("setCommanderCredits", timestamp, BuildCreditsPayload(includeAssets), "credits");
    }
    ...
    private bool ShouldReport(DateTimeOffset reportAt)
    {
        ...
        return !lastReportAt.HasValue || reportAt - lastReportAt.Value >= ReportInterval;
    }

    private JObject BuildCreditsPayload(bool includeAssets)
    {
        var data = new JObject { ["commanderCredits"] = credits!.Value };
        if (loan.HasValue)
        {
    ...
    private static long organicDataValue(JObject entry) =>
        (entry["BioData"] as JArray)
            ?.OfType<JObject>()
            .Sum(item => valueOrZero(item, "Value") + valueOrZero(item, "Bonus"))
        ?? 0;
