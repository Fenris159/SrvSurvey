...
/// </summary>
internal sealed record InaraSession(string Commander, string FrontierId, string? JournalPath, bool IsLive, bool IsBeta)
{
...
internal sealed record InaraEvent(string Name, string Timestamp, JToken Data, string? ReplaceKey = null);
...
{
    public static JObject Build(string appVersion, InaraCredentials credentials, IReadOnlyCollection<InaraEvent> events)
    {
        ...
        return new JObject
        {
            ["header"] = header,
            ["events"] = new JArray(
                events.Select(entry => new JObject
                {
...
internal sealed class InaraEventQueue
{
    ...
    public int Enqueue(string apiKey, IEnumerable<InaraEvent> events, int maximumCount = DefaultMaximumCount)
    {
        lock (sync)
        {
            foreach (var entry in events)
            {
                ...
                {
                    pending.RemoveAll(item => item.ApiKey == apiKey && item.Event.ReplaceKey == entry.ReplaceKey);
                }
    ...
    public int Requeue(IEnumerable<InaraQueuedEvent> events, int maximumCount = DefaultMaximumCount)
    {
        lock (sync)
        {
            var retained = events
                .Where(item =>
                    string.IsNullOrWhiteSpace(item.Event.ReplaceKey)
                    || !pending.Any(current =>
                        current.ApiKey == item.ApiKey && current.Event.ReplaceKey == item.Event.ReplaceKey
                    )
                )
                .ToList();
