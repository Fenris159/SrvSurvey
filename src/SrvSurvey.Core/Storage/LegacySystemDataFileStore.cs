...
internal sealed class LegacySystemDataFileStore
{
    ...
    private static readonly ConcurrentDictionary<string, SemaphoreSlim> ProfileWriteLocks = new(
        StringComparer.OrdinalIgnoreCase
    );
                        ...
                        if (File.Exists(path))
                        {
                            ...
                            root =
                                ...
                                ?? throw new InvalidDataException(
                                    "The system data file is malformed and was not overwritten: " + readResult.Error
                                );
                        }
                        else
                        {
    ...
    public async Task<LegacySystemDataFileMutationResult<T>> UpdateWithResultAsync<T>(
        LegacySystemDataFileContext context,
        ...
        return await ExecuteProfileWriteAsync(
                ...
                async token =>
                {
                    ...
                    try
                    {
                        ...
                        return new LegacySystemDataFileUpdateResult(path, true, changed, null);
                    }
                    finally
                    {
                        updateLock.Release();
    ...
    private static JsonObject CreateSystemData(LegacySystemDataFileContext context)
    {
...
internal sealed record LegacySystemDataFileLoadResult(string Path, bool Exists, JsonObject? Root, string? Error);

internal sealed record LegacySystemDataFileUpdateResult(string Path, bool Exists, bool Changed, string? Error);

internal sealed record LegacySystemDataFileMutationResult<T>(string Path, T Value);
