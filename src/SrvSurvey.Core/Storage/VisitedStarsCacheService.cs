...
public interface IVisitedStarsCacheService
{
    ...
    Task<VisitedStarsCacheRestoreResult> RestoreAsync(string targetPath, CancellationToken cancellationToken = default);
}
...
) : IVisitedStarsCacheService
{
    ...
    private static readonly Uri Endpoint = new("https://edgalaxy.net/visitedstars", UriKind.Absolute);
    private readonly string downloadDirectory = Path.GetFullPath(downloadDirectory);
    ...
    )
    {
        ...
        try
        {
            ...
            try
            {
                ...
                activated = true;
                await VerifyHashAsync(target, download.Sha256, cancellationToken).ConfigureAwait(false);
            ...
            return new VisitedStarsCacheSwapResult(target, backup, download.Path, originalHash, download.Sha256);
        }
        finally
        {
    ...
    )
    {
        ...
        try
        {
            ...
            try
            {
                ...
                await CopyAndVerifyAsync(backup, activationStage, backupHash, overwrite: false, cancellationToken)
                    .ConfigureAwait(false);
                EnsureGameStopped();
                File.Move(activationStage, target, true);
                activated = true;
                await VerifyHashAsync(target, backupHash, cancellationToken).ConfigureAwait(false);
                TryDeleteIfExists(rollback);
            }
            catch
            {
    ...
    private async Task<DownloadedCache> DownloadAsync(string systemName, CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidDataException("The downloaded visited-stars cache exceeded the safety limit.");
        }
        ...
        try
        {
            ...
            await VerifyHashAsync(finalPath, hash, cancellationToken).ConfigureAwait(false);
    ...
    )
    {
        if (File.Exists(backup))
        {
            _ = await ReadOrRecordBackupHashAsync(backup, cancellationToken).ConfigureAwait(false);
            return;
        }
        ...
        try
        {
            ...
            await WriteBackupHashAsync(backup, originalHash, cancellationToken).ConfigureAwait(false);
        }
        finally
        {
    ...
    private static async Task<string> ReadOrRecordBackupHashAsync(string backup, CancellationToken cancellationToken)
    {
        ...
        if (File.Exists(sidecar))
        {
            ...
            await VerifyHashAsync(backup, expected.ToLowerInvariant(), cancellationToken).ConfigureAwait(false);
    ...
    )
    {
        ...
        await VerifyHashAsync(destination, expectedHash, cancellationToken).ConfigureAwait(false);
    }
    ...
    private void EnsureGameStopped()
    {
        if (isGameRunning())
        {
            throw new InvalidOperationException("Close Elite Dangerous before changing VisitedStarsCache.dat.");
        }
    }
...
public sealed record VisitedStarsCacheRestoreResult(string TargetPath, string BackupPath, string RestoredSha256);
