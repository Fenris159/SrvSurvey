...
public sealed class SystemNotesSettingsStore(string dataDirectory)
{
    ...
    public SystemNotesSettingsLoadResult Load()
    {
        if (!File.Exists(Path))
        {
            return new SystemNotesSettingsLoadResult(Path, false, SystemNotesSettingsSnapshot.Default, null);
        }
        ...
        {
            return new SystemNotesSettingsLoadResult(Path, true, null, $"Could not read {Path}: {exception.Message}");
        }
    }

    public async Task SaveAlwaysOnTopAsync(bool alwaysOnTop, CancellationToken cancellationToken = default)
    {
        await SaveSettingsAsync(root => root["systemNotesTopMost"] = alwaysOnTop, cancellationToken)
    ...
    private async Task SaveSettingsAsync(Action<JsonObject> update, CancellationToken cancellationToken)
    {
...
{
    public static SystemNotesSettingsSnapshot Default { get; } = new(false, null, false, false);
}
