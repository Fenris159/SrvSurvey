...
public sealed class HumanSiteMaterialStore
{
    ...
    public HumanSiteMaterialStore(string dataDirectory, TimeProvider? timeProvider = null)
    {
    ...
    )
    {
        ...
        return new HumanSiteMaterialLoadResult(path, true, true, survey, null, warnings);
    }
    ...
    )
    {
        ...
        try
        {
            ...
            await WriteAsync(path, root, cancellationToken).ConfigureAwait(false);
            var warnings = new List<string>();
            return new HumanSiteMaterialMutationResult(path, addedCount, ReadSurvey(root, warnings));
        }
        finally
        {
            fileLock.Release();
        ...
        try
        {
            ...
            if (path is null)
            {
                return new HumanSiteMaterialMutationResult(
                    GetNewPath(folder, context),
                    0,
                    HumanSiteMaterialSurvey.Empty with
                    {
                        Completed = true,
                    }
                );
            ...
            await WriteAsync(path, read.Root, cancellationToken).ConfigureAwait(false);
            var warnings = new List<string>();
            return new HumanSiteMaterialMutationResult(path, 0, ReadSurvey(read.Root, warnings));
        }
        finally
        {
            fileLock.Release();
    ...
    )
    {
        ...
        try
        {
            ...
            await WriteAsync(path, root, cancellationToken).ConfigureAwait(false);
            var warnings = new List<string>();
            return new HumanSiteMaterialMutationResult(path, 0, ReadSurvey(root, warnings));
        }
        finally
        {
            fileLock.Release();
    ...
    private static HumanSiteMaterialSurvey ReadSurvey(JsonObject root, List<string> warnings)
    {
        ...
        if (root["matLocations"] is JsonArray locations)
        {
            foreach (var node in locations)
            {
                ...
                {
                    warnings.Add("A saved settlement material location was invalid and was ignored.");
                }
            }
        }
    ...
    private static bool TryParseLocation(string? text, out HumanSiteCollectedMaterial? material)
    {
        material = null;
        ...
        material = new HumanSiteCollectedMaterial(name, null, type, 1, new HumanSiteMapPoint(x, y), null);
        return true;
    }
    ...
    private static Dictionary<string, int> ReadCounts(JsonNode? node)
    {
        ...
        return counts
            ...
            .ToDictionary(pair => pair.Key, pair => ReadInt32(pair.Value)!.Value, StringComparer.OrdinalIgnoreCase);
    }

    private static void ApplyContext(JsonObject root, HumanSiteMaterialContext context)
    {
    ...
    private static string? FindLatestPath(string folder, HumanSiteMaterialContext context)
    {
        return !Directory.Exists(folder)
            ? null
            : Directory
                .EnumerateFiles(folder, $"{context.Site.SystemAddress}-{context.Site.MarketId}-*.json")
                .OrderBy(path => path, StringComparer.OrdinalIgnoreCase)
                .LastOrDefault();
    }

    private static string GetSessionLockKey(string folder, HumanSiteMaterialContext context)
    {
        return Path.Combine(folder, $"{context.Site.SystemAddress}-{context.Site.MarketId}");
    }
...
public sealed record HumanSiteMaterialContext(string FrontierId, HumanSiteLiveSnapshot Site);
...
{
    public static HumanSiteMaterialSurvey Empty { get; } =
        new(
            false,
            -1,
            0,
