...
public sealed class JourneyStore(string dataDirectory)
{
    ...
    )
    {
        ...
        {
            throw new ArgumentException("The journey name cannot be blank.", nameof(request));
        }
        ...
        try
        {
            if (File.Exists(path))
            {
                throw new InvalidOperationException(
                    "A journey already starts at that journal event: " + Path.GetFileName(path)
                );
            }

            await WriteJourneyAsync(path, [], journey, cancellationToken).ConfigureAwait(false);
        }
        finally
        {
            saveLock.Release();
    ...
    public async Task SaveAsync(JourneyDocument journey, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (File.Exists(path))
            {
                ...
                root =
                    ...
                    ?? throw new InvalidDataException(
                        "The journey file is malformed and was not overwritten: " + readResult.Error
                    );
            }
            else
            {
            ...
            await WriteJourneyAsync(path, root, journey, cancellationToken).ConfigureAwait(false);
        }
        finally
        {
            saveLock.Release();
    ...
    )
    {
        ...
        try
        {
            return new JourneyLoadResult(path, true, ParseJourney(frontierId, path, readResult.Root), null);
        }
    ...
    private static JourneyDocument ParseJourney(string frontierId, string path, JsonObject root)
    {
        ...
        if (root["visitedSystems"] is JsonArray visitedSystems)
        {
            for (var index = 0; index < visitedSystems.Count; index++)
            {
                ...
                {
                    throw new InvalidDataException($"The journey {path} has an invalid visitedSystems[{index}].");
                }
    ...
    private static JourneySystemReference? ParseStarReference(JsonNode? node)
    {
        ...
        return
            ...
            || legacyZ is null
            ? null
            : TryCreateStarReference(name, systemAddress.Value, legacyX.Value, legacyY.Value, legacyZ.Value);
    }
    ...
    )
    {
        try
        {
            return new JourneySystemReference(name, systemAddress, new GalacticCoordinate(x, y, z));
        }
    ...
    )
    {
        ...
        root["visitedSystems"] = MergeVisits(root["visitedSystems"] as JsonArray, journey.VisitedSystems);
    ...
    private static void WriteDictionary(JsonObject root, string propertyName, IReadOnlyDictionary<string, int>? values)
    {
        if (values is null)
        {
    ...
    private static void WriteSet<T>(JsonObject root, string propertyName, IReadOnlySet<T>? values)
    {
        if (values is null)
        {
            root.Remove(propertyName);
    ...
    private static void ValidateFolderName(string value, string parameterName)
    {
        ...
        {
            throw new ArgumentException("The folder name cannot be empty.", parameterName);
        }
        ...
        {
            throw new ArgumentException("The value must be a file or folder name, not a path.", parameterName);
        }
    }
    ...
    private static Dictionary<string, int>? ReadStringIntDictionary(JsonObject root, string propertyName)
    {
    ...
    private static HashSet<long>? ReadInt64Set(JsonObject root, string propertyName)
    {
    ...
    private static HashSet<int>? ReadInt32Set(JsonObject root, string propertyName)
    {
        if (root[propertyName] is not JsonArray array)
        {
            return null;
    ...
    private static HashSet<string>? ReadStringSet(JsonObject root, string propertyName)
    {
        if (root[propertyName] is not JsonArray array)
        {
            return null;
    ...
    private sealed record ExistingVisit(int Index, JsonObject? Root, string? Identity);
