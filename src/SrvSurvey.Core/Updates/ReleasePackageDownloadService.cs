...
public sealed record ReleasePackageDownloadProgress(long DownloadedBytes, long TotalBytes);

public sealed record ReleasePackageDownloadResult(string ArchivePath, bool Downloaded, long Size, string Sha256);

public sealed class ReleasePackageDownloadService : IReleasePackageDownloadService
{
    ...
    )
    {
        ...
        try
        {
            ...
            while (true)
            {
                ...
                {
                    throw new InvalidDataException("The package response exceeded the indexed size.");
                }

                hash.AppendData(buffer, 0, read);
                await output.WriteAsync(buffer.AsMemory(0, read), cancellationToken).ConfigureAwait(false);
                progress?.Report(new ReleasePackageDownloadProgress(total, package.Size));
            }
            ...
            return new ReleasePackageDownloadResult(archivePath, true, package.Size, actualHash);
        }
        finally
        {
    ...
    private static void Validate(ReleaseVersion version, CrossPlatformReleasePackage package, string dataDirectory)
    {
        ...
        if (
            ...
            || !string.Equals(Path.GetFileName(package.ArchiveName), package.ArchiveName, StringComparison.Ordinal)
        )
        {
            throw new InvalidDataException("The release package name or archive type is invalid.");
        }

        if (package.Size is <= 0 or > MaximumPackageBytes)
        {
            throw new InvalidDataException("The release package size is outside the supported range.");
        }

        if (package.Sha256.Length != 64 || package.Sha256.Any(character => !Uri.IsHexDigit(character)))
        {
            throw new InvalidDataException("The release package SHA-256 is invalid.");
        }

        if (!package.DownloadUri.IsAbsoluteUri || package.DownloadUri.Scheme != Uri.UriSchemeHttps)
        {
            throw new InvalidDataException("The release package download URI must use HTTPS.");
        }
    }
