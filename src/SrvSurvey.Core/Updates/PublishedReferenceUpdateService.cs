...
{
    public static PublishedReferenceUris Default { get; } =
        new(
...
public sealed class PublishedReferenceUpdateService : IPublishedReferenceUpdateService
{
    ...
    private static readonly string[] GuardianSiteSourceCatalogs = ["Guardian site index", "Guardian published surveys"];
    ...
    public PublishedReferenceUpdateService(
        ...
        : this(indexClient, versionStore, client, uris, timeProvider, null) { }
    ...
    )
    {
        ...
        return await ApplyUpdatePlanAsync(root, previous, remote, plan, cancellationToken).ConfigureAwait(false);
    }
    ...
    )
    {
        ...
        {
            CleanupUpdateDirectories(root, stageRoot, rollbackRoot, backupRoot, activationCompleted);
        }
    }
    ...
    )
    {
        ...
        await ActivateAsync(request.Root, request.StageRoot, request.RollbackRoot, request.Updated, cancellationToken)
            .ConfigureAwait(false);
        return next;
    }

    private static bool HasAnyEntries(string directory) =>
        Directory.EnumerateFileSystemEntries(directory, "*", SearchOption.AllDirectories).Any();
    ...
    )
    {
        ...
        {
            await StageRegionalCodexCandidatesAsync(stageRoot, cancellationToken).ConfigureAwait(false);
    ...
    )
    {
        if (plan.UpdateKnownSystemAddresses)
        {
            await WriteDownloadAsync(
                    ...
                    Path.Combine(stagePublished, KnownSystemAddressCatalog.LegacyFileName),
                    cancellationToken
                )
                .ConfigureAwait(false);
        ...
        {
            await StageBiologyCriteriaAsync(stageRoot, stagePublished, cancellationToken).ConfigureAwait(false);
        ...
        {
            await StageGuardianCatalogsAsync(stagePublished, cancellationToken).ConfigureAwait(false);
            updated.Add("Guardian site indexes and surveys");
        }

        if (plan.UpdateSettlements)
        {
            await StageSettlementsAsync(stageRoot, stagePublished, cancellationToken).ConfigureAwait(false);
            updated.Add("human settlement templates");
        }

        if (plan.UpdateGreenGasGiants)
        {
            await WriteDownloadAsync(uris.GreenGasGiants, Path.Combine(stagePublished, "ggg.json"), cancellationToken)
                .ConfigureAwait(false);
        ...
        {
            await StageNicknamesAsync(stagePublished, cancellationToken).ConfigureAwait(false);
    ...
    private async Task StageGuardianCatalogsAsync(string stagePublished, CancellationToken cancellationToken)
    {
        await WriteDownloadAsync(uris.GuardianRuins, Path.Combine(stagePublished, "allRuins.json"), cancellationToken)
            .ConfigureAwait(false);
        await WriteDownloadAsync(
    ...
    )
    {
        return new PublishedReferenceVersions(
            ...
            plan.UpdateBiology ? remote.BiologyEngineVersion : previous.BiologyEngine,
            plan.UpdateGuardianTemplates
                ? Math.Max(previous.SettlementTemplate, remote.SettlementTemplateVersion)
                : previous.SettlementTemplate,
            plan.UpdateGuardian ? Math.Max(previous.Guardian, remote.GuardianVersion) : previous.Guardian,
            plan.UpdateSettlements ? Math.Max(previous.Settlements, remote.SettlementsVersion) : previous.Settlements,
            plan.UpdateNicknames ? Math.Max(previous.Nicknames, remote.NicknamesVersion) : previous.Nicknames,
            plan.UpdateGreenGasGiants
                ? Math.Max(previous.GreenGasGiants, remote.GreenGasGiantsVersion)
    ...
    )
    {
        ...
        if (
            ...
            || !Directory.EnumerateFileSystemEntries(rollbackRoot, "*", SearchOption.AllDirectories).Any()
        )
        {
        ...
        if (
            ...
            && !Directory.EnumerateFileSystemEntries(backupRoot, "*", SearchOption.AllDirectories).Any()
        )
        {
    ...
    )
    {
        ...
        try
        {
            ...
            checkpoint?.Invoke(PublishedReferenceUpdateCheckpoint.ExistingReferencesMoved);
    ...
    private static ActivationPaths CreateActivationPaths(string root, string stageRoot, string rollbackRoot)
    {
    ...
    private static void MoveExistingReferences(ActivationPaths paths, ActivationMoveState moved)
    {
    ...
    private static void ActivateCandidateReferences(ActivationPaths paths, ActivationMoveState moved)
    {
    ...
    private static void RollbackActivation(ActivationPaths paths, ActivationMoveState moved, string stageRoot)
    {
        MoveAwayIfActivated(
        ...
        RestoreIfMoved(moved.PublishedMoved, paths.RollbackPublished, paths.LivePublished, isDirectory: true);
        RestoreIfMoved(moved.CodexMoved, paths.RollbackCodex, paths.LiveCodex, isDirectory: false);
        RestoreIfMoved(
    ...
    private static void MoveAwayIfActivated(bool activated, string livePath, string failedPath, bool isDirectory)
    {
        if (!activated)
        {
            return;
    ...
    private static void RestoreIfMoved(bool moved, string rollbackPath, string livePath, bool isDirectory)
    {
        if (!moved)
        {
            return;
    ...
    private static bool NeedsUpdate(int currentVersion, int remoteVersion, ReferenceCatalogSource source)
    {
        return remoteVersion > currentVersion || !source.IsLocal;
    }

    private UpdatePlan EvaluateUpdatePlan(string root, PublishedReferenceVersions previous, PublishedDataIndex remote)
    {
        var sources = LegacyReferenceCatalogLoader
            .Load(root)
        ...
        return new UpdatePlan(
            ...
            NeedsUpdate(previous.Settlements, remote.SettlementsVersion, sources["human settlement templates"]),
            NeedsUpdate(previous.GreenGasGiants, remote.GreenGasGiantsVersion, sources["Green Gas Giant criteria"]),
            remote.NicknamesVersion > previous.Nicknames || SystemNicknameCatalog.Load(root).RavenCount == 0,
            warnings
        );
    }
    ...
    private static bool KnownSystemAddressesNeedUpdate(string root)
    {
        ...
        return !knownSystemAddresses.HasData || knownSystemAddresses.Warnings.Count > 0;
    }
    ...
    )
    {
        ...
        if (!updateBiology || remote.BiologyEngineVersion <= BiologyCriteriaCatalog.EngineVersion)
        {
    ...
    private static void ValidateCandidate(string candidateRoot, IReadOnlyCollection<string> updated)
    {
        ...
        var expectedSources = updated
            .SelectMany(name =>
                name switch
                {
                    "Guardian site indexes and surveys" => GuardianSiteSourceCatalogs,
                    _ => new[] { name },
                }
            )
    ...
    private static bool TryValidateSpecialCatalog(string candidateRoot, string catalogName)
    {
    ...
    private static void EnsureCatalogReady(bool hasData, IReadOnlyList<string> warnings, string emptyMessage)
    {
        ...
        throw new InvalidDataException(warnings.Count > 0 ? warnings[0] : emptyMessage);
    }
    ...
    private static void ValidateLocalCatalogSource(LegacyReferenceCatalogLoadResult result, string catalogName)
    {
        ...
        if (!source.IsLocal)
        {
            throw new InvalidDataException(source.Warning ?? $"The staged {catalogName} did not become active.");
        }
    }
    ...
    private static SortedDictionary<string, string> ParseNicknameMap(byte[] bytes)
    {
        try
        {
            ...
            {
                throw new InvalidDataException("The Raven nickname response is not a JSON array.");
            }
            ...
            if (result.Count == 0)
            {
                throw new InvalidDataException("The Raven nickname response contains no nicknames.");
            }
        ...
        {
            throw new InvalidDataException("The Raven nickname response is not valid JSON.", exception);
        }
    }
    ...
    private async Task<byte[]> DownloadAsync(Uri uri, CancellationToken cancellationToken)
    {
        ...
        if (output.Length == 0)
        {
            throw new InvalidDataException($"Published reference download was empty: {uri}");
        }

        return output.ToArray();
    }

    private static List<ArchiveEntryPayload> ValidateArchive(byte[] bytes, params string[] allowedExtensions)
    {
        using var stream = new MemoryStream(bytes, writable: false);
        using var archive = new ZipArchive(stream, ZipArchiveMode.Read);
        if (archive.Entries.Count > MaximumArchiveEntries)
        {
            throw new InvalidDataException("The published reference archive contains too many entries.");
        }
        ...
        if (result.Count == 0)
        {
            throw new InvalidDataException("The published reference archive contains no supported files.");
        }
    ...
    )
    {
        ...
        if (!allowedExtensions.Contains(Path.GetExtension(entry.Name), StringComparer.OrdinalIgnoreCase))
        {
    ...
    )
    {
        ...
        if (File.Exists(sourceRegionalCodex))
        {
            await CopyFileVerifiedAsync(
                    sourceRegionalCodex,
                    Path.Combine(destinationRoot, RegionalCodexCandidateCatalog.LegacyFileName),
                    cancellationToken
                )
                .ConfigureAwait(false);
        }
    ...
    )
    {
        ...
        foreach (var file in Directory.EnumerateFiles(sourceRoot, "*", SearchOption.AllDirectories))
        {
            ...
            {
                throw new InvalidDataException($"Published reference updates do not follow linked files: {file}");
            }
