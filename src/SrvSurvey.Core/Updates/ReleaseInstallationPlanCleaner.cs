...
{
    public static readonly TimeSpan DefaultMinimumAge = ReleaseInstallationHistoryCleaner.DefaultMinimumAge;

    private readonly TimeProvider timeProvider;
    private readonly TimeSpan minimumAge;
    private readonly Action<string> deleteDirectory;

    public ReleaseInstallationPlanCleaner(TimeProvider? timeProvider = null, TimeSpan? minimumAge = null)
        : this(timeProvider, minimumAge, path => Directory.Delete(path, recursive: true)) { }
    ...
    )
    {
        ...
        foreach (var candidate in candidates)
        {
            ...
            {
                if (
                    (candidate.Attributes & FileAttributes.ReparsePoint) != 0
                    || timeProvider.GetUtcNow() - candidate.LastWriteTimeUtc < minimumAge
                )
                {
                    retained++;
                    continue;
        ...
        return new ReleaseInstallationPlanCleanupResult(deleted, retained, failures);
    }
    ...
    private static DirectoryInfo[] FindCandidates(string root, List<string> failures)
    {
        try
        {
            ...
            {
                failures.Add($"{root}: the installation-plan root is a reparse point.");
                return [];
            }
