...
public static class ShipLockerFileReader
{
        ...
        for (var attempt = 1; attempt <= maximumAttempts; attempt++)
        {
            ...
            try
            {
                ...
                var items = Section("Items", data.Items)
                    ...
                    .GroupBy(item => (item.Category, item.Name), StringTupleComparer.OrdinalIgnoreCase)
                    .Select(group => new ShipLockerItem(
                        group.Key.Category,
                        group.Key.Name,
                        group
                            .Select(item => item.LocalizedName)
                ...
                when (exception is IOException or UnauthorizedAccessException or JsonException or OverflowException)
            {
    ...
    private static IEnumerable<ShipLockerItem> Section(string category, IReadOnlyList<ShipLockerItemData>? values)
    {
        return (values ?? [])
            .Where(item => !string.IsNullOrWhiteSpace(item.Name) && item.Count > 0)
            .Select(item => new ShipLockerItem(category, item.Name!, item.LocalizedName, item.Count));
    }
    ...
    private sealed class StringTupleComparer : IEqualityComparer<(string Category, string Name)>
    {
...
public sealed record ShipLockerItem(string Category, string Name, string? LocalizedName, int Count);
