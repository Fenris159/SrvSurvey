...
public sealed class SurfaceSurveyJournalTracker
{
    ...
    private static readonly Dictionary<string, string> GenusShortNames = new Dictionary<string, string>(
    ...
    public SurfaceSurveyJournalTracker(SystemSurfaceStore store, ExobiologyReferenceCatalog catalog)
    {
    ...
    )
    {
        ...
        foreach (var journalEvent in journalEvents)
        {
            ...
            try
            {
                ...
                {
                    "Touchdown" => await ApplyTouchdownAsync(session, journalEvent.Payload, cancellationToken)
                        .ConfigureAwait(false),
                    "Liftoff" or "ShipDismissed" => ApplyShipDeparted(),
                    "Disembark" => ApplyDisembark(session, journalEvent.Payload),
                    ...
                    or "Shutdown"
                    or "Died"
                    or "Resurrect" => ClearVehicleLocations(),
                    "Music"
                        when string.Equals(
                    ...
                    "ScanOrganic" => await ApplyOrganicScanAsync(
                        ...
                        .ConfigureAwait(false),
                    "SendText" => await ApplyBookmarkCommandAsync(session, journalEvent.Payload, cancellationToken)
                        .ConfigureAwait(false),
                    _ => 0,
                };
            }
            ...
            {
                warnings.Add($"{journalEvent.EventName} surface history was not saved: " + exception.Message);
            }
        }
        ...
        return new SurfaceSurveyJournalUpdateResult(mutationCount, warnings);
    }
    ...
    )
    {
        ...
        if (message == "---")
        {
            await store.ClearBookmarksAsync(context, cancellationToken).ConfigureAwait(false);
            return 1;
        }
        ...
        var result = message[0] switch
        {
            '+' => await store
                ...
                .ConfigureAwait(false),
            '-' => await store
                .RemoveBookmarkAsync(context, name, location, nearest: true, cancellationToken: cancellationToken)
                .ConfigureAwait(false),
            '=' => await store
                .RemoveBookmarkAsync(context, name, location, nearest: false, cancellationToken: cancellationToken)
                .ConfigureAwait(false),
            _ => null,
        };
        return result?.Mutation is SurfaceBookmarkMutation.Added or SurfaceBookmarkMutation.Removed ? 1 : 0;
    }
    ...
    )
    {
        ...
        return catalog
                .BiologyEntries.Select(ExobiologyReferenceCatalog.GetGenusName)
                .Distinct(StringComparer.Ordinal)
                .FirstOrDefault(genus =>
                    string.Equals(
    ...
    private static bool TryCreateBodyContext(SurfaceSurveySessionContext session, out SystemSurfaceContext context)
    {
        context = null!;
        if (
            session.BodyId is not { } bodyId
    ...
    private static bool TryGetStatusCoordinate(EliteStatus currentStatus, out SurfaceCoordinate coordinate)
    {
        coordinate = default;
        ...
        try
        {
            coordinate = new SurfaceCoordinate(currentStatus.Latitude, currentStatus.Longitude);
            return true;
        }
    ...
    {
        if (GetCoordinate(root) is not { } location || CreateBodyContext(session, root) is not { } context)
        {
            return 0;
        }
        ...
        HasShipDeparted = false;
        await store.SetLastTouchdownAsync(context, location, cancellationToken).ConfigureAwait(false);
        return 1;
    }
    ...
    private int ApplyDisembark(SurfaceSurveySessionContext session, JsonElement root)
    {
        ...
        if (session.NomadVehicleId is { } nomadVehicleId && GetInt64(root, "ID") == nomadVehicleId)
        {
    ...
    {
        if (ShipLocation is null && !HasShipDeparted && SrvLocation is null)
        {
            return 0;
        }
    ...
    {
        if (
            !options.AutoTrackCompositionScans
    ...
    )
    {
        ...
        else if (scanType == "Analyse")
        {
            var completed = new[] { sample, scanOne, scanTwo }
                ...
                .ToArray();
            await store.AppendBioScansAsync(context, completed, cancellationToken).ConfigureAwait(false);
            mutations += completed.Length;
            if (options.AutoRemoveTrackerOnFinalSample)
            {
                await store.RemoveBookmarkGroupAsync(context, genus, cancellationToken).ConfigureAwait(false);
                mutations++;
            }
    ...
    )
    {
        ...
        foreach (var sample in new[] { scanOne, scanTwo })
        {
            if (
                ...
                || !string.Equals(sample.Body, context.BodyName, StringComparison.OrdinalIgnoreCase)
            )
            {
                continue;
            }
    ...
    private static SurfaceBioScan ToSurfaceScan(BioSampleSnapshot sample, string bodyName)
    {
...
public sealed record SurfaceSurveyTrackingOptions(
    ...
    IReadOnlyDictionary<int, IReadOnlySet<string>>? AnalyzedSpeciesByBodyId = null
)
{
    public static SurfaceSurveyTrackingOptions Default { get; } =
        new(AutoRemoveTrackerOnSampling: true, AutoRemoveTrackerOnFinalSample: false);
}

public sealed record SurfaceSurveyJournalUpdateResult(int MutationCount, IReadOnlyList<string> Warnings);
