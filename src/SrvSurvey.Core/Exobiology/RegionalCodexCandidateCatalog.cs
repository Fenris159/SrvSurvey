...
public sealed record RegionalCodexCandidate(int RegionId, string RegionName, long EntryId, string Variant);
...
public sealed class RegionalCodexCandidateCatalog
{
    ...
    )
    {
        ...
        entryIdsByRegion = entries
            ...
            .ToDictionary(
                group => group.Key,
                group => (IReadOnlySet<long>)group.Select(entry => entry.EntryId).ToHashSet()
            );
    }

    public static RegionalCodexCandidateCatalog Empty { get; } = new([], null, []);
    ...
    public static RegionalCodexCandidateCatalog FromEntries(IEnumerable<RegionalCodexCandidate> entries)
    {
        ArgumentNullException.ThrowIfNull(entries);
        return Create(entries, null);
    }

    internal static RegionalCodexCandidateCatalog Load(Stream stream, string? sourcePath)
    {
        ArgumentNullException.ThrowIfNull(stream);
        using var document = JsonDocument.Parse(stream);
        if (document.RootElement.ValueKind != JsonValueKind.Object)
        {
            throw new InvalidDataException("The regional Codex candidate catalog is not a JSON object.");
        }
        ...
        foreach (var property in document.RootElement.EnumerateObject())
        {
            ...
            foreach (var item in property.Value.EnumerateArray())
            {
                ...
                candidates.Add(new RegionalCodexCandidate(region.Id, region.Name, entryId, variant));
                if (candidates.Count > MaximumEntries)
                {
                    throw new InvalidDataException("The regional Codex candidate catalog contains too many entries.");
                }
            }
        }
            ...
            .ToDictionary(
                ...
                group =>
                    group
                        ...
                        .Select(entry => entry.EntryId.ToString(CultureInfo.InvariantCulture) + "_" + entry.Variant)
                        .ToArray(),
    ...
    internal static RegionalCodexCandidateCatalog ParsePublishedCsv(byte[] bytes, ExobiologyReferenceCatalog references)
    {
    ...
    private static List<IReadOnlyList<string>> ReadPublishedCsvRows(byte[] bytes)
    {
        if (bytes.Length == 0)
        {
            throw new InvalidDataException("The published regional Codex candidate CSV is empty.");
        }
    ...
    {
        if (
            header.Count < PublishedCsvColumns.Length
            || !PublishedCsvColumns
                .Select(
                    (column, index) =>
                        string.Equals(header[index].TrimStart('\uFEFF'), column, StringComparison.Ordinal)
                )
                .All(matches => matches)
        )
        {
            throw new InvalidDataException("The published regional Codex candidate CSV header is incompatible.");
        }
    }
    ...
    )
    {
        ...
        if (!long.TryParse(row[5], NumberStyles.Integer, CultureInfo.InvariantCulture, out entryId) || entryId <= 0)
        {
            throw new InvalidDataException($"Published regional Codex row {rowIndex + 1:N0} has an invalid entry ID.");
        }
    ...
    )
    {
        ...
        if (variant.Length == 0)
        {
            throw new InvalidDataException($"Published regional Codex row {rowIndex + 1:N0} has no variant.");
        }

        return variant;
    }

    private static RegionalCodexCandidateCatalog Create(IEnumerable<RegionalCodexCandidate> entries, string? sourcePath)
    {
        var regionsById = GalacticRegionMap.Regions.ToDictionary(region => region.Id);
    ...
    private sealed class CsvRowParser(string text)
    {
        ...
        private void AddRow()
        {
            ...
            if (row.Any(value => value.Length > 0))
            {
                ...
                {
                    throw new InvalidDataException("The published regional Codex candidate CSV has too many rows.");
                }
            }
        ...
        private void AddField()
        {
            ...
            {
                throw new InvalidDataException("The published regional Codex candidate CSV has too many columns.");
            }
    ...
    private static bool ParseCsvBoolean(string value, int zeroBasedRowIndex, string column)
    {
    ...
    private static bool TryParseLegacyEntry(string? value, out long entryId, out string variant)
    {
        entryId = 0;
