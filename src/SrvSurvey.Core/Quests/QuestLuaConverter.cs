...
internal static class QuestLuaConverter
{
    ...
    private static JsonNode? ToJsonNode(LuaValue value)
    {
        return value.Type switch
        {
            ...
            _ => throw new InvalidDataException($"Lua value type '{value.TypeToString}' cannot be persisted."),
        };
    }
    ...
    private static JsonNode ToJsonTable(LuaTable table)
    {
        ...
        {
            throw new InvalidDataException("Lua tables must be either an array or a string-keyed map to be persisted.");
        }
        ...
        if (table.HashMapCount > 0)
        {
            ...
            foreach (var pair in table)
            {
                ...
                {
                    throw new InvalidDataException("Persisted Lua table map keys must be strings.");
                }
