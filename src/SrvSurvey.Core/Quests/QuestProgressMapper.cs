...
public static class QuestProgressMapper
{
    public static RavenCommanderQuest FromLegacy(LegacyQuestProgress progress)
    {
        ...
        return new RavenCommanderQuest
        {
            ...
            Quest =
                progress.PortableDefinition ?? (progress.Definition is null ? null : FromLegacy(progress.Definition)),
            ...
            Chapters = progress
                .Chapters.Select(chapter => new RavenQuestChapterState
                {
                    Id = chapter.Id,
                ...
                .ToList(),
            Messages = progress.Messages.Select(message => MapMessage(message, progress.Definition)).ToList(),
            Variables = CloneJsonMap(progress.Variables),
            KeptJournalEvents = CloneJsonMap(progress.KeptJournalEvents),
            Routes = progress
                .Routes.Select(route => new RavenQuestRoute
                {
                    Id = route.Id,
                    Width = route.Width,
    ...
    public static RavenQuestDefinition FromLegacy(LegacyQuestDefinition definition)
    {
        ...
        return new RavenQuestDefinition
        {
            ...
            OnlySquadrons = definition.OnlySquadrons.ToHashSet(StringComparer.Ordinal),
            OnlyCommanders = definition.OnlyCommanders.ToHashSet(StringComparer.Ordinal),
            Hidden = definition.Hidden,
            FirstChapter = definition.FirstChapter,
            Objectives = definition.Objectives.ToDictionary(StringComparer.Ordinal),
            Strings = definition.Strings.ToDictionary(StringComparer.Ordinal),
            Messages = definition
                .Messages.Select(message => new RavenQuestMessageDefinition
                {
                    Id = message.Id,
                    From = message.From,
                    Subject = message.Subject,
                    Body = message.Body,
                    Actions = message.Actions.Count == 0 ? null : message.Actions.ToDictionary(StringComparer.Ordinal),
                    Tags = message.Tags.Count == 0 ? null : message.Tags.ToHashSet(StringComparer.Ordinal),
                })
                .ToList(),
    ...
    private static RavenQuestMessage MapMessage(LegacyQuestMessage message, LegacyQuestDefinition? definition)
    {
        ...
        return new RavenQuestMessage
        {
            ...
            Actions = message.Actions.Count == 0 ? null : message.Actions.ToArray(),
            Read = message.Read,
