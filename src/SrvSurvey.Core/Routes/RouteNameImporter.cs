...
public sealed class RouteNameImporter(IStarSystemResolver resolver)
{
    ...
    )
    {
        ...
        for (var index = 0; index < normalizedNames.Length; index++)
        {
            ...
            progress?.Report(
                new RouteNameImportProgress(index + 1, normalizedNames.Length, requestedName, match is not null)
            );
        }

        return new RouteNameImportResult(hops, resolvedCount, hops.Count - resolvedCount);
    }
...
public sealed record RouteNameImportResult(IReadOnlyList<FollowRouteHop> Hops, int ResolvedCount, int UnresolvedCount);

public sealed record RouteNameImportProgress(int CompletedCount, int TotalCount, string SystemName, bool Resolved);
