...
internal sealed class EddnOutbox : IDisposable
{
    ...
    )
    {
        ...
        ArgumentOutOfRangeException.ThrowIfNegativeOrZero(maximumPendingMessages);
    ...
    internal bool hasExclusiveOwnership
    {
        get
        {
            lock (sync)
            {
                return ownershipLease is not null;
            }
        }
    }
    ...
    internal void setEnabled(bool value, bool discardPendingWhenDisabled)
    {
        lock (sharedConsentSync)
        {
            ...
            if (!value)
            {
                ...
                if (requestedStateChanged || !isSharedConsentDisabled())
                {
    ...
    private void applyEnabledState(bool value, bool discardPendingWhenDisabled)
    {
        ...
        lock (sync)
        {
            ...
            if (!enabled)
            {
                (cancellation, persistenceLog, sharingLog) = disableLocked(discardPendingWhenDisabled);
            }

            canSchedule = enabled && !suspended && ownershipLease is not null;
            shouldReleaseOwnership = !enabled && ownershipLease is not null;
        }
        ...
        if (canSchedule && (changed || acquiredOwnership) && automaticProcessing)
        {
    ...
    private (CancellationTokenSource Cancellation, string? PersistenceLog, string? SharingLog) disableLocked(
    ...
    internal bool enqueue(EddnQueuedMessage message, bool allowWhileSuspended = false)
    {
        ...
        if (automaticProcessing && !suspended)
        {
            schedule(TimeSpan.Zero);
        }
    ...
    {
        if (!await processing.WaitAsync(0, cancellationToken).ConfigureAwait(false))
        {
            return;
        }
    ...
    )
    {
        ...
        lock (sync)
        {
            if (!enabled || suspended || disposed || ownershipLease is null)
            {
                return false;
            }
    ...
    private bool ApplyUploadOutcome(EddnQueuedMessage next, EddnUploadResult? result, Exception? failure, bool retry)
    {
        string? persistenceLog = null;
        ...
        lock (sync)
        {
            ...
            {
                (persistenceLog, resultLog) = CompleteUploadLocked(next, result, reloadLogs);
            }
        }
    ...
    )
    {
        ...
        if (pending.Count == 0)
        {
            if (loadingTruncated)
            {
                pending = load(reloadLogs, continueTruncatedLoad: true);
            }
            else
            {
    ...
    private void triggerProcessing()
    {
        ...
        processDue(shutdown.Token)
            .ContinueWith(
                task =>
                    writeLog($"EDDN queue processing failed: {singleLine(task.Exception?.GetBaseException().Message)}"),
    ...
    {
        if (disposed || !automaticProcessing)
        {
            return;
        }
    ...
    {
        if (!enabled || suspended || ownershipLease is null || pending.Count == 0)
        {
            stopTimer();
            return;
        }
    ...
    private List<EddnQueuedMessage> load(List<string> messages, bool continueTruncatedLoad = false)
    {
    ...
    private List<EddnQueuedMessage> loadMessageFiles(List<string> messages, HashSet<Guid> ids)
    {
        ...
        foreach (var path in Directory.EnumerateFiles(storeFolder, "*.json"))
        {
            ...
            try
            {
                ...
                when (exception is IOException or JsonException or UnauthorizedAccessException or InvalidDataException)
            {
                quarantine(path, messages);
                messages.Add("EDDN could not load a pending upload: " + exception.Message);
            }
        }
    ...
    private void migrateLegacyStore(List<EddnQueuedMessage> loaded, List<string> messages)
    {
            ...
            when (exception is IOException or JsonException or UnauthorizedAccessException or InvalidDataException)
        {
            ...
            messages.Add("EDDN could not load its legacy pending uploads: " + exception.Message);
        }
    }
    ...
    )
    {
        ...
        foreach (var item in legacy)
        {
            ...
            if (!isValid(item))
            {
                throw new InvalidDataException("the queue contained invalid entries");
            }
    ...
    private bool persistMessage(EddnQueuedMessage message, out string? errorLog)
    {
        ...
        {
            errorLog = "EDDN could not persist a pending upload: " + exception.Message;
            return false;
        }
    }
    ...
    private string? deleteMessage(EddnQueuedMessage message)
    {
        ...
        {
            return "EDDN could not remove a delivered upload: " + exception.Message;
        }
    }
    ...
    private static void quarantine(string path, List<string> messages)
    {
        ...
        {
            messages.Add("EDDN could not preserve an unreadable queue entry: " + exception.Message);
        }
    }
    ...
    private static void addLog(List<string> messages, string? message)
    {
        if (!string.IsNullOrWhiteSpace(message))
        {
    ...
    private void tryAcquireOwnershipLocked(List<string> messages)
    {
        ...
        try
        {
            ...
            ownershipLease = new FileStream(ownershipPath, FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.None);
            pending = load(messages);
            if (ownershipWarningReported)
            {
                messages.Add("EDDN acquired the local outbox after the other SrvSurvey instance released it.");
        ...
        catch (Exception exception) when (exception is IOException or UnauthorizedAccessException)
        {
            if (!ownershipWarningReported)
            {
                messages.Add("EDDN uploads are paused because another SrvSurvey instance owns the local outbox.");
    ...
    private static bool isValid(EddnQueuedMessage? message)
    {
        return message is not null
            ...
            && message.schemaRef.StartsWith("https://eddn.edcd.io/schemas/", StringComparison.Ordinal)
            && message.header is not null
            && !string.IsNullOrWhiteSpace(message.header.uploaderID)
            && message.message is not null
            && !string.IsNullOrWhiteSpace(message.message.Value<string>("event"));
    }
    ...
    private FileSystemWatcher? createSharedConsentWatcher(List<string> messages)
    {
        try
        {
            ...
            Directory.CreateDirectory(folder);
            var watcher = new FileSystemWatcher(folder, Path.GetFileName(sharedDisablePath))
            {
        ...
        {
            messages.Add("EDDN could not monitor shared opt-out state: " + exception.Message);
            return null;
        }
    }
    ...
    private string? writeSharedDisableMarker()
    {
        try
        {
            ...
            File.WriteAllText(sharedDisablePath, "EDDN sharing is disabled for all SrvSurvey instances.");
            return null;
        }
        catch (Exception exception) when (exception is IOException or UnauthorizedAccessException)
        {
            return "EDDN could not publish the shared opt-out state: " + exception.Message;
        }
    }

    private string? acquireSharedDisableLease()
    {
        if (sharedDisableLease is not null)
        {
            return null;
        }
        ...
        {
            return "EDDN could not acquire the shared opt-out lease: " + exception.Message;
        }
    }
    ...
    private string? tryClearSharedDisableMarker()
    {
        ...
        {
            return "EDDN could not inspect the shared opt-out state: " + exception.Message;
        }
        ...
        using (activeOptOutProbe)
        {
            ...
            {
                return "EDDN could not clear the shared opt-out state: " + exception.Message;
            }
        }
    }
