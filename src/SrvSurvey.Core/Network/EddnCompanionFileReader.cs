...
internal static class EddnCompanionFileReader
{
    ...
    )
    {
        ...
        return new EddnCompanionReadResult(null, lastError ?? $"{eventName}.json could not be read");
    }
    ...
    )
    {
        try
        {
            return await tryReadCompanionFile(filepath, eventName, journalEvent, cancellationToken)
                .ConfigureAwait(false);
        }
        catch (Exception ex) when (ex is IOException or JsonException)
        {
            return new EddnCompanionReadResult(null, $"{eventName}.json could not be read: {ex.Message}");
        }
    }
    ...
    )
    {
        ...
        {
            return new EddnCompanionReadResult(null, $"{eventName}.json was not found");
        }
        ...
        {
            return new EddnCompanionReadResult(null, $"{eventName}.json did not match the event's MarketID");
        }

        if (!isCurrent(journalEvent, content))
        {
            return new EddnCompanionReadResult(null, $"{eventName}.json was older than the journal event");
        }
