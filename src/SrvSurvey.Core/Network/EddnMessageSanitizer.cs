...
internal sealed record EddnLocationContext(string systemName, long systemAddress, double[] starPosition);
...
internal sealed record EddnPreparedMessage(string eventName, string schemaRef, JObject message);
...
internal static class EddnMessageSanitizer
{
    ...
    private const string TheEventDidNotMatchTrackedSystemMessage = "the event did not match the tracked system";
    ...
    private static readonly string[] CommodityStatusFlags = ["Producer", "Consumer", "Rare"];
    ...
    internal static EddnLocationContext? getLocation(JObject raw)
    {
        ...
        return new EddnLocationContext(systemName, systemAddress.Value, position.Values<double>().ToArray());
    }
    ...
    )
    {
        ...
        return finalizeJournalMessage(eventName, schema, message!, out prepared, out reason);
    }
    ...
    private static bool failUnsupportedEvent(out JObject? message, out string schema, out string reason)
    {
        message = null;
        schema = string.Empty;
    ...
    )
    {
        ...
        removeNulls(message);
        if (eventName == CodexEntryEvent && !hasValidCodexStrings(message, out reason))
        {
            return false;
        }
    ...
    )
    {
        ...
        message = select(
            ...
            LongitudeProperty,
            SubCategoryProperty,
            NearestDestinationProperty,
            VoucherAmountProperty,
            TraitsProperty,
            BodyIdProperty,
            BodyNameProperty
        );
    ...
    private static bool BodyNamesAgree(EddnMessageContext context)
    {
        return !string.IsNullOrWhiteSpace(context.statusBodyName)
            && string.Equals(context.statusBodyName, context.trackedBodyName, StringComparison.Ordinal);
    }

    private static void TryFillCodexBodyName(JObject message, EddnMessageContext context, bool bodyNamesAgree)
    {
        if (
            message.ContainsKey(BodyNameProperty)
            || !bodyNamesAgree
            || (message.ContainsKey(BodyIdProperty) && message.Value<int?>(BodyIdProperty) != context.trackedBodyId)
        )
        {
            return;
        }
    ...
    private static void TryFillCodexBodyId(JObject message, EddnMessageContext context, bool bodyNamesAgree)
    {
        if (
    ...
    )
    {
        ...
        message = select(raw, TimestampKey, EventKey, SystemNameProperty, SystemAddressProperty, CountProperty);
        message[StarPosProperty] = position(context.location!);
        addFlags(message, context);
        reason = string.Empty;
        return true;
    ...
    )
    {
        ...
        message = select(raw, TimestampKey, EventKey, SystemAddressProperty, NumBodiesProperty);
        message[StarSystemProperty] = context.location!.systemName;
        message[StarPosProperty] = position(context.location);
        addFlags(message, context);
        reason = string.Empty;
    ...
    )
    {
        ...
        prepared = new EddnPreparedMessage("FSSSignalDiscovered", schemaRoot + "fsssignaldiscovered/1", message);
        reason = string.Empty;
        return true;
    }
    ...
    )
    {
        ...
        switch (eventName)
        {
            case DockedEvent:
                ...
                if (
                    !message.ContainsKey(BodyProperty)
        ...
        prepared = new EddnPreparedMessage(eventName, schemaRoot + "journal/1", message);
        reason = string.Empty;
        return true;
    }
        ...
        foreach (var item in source[ItemsKey] as JArray ?? [])
        {
            if (
                ...
                || commodity
                    ...
                    ?.Contains("NonMarketable", StringComparison.OrdinalIgnoreCase) == true
            ...
            if (RequiredCommodityFields.Any(field => !hasValue(output, field)))
            {
                continue;
            }
            ...
            {
                output["statusFlags"] = statusFlags;
            }

            commodities.Add(output);
        }

        var sorted = new JArray(commodities.OrderBy(item => item.Value<string>("name"), StringComparer.Ordinal));
        var message = new JObject
        {
    ...
    private static JObject buildOutfitting(JObject source, EddnMessageContext context)
    {
        ...
        foreach (var item in source[ItemsKey] as JArray ?? [])
        {
            ...
            if (
                ...
                || (!string.IsNullOrWhiteSpace(sku) && !sku.Equals(horizonsSku, StringComparison.Ordinal))
            )
            {
                continue;
            }
        ...
        addFlags(message, context with { horizons = source.Value<bool?>("Horizons") ?? context.horizons });
        removeNulls(message);
        return message;
    }
    ...
    private static bool hasRequiredFields(JObject message, string eventName, out string reason)
    {
        string[] required = eventName switch
        {
            ...
            _ when genericEvents.Contains(eventName) =>
            [
                TimestampKey,
                EventKey,
                StarSystemProperty,
        ...
        if (eventName is OutfittingEvent or ShipyardEvent && message[required[^1]] is JArray array && array.Count == 0)
        {
    ...
    private static bool hasValidCodexStrings(JObject message, out string reason)
    {
        ...
        if (
            message["Traits"] is JArray traits
            && traits.Any(value => value.Type != JTokenType.String || string.IsNullOrWhiteSpace(value.Value<string>()))
        )
        {
    ...
    {
        if (context.horizons.HasValue)
        {
            message["horizons"] = context.horizons.Value;
        }

        if (context.odyssey.HasValue)
        {
            message["odyssey"] = context.odyssey.Value;
        }
    }
    ...
    {
        if (message["Factions"] is not JArray factions)
        {
            return;
        }
