...
internal readonly record struct OrganismIdentity(string? Genus, long? EntryId, string? Variant, string? Species);
...
internal static class OrganismIdentityMatcher
{
        ...
        where T : class
    {
        ...
        if (
            ...
            && FindBy(materialized, identitySelector, identity => identity.EntryId == incoming.EntryId)
                is { } entryMatch
        )
        {
            return entryMatch;
        ...
        if (
            !string.IsNullOrWhiteSpace(incoming.Variant)
            && FindBy(
                materialized,
                identitySelector,
                identity => string.Equals(identity.Variant, incoming.Variant, StringComparison.Ordinal)
            )
                is { } variantMatch
        )
        {
        ...
        if (
            ...
            && FindBy(
                sameGenus,
                identitySelector,
                identity => string.Equals(identity.Species, incoming.Species, StringComparison.Ordinal)
            )
                is { } speciesMatch
        )
        {
        ...
        var placeholder = FindBy(
            ...
            identity =>
                ...
                && (
                    ...
                    || string.Equals(identity.Species, incoming.Species, StringComparison.Ordinal)
                )
        );
    ...
    private static T? FindBy<T>(
        ...
        where T : class => candidates.FirstOrDefault(candidate => predicate(identitySelector(candidate)));
