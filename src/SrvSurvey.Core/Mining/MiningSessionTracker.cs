...
public sealed record MiningProspect(
    DateTimeOffset Time,
    IReadOnlyList<MiningMaterial> Materials,
    string Core,
    string Content
)
{
...
public sealed record MiningSession
{
    ...
    public double RefinedTons =>
        (Imported?.Tons ?? 0) + Collections.Where(item => !item.Engineering).Sum(item => item.Count);
    public int EngineeringCount =>
        (Imported?.Engineering ?? 0) + Collections.Where(item => item.Engineering).Sum(item => item.Count);
    ...
    public TimeSpan DurationAt(DateTimeOffset now) =>
        TimeSpan.FromTicks(Math.Max(0, ((PausedAt ?? Ended ?? now) - Started - PausedDuration).Ticks));

    public IReadOnlyList<MiningMaterialSummary> Summarize(IReadOnlyDictionary<string, double> thresholds) =>
        Prospects
            .SelectMany(MaterialFinds)
            .GroupBy(m => m.Name, StringComparer.OrdinalIgnoreCase)
            .Select(group => new MiningMaterialSummary(
                group.Key,
                group.Count(),
                Math.Clamp(
                    group.Count(m =>
                        ...
                        || m.Percentage
                            >= thresholds
                                .FirstOrDefault(p => p.Key.Equals(group.Key, StringComparison.OrdinalIgnoreCase))
                                .Value
                    ) + QualityAdjustments.GetValueOrDefault(group.Key.ToLowerInvariant()),
                    0,
                    group.Count()
                ),
                ...
                Collections
                    .Where(c => !c.Engineering && c.Name.Equals(group.Key, StringComparison.OrdinalIgnoreCase))
                    .Sum(c => c.Count)
            ))
    ...
    private static IEnumerable<MaterialFind> MaterialFinds(MiningProspect prospect)
    {
        foreach (var material in prospect.Materials)
        {
            yield return new(
                material.Name,
                material.Percentage,
                material.Name.Equals(prospect.Core, StringComparison.OrdinalIgnoreCase)
            );
        }

        if (
            !string.IsNullOrEmpty(prospect.Core)
            && !prospect.Materials.Any(m => m.Name.Equals(prospect.Core, StringComparison.OrdinalIgnoreCase))
        )
        {
...
public sealed record MiningMaterialSummary(
    string Name,
    int Finds,
    int QualityHits,
    double Average,
...
public sealed class MiningSessionTracker
{
    ...
    {
        if (Current is { PausedAt: null } session)
        {
    ...
    {
        if (Current is not { PausedAt: { } paused } session)
        {
            return;
        }
    ...
    {
        if (Current is not { PausedAt: null } session || entry.Timestamp is not { } time || time < session.Started)
        {
            return false;
        }
        ...
        return entry.EventName switch
        {
            ...
            "StartJump"
                when MiningJson.Text(data, "JumpType").Equals("Hyperspace", StringComparison.OrdinalIgnoreCase) =>
                ReleaseProspect(session),
            _ => false,
        };
    }
    ...
    private static bool ApplyDrone(MiningSession session, JsonElement data)
    {
        ...
        else if (type.Equals("Collection", StringComparison.OrdinalIgnoreCase))
        {
            session.CollectorLimpets++;
        }
        else
        {
            return false;
    ...
    private static bool ApplyProspect(MiningSession session, DateTimeOffset time, JsonElement data)
    {
        var materials = MiningJson
            ...
            .Select(material => new MiningMaterial(
                ...
                MiningJson.Number(material, "Proportion")
            ))
        ...
        var remaining =
            ...
            && double.IsFinite(parsedRemaining)
                ...
                : 100;
        var prospect = new MiningProspect(
            time,
            materials,
            MiningJson.Text(data, "MotherlodeMaterial"),
            MiningJson.Text(data, "Content")
        )
        {
            Remaining = remaining,
        };
    ...
    {
        if (!MiningJson.Text(data, "Category").Equals("Raw", StringComparison.OrdinalIgnoreCase))
        {
            return false;
        }
