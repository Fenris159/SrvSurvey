...
public sealed class MiningWorkspaceState
{
    ...
    {
        if (!RelevantEvents.Contains(entry.EventName))
        {
            return false;
        }
        ...
        if (!bootstrap && Data.Settings.AutoStart)
        {
    ...
    {
        if (
            entry.Timestamp is not { } time
            || entry.EventName != "LaunchDrone"
            || !MiningJson.Text(entry.Payload, "Type").Equals("Prospector", StringComparison.OrdinalIgnoreCase)
        )
        {
            return;
        }
    ...
    public void CacheRing(MiningRing ring)
    {
        ...
        if (existing is not null && existing.Scanned > ring.Scanned)
        {
            return;
        }
    ...
    private void AddNotice(JournalEventEnvelope entry)
    {
        ...
        string? text = entry.EventName switch
        {
            "MiningRefined" when settings.NotifyRefined =>
                $"{(MiningJson.Text(json, "Type_Localised") is { Length: > 0 } localized ? localized : MiningJson.Text(json, "Type"))} ×1",
            "MaterialCollected" when settings.NotifyMaterials =>
                $"Collected: {MiningJson.Text(json, "Name")} ×{MiningJson.Number(json, "Count")}",
    ...
    private string? FormatProspect()
    {
        ...
        var selected = last
            ...
            .ToArray();
        if (selected.Length == 0 && string.IsNullOrEmpty(last.Core))
        {
            return null;
        }
    ...
    private string? FormatCurrentProspect()
    {
        ...
        if (!string.IsNullOrEmpty(prospect.Core))
        {
            parts.Add($"Core: {prospect.Core}");
        }
    ...
    private void ApplyRing(JournalEventEnvelope entry, string system, GalacticCoordinate? position)
    {
        ...
        if (entry.EventName != "SAASignalsFound")
        {
            return;
        }
