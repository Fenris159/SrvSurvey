...
public sealed class MiningMissionTracker
{
    ...
    public bool Apply(JournalEventEnvelope entry)
    {
        ...
        if (entry.EventName == "MissionAccepted")
        {
            if (
                ...
                || !MiningJson.Text(data, "Name").Contains("Mining", StringComparison.OrdinalIgnoreCase)
            )
            {
                return false;
            }
            ...
            Missions.Add(
                new MiningMission
                {
                    ...
                    Destination =
                        MiningJson.Text(data, "DestinationSystem")
                        + " / "
                        + MiningJson.Text(data, "DestinationStation"),
                }
            );
            return true;
        }
        ...
        foreach (var mission in Missions)
        {
            mission.OnBoard =
                mission.Status == "Active"
                    ? Math.Min(
                        Math.Max(0, mission.Required - mission.Delivered),
                        available.GetValueOrDefault(mission.Commodity)
                    )
                    : 0;
    ...
    private static string NormalizeCommodity(string name) =>
        name.Trim().TrimStart('$').Replace("_name;", "", StringComparison.OrdinalIgnoreCase).ToLowerInvariant();
}
