...
public static class MiningReport
{
        ...
        foreach (var s in sessions)
        {
            text.AppendLine(
                string.Join(
                    ...
                    new[]
                    {
                        s.Started.ToString("O"),
                        s.System,
                        s.Ring,
                        s.Ship,
                        s.ActiveDuration.TotalMinutes.ToString("0.00", CultureInfo.InvariantCulture),
                        s.RefinedTons.ToString(CultureInfo.InvariantCulture),
                        s.TonsPerHour.ToString("0.00", CultureInfo.InvariantCulture),
                        s.Asteroids.ToString(CultureInfo.InvariantCulture),
                        s.CoreHits.ToString(CultureInfo.InvariantCulture),
                        s.ProspectorLimpets.ToString(CultureInfo.InvariantCulture),
                        s.CollectorLimpets.ToString(CultureInfo.InvariantCulture),
                        s.EngineeringCount.ToString(CultureInfo.InvariantCulture),
                        s.Notes,
                    }.Select(Cell)
                )
            );
    ...
    public static string Html(IReadOnlyList<MiningSession> sessions)
    {
        ...
        text.Append(
            ...
            $"<div class='summary'><div><div class='metric'>{sessions.Count}</div>sessions</div><div><div class='metric'>{sessions.Sum(s => s.RefinedTons):N0} t</div>refined</div><div><div class='metric'>{sessions.Sum(s => s.ActiveDuration.TotalHours):0.0} h</div>active time</div></div>"
        );
        if (sessions.Count > 1)
        {
        ...
        return text.Append(
                "<footer><small>Journal observations describe events recorded during the session. Refining is measured separately from cargo transfers and purchases.</small></footer></body></html>"
            )
            .ToString();
    }
    ...
    private static void AppendSession(StringBuilder text, MiningSession s)
    {
        ...
        text.Append(
            CultureInfo.InvariantCulture,
            $"<p class='metric'>{s.RefinedTons} t · {s.TonsPerHour:0.0} t/h · {s.ActiveDuration:hh\\:mm\\:ss}</p><p>{s.Asteroids} asteroids · {s.CoreHits} cores · {s.TonsPerAsteroid:0.00} t/asteroid · {s.ProspectorLimpets} prospectors · {s.CollectorLimpets} collectors</p>"
        );
        if (s.Notes.Length > 0)
        {
    ...
    private static void AppendObservations(StringBuilder text, MiningSession s)
    {
        ...
        foreach (var material in s.Summarize(s.Thresholds))
        {
            text.Append(
                CultureInfo.InvariantCulture,
                $"<tr><td>{H(material.Name)}</td><td>{material.Finds}</td><td>{material.QualityHits}</td><td>{material.Average:0.0}</td><td>{material.Best:0.0}</td></tr>"
            );
        }
    ...
    private static void Bar(StringBuilder text, string label, double value, double maximum, string unit) =>
        text.Append(
            CultureInfo.InvariantCulture,
            $"<div class='bar'><label>{H(label)}</label><progress max='{maximum:0.00}' value='{value:0.00}'></progress><span>{value:0.0} {unit}</span></div>"
        );
    ...
    private static void AppendScreenshot(StringBuilder text, string path)
    {
        ...
        if (mime is null || new FileInfo(path).Length > 20 * 1024 * 1024)
        {
            return;
        }
