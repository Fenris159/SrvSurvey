...
public sealed record MiningImportedReport(
    double Tons,
    int Asteroids,
    int Cores,
    int Engineering,
...
public static class MiningReportImport
{
    ...
    private static MiningSession ReadRow(string[] header, string[] row)
    {
        ...
        {
            throw new JsonException("Invalid session duration.");
        }

        var id = new Guid(
            SHA256.HashData(
                Encoding.UTF8.GetBytes(
                    string.Join("|", started.ToString("O"), Get("system", "System"), Get("body", "Ring"))
                )
            )[..16]
        );
        return new MiningSession
        {
            ...
            Imported = new(
                Number("total_tons", "Tons"),
                (int)Number("asteroids_prospected", "Asteroids"),
                (int)Number("", "CoreHits"),
                (int)Number("", "EngineeringMaterials"),
                fields
            ),
        };
    }
