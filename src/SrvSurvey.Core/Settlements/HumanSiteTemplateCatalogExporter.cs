...
public sealed class HumanSiteTemplateCatalogExporter
{
    ...
    internal HumanSiteTemplateCatalogExporter(Func<string, Task> beforeActivation)
    {
        this.beforeActivation = beforeActivation ?? throw new ArgumentNullException(nameof(beforeActivation));
    }
    ...
    )
    {
        ...
        try
        {
            return await ExportLockedAsync(catalog, targetPath, cancellationToken).ConfigureAwait(false);
        }
        finally
        {
            pathLock.Release();
    ...
    )
    {
        ...
        try
        {
            ...
            await ValidateFileAsync(temporaryPath, expectedHash, catalog.Count, cancellationToken)
                .ConfigureAwait(false);
            EnsureUnchanged(original, await CaptureAsync(targetPath, cancellationToken).ConfigureAwait(false));
            ...
            activated = true;
            await ValidateFileAsync(targetPath, expectedHash, catalog.Count, cancellationToken).ConfigureAwait(false);
        ...
        catch
        {
            if (activated)
            {
                await RestoreAsync(targetPath, backupPath, original, cancellationToken).ConfigureAwait(false);
            }
    ...
    private static byte[] Serialize(IReadOnlyList<HumanSiteTemplate> templates)
    {
        ...
        var rows = templates
            .Select(template => new TemplateRow(
                ...
                template.SubType,
                template.Name,
                template.LandingPads.Select(point => ToRow(point, size: point.Size.ToString())).ToArray(),
                template.SecureDoors.Select(point => ToRow(point)).ToArray(),
                template.NamedPoints.Select(point => ToRow(point, name: point.Name)).ToArray(),
                template.DataTerminals.Select(point => ToRow(point)).ToArray(),
                template.ConflictZonePoints.Select(point => ToRow(point)).ToArray(),
                template
                    .Buildings.Select(building => new BuildingRow(
                        building.Name,
                        building
                            .Paths.Select(path => new PathRow(
    ...
    private static PoiRow ToRow(HumanSitePointOfInterest point, string? name = null, string? size = null)
    {
        return new PoiRow(ToRow(point.Offset), point.Rotation, point.SecurityLevel, point.Floor, name, size);
    }
    ...
    private static async Task WriteNewFileAsync(string path, byte[] payload, CancellationToken cancellationToken)
    {
    ...
    )
    {
        ...
        {
            throw new InvalidDataException("The settlement template export did not retain every catalog entry.");
        }
    }

    private static async Task<FileFingerprint?> CaptureAsync(string path, CancellationToken cancellationToken)
    {
        if (!File.Exists(path))
        {
            return null;
    ...
    private static void EnsureUnchanged(FileFingerprint? expected, FileFingerprint? actual)
    {
        if (expected != actual)
        {
    ...
    )
    {
        ...
        {
            throw new IOException("The settlement template export failed and its verified backup is unavailable.");
        }
    ...
    private static void RejectReparsePoint(string path)
    {
        ...
        {
            throw new IOException("Settlement template exports cannot replace a symbolic link or reparse point.");
        }
    }
