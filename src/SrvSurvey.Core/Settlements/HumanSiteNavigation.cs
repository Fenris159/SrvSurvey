...
public sealed class HumanSiteNavigation(HumanSiteTemplateCatalog templates)
{
    ...
    private static void ValidateRadius(double bodyRadius)
    {
        ...
        {
            throw new ArgumentOutOfRangeException(nameof(bodyRadius), "The body radius must be positive.");
        }
    }
...
{
    private static readonly ConcurrentDictionary<string, HumanSiteMapPoint> Offsets = new(
    ...
    {
        return string.IsNullOrWhiteSpace(vehicle) ? default : Offsets.GetValueOrDefault(vehicle);
    }
    ...
    public static void Set(string vehicle, HumanSiteMapPoint offset)
    {
        ...
        if (!offset.IsFinite)
        {
            throw new ArgumentOutOfRangeException(nameof(offset), "The vehicle offset must be finite.");
        }
