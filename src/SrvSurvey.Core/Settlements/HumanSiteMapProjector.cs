...
public sealed class HumanSiteMapProjector
{
    ...
    public HumanSiteMapProjection Project(HumanSiteTemplate template, HumanSiteMapDisplayOptions? options = null)
    {
        ...
        var namedPoints = template
            ...
            .Where(point =>
                options.ShowBatteries || !string.Equals(point.Name, "Battery", StringComparison.OrdinalIgnoreCase)
            )
    ...
    private static HumanSiteProjectedBuilding ProjectBuilding(HumanSiteBuilding building)
    {
        return new HumanSiteProjectedBuilding(building.Name, building.Paths.Select(ProjectPath).ToArray());
    }
    ...
    private static HumanSiteProjectedPath ProjectPath(HumanSiteBuildingPath path)
    {
        ...
        while (index < path.Points.Count)
        {
            ...
            {
                segments.Add(
                    new HumanSitePathSegment(HumanSitePathSegmentKind.Move, path.Points[index], default, default)
                );
            }
            else if (baseType == LinePoint)
            {
                segments.Add(
                    new HumanSitePathSegment(HumanSitePathSegmentKind.Line, path.Points[index], default, default)
                );
            }
            else if (baseType == BezierPoint)
            {
                if (
                    index + 2 >= path.Points.Count
                    || (path.PointTypes[index + 1] & PathTypeMask) != BezierPoint
                    || (path.PointTypes[index + 2] & PathTypeMask) != BezierPoint
                )
                {
                    throw new InvalidDataException("A human settlement building has an incomplete Bézier segment.");
                }

                segments.Add(
                    new HumanSitePathSegment(
            ...
            {
                throw new InvalidDataException($"Unknown human settlement path-point type '{baseType}'.");
            }

            if ((type & CloseSubpath) != 0)
            {
                segments.Add(new HumanSitePathSegment(HumanSitePathSegmentKind.Close, default, default, default));
            }
        ...
        return new HumanSiteProjectedPath(
            path.FillMode == 1 ? HumanSitePathFillRule.NonZero : HumanSitePathFillRule.EvenOdd,
            segments
        );
    }
    ...
    private static HumanSiteProjectedPoint? ProjectNamedPoint(HumanSiteNamedPointOfInterest point, ref int skipped)
    {
        if (!point.Offset.IsPlausibleMapOffset())
        {
            skipped++;
    ...
    private static IEnumerable<HumanSiteMapPoint> GetSegmentPoints(HumanSitePathSegment segment)
    {
        return segment.Kind switch
        {
            HumanSitePathSegmentKind.Move or HumanSitePathSegmentKind.Line => [segment.First],
            HumanSitePathSegmentKind.CubicBezier => [segment.First, segment.Second, segment.Third],
            _ => [],
        };
    }
}
...
{
    public static HumanSiteMapDisplayOptions Default { get; } = new(true, true, true, false);
}
...
public sealed record HumanSiteProjectedBuilding(string Name, IReadOnlyList<HumanSiteProjectedPath> Paths);
