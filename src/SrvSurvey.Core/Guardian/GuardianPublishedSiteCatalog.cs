...
{
    private const string EmbeddedResourceName = "SrvSurvey.Core.Resources.guardian.zip";
    ...
    public GuardianPublishedSiteCatalog(IEnumerable<GuardianPublishedSite> sites)
    {
        ...
        itemCodesByLog = allSites
            ...
            .ToDictionary(
                group => group.Key,
                group =>
                    group.Select(obelisk => obelisk.ItemCodes).FirstOrDefault(itemCodes => itemCodes.Count > 0)
                        is { } itemCodes
                        ? itemCodes.ToArray()
                        : [],
                StringComparer.OrdinalIgnoreCase
    ...
    public GuardianPublishedSite? Find(GuardianSiteReference reference)
    {
        ...
        return sites.GetValueOrDefault(GetIdentity(reference.Kind, reference.FullBodyName, reference.Index));
    }
    ...
    public GuardianPublishedSite? Find(GuardianSiteKind kind, string fullBodyName, int index)
    {
    ...
    private static string GetIdentity(GuardianSiteKind kind, string fullBodyName, int index)
    {
    ...
    private static Dictionary<string, GuardianPoiStatus> ReadStatuses(JsonElement root)
    {
    ...
    private static GuardianObelisk[] ReadObelisks(JsonElement root)
    {
        ...
        {
            throw new InvalidDataException("Guardian active obelisks are not an array.");
        }

        return value.EnumerateArray().Select(element => ParseObelisk(element.GetString())).ToArray();
    }
    ...
    private static string GetRequiredString(JsonElement root, string propertyName)
    {
        return GetString(root, propertyName) is { Length: > 0 } value
            ? value
            : throw new InvalidDataException($"A Guardian published survey is missing {propertyName}.");
    }
...
public sealed record GuardianObelisk(string Name, string LogCode, bool Scanned, IReadOnlyList<string> ItemCodes);

public readonly record struct GuardianSurfaceLocation(double Latitude, double Longitude);
