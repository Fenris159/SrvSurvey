...
public sealed class GuardianCommanderDataReader
{
    ...
    public GuardianCommanderDataReader(string dataDirectory, GuardianPublishedSiteCatalog? publishedSites = null)
    {
    ...
    )
    {
        ...
        foreach (var path in Directory.EnumerateFiles(folder, "*.json", SearchOption.TopDirectoryOnly))
        {
            ...
            if (filename.EndsWith("-beacon.json", StringComparison.OrdinalIgnoreCase))
            {
                ...
                filename.Contains("-ruins-", StringComparison.OrdinalIgnoreCase)
        ...
        return new GuardianCommanderDataReadResult(
            surveys
                .OrderBy(survey => survey.SystemName)
                .ThenBy(survey => survey.BodyName)
                .ThenBy(survey => survey.Index)
                .ToArray(),
            beacons.OrderBy(beacon => beacon.SystemName).ThenBy(beacon => beacon.BodyName).ToArray(),
            errors
        );
    }
        ...
        using (document)
        {
            try
            {
                ...
                return new GuardianCommanderSiteSurvey(
                    ...
                    GetDateTimeOffset(root, "firstVisited") ?? DateTimeOffset.MinValue,
                    GetDateTimeOffset(root, "lastVisited") ?? DateTimeOffset.MinValue,
                    ...
                    new GuardianSurveyData
                    {
                        ...
                        RelicTowerHeading = GetInt32(root, "relicTowerHeading") ?? -1,
                ...
                )
                {
                    ...
                    DistanceToArrivalLs = GetDouble(root, "distanceToArrival"),
    ...
    private static Dictionary<string, GuardianPoiStatus> ReadPoiStatuses(JsonElement root)
    {
        ...
        if (statuses.Count == 0)
        {
            AddCompactStatuses(root, "poiPresent", GuardianPoiStatus.Present, statuses);
            AddCompactStatuses(root, "poiAbsent", GuardianPoiStatus.Absent, statuses);
            AddCompactStatuses(root, "poiEmpty", GuardianPoiStatus.Empty, statuses);
        }
    ...
    private static Dictionary<string, GuardianComponentLoadout> ReadComponentMaterials(JsonElement root)
    {
    ...
    private static GuardianPointOfInterest[]? ReadRawPoints(JsonElement root)
    {
        ...
        return value.EnumerateArray().Select(GuardianSiteTemplateCatalog.ReadPointOfInterest).ToArray();
    }
    ...
    private GuardianObelisk[] ReadActiveObelisks(JsonElement root)
    {
        ...
        if (value.ValueKind == JsonValueKind.Array)
        {
            return value
                .EnumerateArray()
                .Select(element => GuardianPublishedSiteCatalog.ParseObelisk(element.GetString()))
                .ToArray();
        }
            ...
            return value
                ...
                .Select(property =>
                {
                    ...
                    return new GuardianObelisk(
                        ...
                        publishedSites.FindItemCodesByLog(GetString(old, "msg"))
                    );
                })
                .ToArray();
        }

        throw new InvalidDataException("Guardian active obelisks are neither an array nor an object.");
    }
    ...
    private static HashSet<char> ReadObeliskGroups(JsonElement root)
    {
        ...
        if (value.ValueKind == JsonValueKind.Array)
        {
            return value
                .EnumerateArray()
                .Select(element =>
                    element.ValueKind == JsonValueKind.String && element.GetString() is { Length: > 0 } item
                        ? item[0]
                        : throw new InvalidDataException("A Guardian obelisk group is invalid.")
                )
                .ToHashSet();
        }

        throw new InvalidDataException("Guardian obelisk groups are neither a string nor an array.");
    }

    private static Dictionary<DateTimeOffset, GuardianSurfaceLocation> ReadScannedLocations(JsonElement root)
    {
        ...
        foreach (var property in value.EnumerateObject())
        {
            ...
            {
                throw new InvalidDataException("A Guardian beacon scanned location is invalid.");
            }
    ...
    {
        return
            GetString(root, propertyName) is { } value
            && DateTimeOffset.TryParse(
                value,
                CultureInfo.InvariantCulture,
                DateTimeStyles.RoundtripKind,
...
public sealed record GuardianCommanderBeaconVisit(
    ...
    string Notes,
    bool Legacy,
    IReadOnlyDictionary<DateTimeOffset, GuardianSurfaceLocation> ScannedLocations
);
...
{
    public static GuardianCommanderDataReadResult Empty { get; } = new([], [], []);
}
