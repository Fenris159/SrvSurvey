...
{
    private const string EmbeddedResourceName = "SrvSurvey.Core.Resources.guardianSiteTemplates.json";

    private readonly Dictionary<string, GuardianSiteTemplate> templates;

    public GuardianSiteTemplateCatalog(IEnumerable<GuardianSiteTemplate> templates)
    {
        ArgumentNullException.ThrowIfNull(templates);
        this.templates = templates.ToDictionary(template => template.SiteType, StringComparer.OrdinalIgnoreCase);
    }

    public IReadOnlyCollection<GuardianSiteTemplate> Templates => templates.Values;

    public int Count => templates.Count;

    public GuardianSiteTemplate? Find(string? siteType)
    {
        return siteType is not null ? templates.GetValueOrDefault(siteType) : null;
    }

    public GuardianSiteTemplateCatalog WithTemplate(GuardianSiteTemplate template)
    {
        ArgumentNullException.ThrowIfNull(template);
        return new GuardianSiteTemplateCatalog(
            templates
                .Values.Where(candidate =>
                    !string.Equals(candidate.SiteType, template.SiteType, StringComparison.OrdinalIgnoreCase)
                )
                .Append(template)
        );
    }
    ...
    public static GuardianSiteTemplateCatalog Load(Stream stream)
    {
        ...
        {
            throw new InvalidDataException("The Guardian template reference is not a JSON object.");
        }
    ...
    private static GuardianPointOfInterest[] ReadPoints(JsonElement root, string propertyName)
    {
        ...
        {
            throw new InvalidDataException($"Guardian template {propertyName} is not an array.");
        }

        return value.EnumerateArray().Select(ReadPointOfInterest).ToArray();
    }

    internal static GuardianPointOfInterest ReadPointOfInterest(JsonElement value)
    {
        if (value.ValueKind != JsonValueKind.Object)
        {
    ...
    private static GuardianPoiType ReadPoiType(JsonElement root)
    {
        ...
        if (value.ValueKind == JsonValueKind.String)
        {
            ...
            if (string.Equals(name, "destructablePanel", StringComparison.OrdinalIgnoreCase))
            {
        ...
        throw new InvalidDataException("A Guardian point of interest has an unknown type.");
    }
    ...
    private static Dictionary<string, GuardianMapPoint> ReadNamedPoints(JsonElement root, string propertyName)
    {
        ...
        if (value.ValueKind != JsonValueKind.Object)
        {
            throw new InvalidDataException($"Guardian template {propertyName} is not an object.");
        }
        ...
        return value
            ...
            .ToDictionary(
                ...
                property =>
                    ParsePoint(property.Value)
                    ?? throw new InvalidDataException($"Guardian map point {property.Name} is invalid.")
            );
    }
...
)
{
    ...
    public IReadOnlyList<GuardianPointOfInterest> RelicTowers { get; } =
        PointsOfInterest.Where(point => point.Type == GuardianPoiType.Relic).ToArray();
}
