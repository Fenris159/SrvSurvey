...
public sealed class GuardianSiteMapProjector
{
    ...
    )
    {
        ...
        if (
            survey?.RawPointsOfInterest?.Any(raw =>
                ReferenceEquals(raw, point) || string.Equals(raw.Name, point.Name, StringComparison.Ordinal)
            ) == true
        )
        {
        ...
        return point.Type == GuardianPoiType.EmptyPuddle ? GuardianPoiStatus.Empty : GuardianPoiStatus.Unknown;
    }
    ...
    private static bool IsVisible(GuardianPointOfInterest point, IReadOnlySet<char>? obeliskGroups)
    {
        if (
            obeliskGroups is not { Count: > 0 }
            || point.Type is not GuardianPoiType.Obelisk and not GuardianPoiType.BrokenObelisk
...
{
    public int SurveyablePointCount =>
        Points.Count(point =>
            point.Type
    ...
    public int ConfirmedPointCount =>
        Points.Count(point =>
            point.Type
                is not GuardianPoiType.Obelisk
                    and not GuardianPoiType.BrokenObelisk
...
public sealed record GuardianProjectedGroup(string Name, double X, double Y, double Angle, double Distance);
