...
public sealed class GuardianSiteTemplateCatalogExporter
{
    ...
    internal GuardianSiteTemplateCatalogExporter(Func<string, Task> beforeActivation)
    {
    ...
    )
    {
        ...
        try
        {
            ...
            await ValidateFileAsync(temporaryPath, expectedHash, catalog, cancellationToken).ConfigureAwait(false);
            ...
            if (original is not null)
            {
                ...
                EnsureUnchanged(original, await CaptureAsync(backupPath, cancellationToken).ConfigureAwait(false));
            }
            ...
            activated = true;
            await ValidateFileAsync(targetPath, expectedHash, catalog, cancellationToken).ConfigureAwait(false);
    ...
    private static byte[] Serialize(GuardianSiteTemplateCatalog catalog)
    {
        ...
        )
        {
            var row = new JsonObject
            {
                ...
                ["obeliskGroupNameLocations"] = WriteGroups(template.ObeliskGroupNameLocations),
                ["destructablePanels"] = WritePoints(template.DestructiblePanels),
            };
    ...
    )
    {
        ...
        foreach (var template in expected.Templates)
        {
            ...
            if (
                roundTrip.PointsOfInterest.Count != template.PointsOfInterest.Count
                || roundTrip.DestructiblePanels.Count != template.DestructiblePanels.Count
                || roundTrip.ObeliskGroupNameLocations.Count != template.ObeliskGroupNameLocations.Count
            )
            {
    ...
    )
    {
        ...
        {
            throw new IOException("The Guardian template export failed and its verified backup is unavailable.");
        }
    ...
    private static void RejectReparsePoint(string path)
    {
        ...
        {
            throw new IOException("Guardian template exports cannot replace a symbolic link or reparse point.");
        }
    }
