...
{
    private static readonly IReadOnlyDictionary<string, string> SignalBuildTypes = new Dictionary<string, string>(
    ...
    private readonly HashSet<string> orbitalSignalNames = new(StringComparer.OrdinalIgnoreCase);
    ...
    )
    {
        ...
        this.nextIdentifierValue = nextIdentifierValue ?? (() => DateTimeOffset.UtcNow.ToUnixTimeMilliseconds());
    }
    ...
    public bool IsBodyScanComplete =>
        HasAllBodiesFound || (ExpectedBodyCount is > 0 && ScannedBodyCount >= ExpectedBodyCount.Value);

    public int ApplyJournalEvents(IList<ColonizationSystemSite> sites, IEnumerable<JournalEventEnvelope> journalEvents)
    {
        ArgumentNullException.ThrowIfNull(sites);
        ArgumentNullException.ThrowIfNull(journalEvents);
        return journalEvents.Count(journalEvent => ApplyJournalEvent(sites, journalEvent));
    }

    public bool ApplyJournalEvent(IList<ColonizationSystemSite> sites, JournalEventEnvelope journalEvent)
    {
        ArgumentNullException.ThrowIfNull(sites);
        ...
        switch (journalEvent.EventName)
        {
            case "FSSDiscoveryScan":
                ...
                ExpectedBodyCount = GetInt32(root, "BodyCount") ?? ExpectedBodyCount;
                return false;
            case "FSSAllBodiesFound":
                HasAllBodiesFound = true;
                ExpectedBodyCount = GetInt32(root, "Count") ?? ExpectedBodyCount;
                return false;
            case "NavBeaconScan":
    ...
    )
    {
        ...
        if (
            !captureUnknownSurfaceSite
            || name.StartsWith(SystemName, StringComparison.OrdinalIgnoreCase)
            || name.Contains("Construction site", StringComparison.OrdinalIgnoreCase)
        ...
        sites.Insert(0, CreateSite(sites, name, destination.Body, "settlement?"));
        return true;
    }
    ...
    private static string? InferDockedBuildType(JsonElement root, string? currentBuildType)
    {
        ...
        if (!string.Equals(stationType, "Outpost", StringComparison.OrdinalIgnoreCase))
        {
    ...
    private static int CountSignificantEconomies(JsonElement root)
    {
        ...
        return economies.EnumerateArray().Count(economy => GetDouble(economy, "Proportion") is >= 1);
    }
    ...
    {
        return bodyNumber >= 0 && (knownBodyNumbers.Count == 0 || knownBodyNumbers.Contains(bodyNumber));
    }

    private static int FindSiteIndex(IList<ColonizationSystemSite> sites, string? name)
    {
    ...
    {
        return !string.IsNullOrWhiteSpace(name) && !name.TrimStart().StartsWith('$');
    }
    ...
    private static bool KnownFieldsEqual(ColonizationSystemSite left, ColonizationSystemSite right)
    {
        return string.Equals(left.Id, right.Id, StringComparison.Ordinal)
            && string.Equals(left.Name, right.Name, StringComparison.Ordinal)
            && left.BodyNumber == right.BodyNumber
            && string.Equals(left.BuildType, right.BuildType, StringComparison.Ordinal)
