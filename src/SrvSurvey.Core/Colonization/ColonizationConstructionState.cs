...
public sealed class ColonizationConstructionState
{
    ...
    public ColonizationContributionSnapshot? LastContribution => lastContribution;
    ...
    public bool Apply(JournalEventEnvelope journalEvent)
    {
        ...
        var changed = journalEvent.EventName switch
        {
            ...
            "ColonisationContribution" => ApplyContribution(journalEvent.Payload, journalEvent.Timestamp),
            "ColonisationSystemClaim" => ApplyClaim(journalEvent.Payload, journalEvent.Timestamp),
            "ColonisationBeaconDeployed" => ApplyBeacon(journalEvent.Timestamp),
    ...
    private bool ApplyContribution(JsonElement root, DateTimeOffset? timestamp)
    {
        ...
        var updated = new ColonizationContributionSnapshot(timestamp, marketId.Value, contributions);
    ...
    private bool ApplyClaim(JsonElement root, DateTimeOffset? timestamp)
    {
        ...
        var updated = new ColonizationSystemClaimSnapshot(timestamp, systemAddress.Value, systemName);
    ...
    private static bool DockEquals(ColonizationDockingSnapshot left, ColonizationDockingSnapshot? right)
    {
        return right is not null
            && left.MarketId == right.MarketId
            && left.SystemAddress == right.SystemAddress
            && string.Equals(left.SystemName, right.SystemName, StringComparison.Ordinal)
            && string.Equals(left.StationName, right.StationName, StringComparison.Ordinal)
            && string.Equals(left.FactionName, right.FactionName, StringComparison.Ordinal)
            && string.Equals(left.StationType, right.StationType, StringComparison.Ordinal)
            && left.Timestamp == right.Timestamp
            && left.StationServices.SequenceEqual(right.StationServices, StringComparer.OrdinalIgnoreCase);
    }
    ...
    )
    {
        return right is not null
            && left.Timestamp == right.Timestamp
            && left.MarketId == right.MarketId
            && Math.Abs(left.ReportedProgress - right.ReportedProgress) <= 0.0000001d
...
{
    public bool IsSquadronBankOpen =>
        string.Equals(MusicTrack, "Squadrons", StringComparison.Ordinal)
        && CurrentDock?.StationServices.Contains("squadronBank", StringComparer.OrdinalIgnoreCase) == true;
}
...
)
{
    ...
    public const string ExternalPanelColonisationShip = "$EXT_PANEL_ColonisationShip";
    public const string PlanetaryConstructionSite = "Planetary Construction Site:";
    public const string OrbitalConstructionSite = "Orbital Construction Site:";
    ...
    public bool IsConstructionSite =>
        ...
        && StationServices.Contains("colonisationcontribution", StringComparer.OrdinalIgnoreCase);

    public string DefaultProjectName =>
        IsPrimaryPortShip
            ? "Primary port"
            : StationName
                .Replace(ExternalPanelColonisationShip + "; ", string.Empty, StringComparison.OrdinalIgnoreCase)
                .Replace(PlanetaryConstructionSite, string.Empty, StringComparison.OrdinalIgnoreCase)
                .Replace(OrbitalConstructionSite, string.Empty, StringComparison.OrdinalIgnoreCase)
                .Trim();
...
{
    public long TotalRequired => Resources.Sum(resource => (long)resource.RequiredAmount);

    public long TotalProvided => Resources.Sum(resource => (long)resource.ProvidedAmount);

    public long TotalRemaining => Resources.Sum(resource => (long)resource.RemainingAmount);

    public double? CalculatedProgress =>
        TotalRequired > 0 ? Math.Clamp(TotalProvided / (double)TotalRequired, 0, 1) : null;
}
...
public sealed record ColonizationSystemClaimSnapshot(DateTimeOffset? Timestamp, long SystemAddress, string SystemName);
