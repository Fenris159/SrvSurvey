...
{
    private static readonly IReadOnlyDictionary<string, string> Categories = CreateCategories();

    private static readonly IReadOnlyDictionary<string, string> DisplayNames = CreateDisplayNames();

    public static ColonizationCommodityPlan Create(ColonizationCommodityPlanRequest request)
    {
        ...
        return new ColonizationCommodityPlan
        {
            ...
            ProjectNames = relevantProjects
                .Select(project => $"{project.BuildName} ({project.BuildType})")
                .Order(StringComparer.OrdinalIgnoreCase)
                .ToArray(),
            Rows = rows,
            FleetCarriers = relevantCarriers,
            TotalRemaining = totalRemaining,
            TripsInCurrentShip = capacity > 0 ? (long?)Math.Ceiling(totalRemaining / (double)capacity) : null,
            FleetCarrierDeficit = carrierDeficit,
            FleetCarrierDeficitTrips = capacity > 0 ? (long?)Math.Ceiling(carrierDeficit / (double)capacity) : null,
    ...
    )
    {
        ...
        var marketIds = projects
            ...
            .ToHashSet();
        return carriers.Where(carrier => marketIds.Contains(carrier.MarketId)).ToArray();
    }

    private static Dictionary<string, Requirement> CreateDepotRequirements(ColonizationConstructionDepotSnapshot depot)
    {
        return depot
            .Resources.GroupBy(resource => Normalize(resource.Name))
            .ToDictionary(
                group => group.Key,
                group => new Requirement(
                    group.Sum(resource => resource.RemainingAmount),
                    group
                        .Select(resource => resource.LocalizedName)
    ...
    )
    {
        return projects
            ...
            .ToDictionary(
                group => group.Key,
                group => new Requirement(group.Sum(pair => Math.Max(0, pair.Value)), null),
                StringComparer.OrdinalIgnoreCase
            );
    }
    ...
    )
    {
        var assigners = context
            ...
            .Where(pair =>
                pair.Value.Any(assigned =>
                    string.Equals(Normalize(assigned), commodity, StringComparison.OrdinalIgnoreCase)
                )
            )
        ...
        return new ColonizationCommodityPlanRow
        {
            ...
            DisplayName = string.IsNullOrWhiteSpace(localizedName)
                ? DisplayNames.GetValueOrDefault(commodity, commodity)
                : localizedName,
    ...
    private static LocalMarketContext GetLocalMarketContext(MarketSnapshot? market, ColonizationDockingSnapshot? dock)
    {
        if (
            market is null
        ...
        return new LocalMarketContext(
            ...
            market
                .Items.Where(item => !string.IsNullOrWhiteSpace(item.LocalizedName))
                .GroupBy(item => item.Commodity, StringComparer.OrdinalIgnoreCase)
                .ToDictionary(
                    group => group.Key,
    ...
    private static Dictionary<string, string> CreateCategories()
    {
        ...
        AddCategory(result, "Chemicals", "liquidoxygen", "pesticides", "surfacestabilisers", "water");
        AddCategory(result, "Consumer items", "evacuationshelter", "survivalequipment");
        AddCategory(
            result,
            "Foods",
            "animalmeat",
        ...
        AddCategory(result, "Metals", "aluminium", "copper", "steel", "titanium");
        AddCategory(
            result,
            "Technology",
            "advancedcatalysers",
        ...
        AddCategory(result, "Weapons", "battleweapons", "nonlethalweapons", "reactivearmour");
        return result;
    }
    ...
    {
        public static LocalMarketContext Empty { get; } =
            new(
                false,
...
public sealed record ColonizationFleetCarrier
{
    ...
    public Dictionary<string, int> Cargo { get; init; } = new(StringComparer.OrdinalIgnoreCase);
}
