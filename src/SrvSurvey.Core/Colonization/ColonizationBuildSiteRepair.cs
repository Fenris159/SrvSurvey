...
{
    private const string PlanetaryConstructionPrefix = "Planetary Construction Site: ";
    private const string OrbitalConstructionPrefix = "Orbital Construction Site: ";

    private static readonly string[] PlayerColonyMarketIdPrefixes = ["395", "396", "397", "42", "43"];
    ...
    public static bool ShouldSkipDockContext(string? stationType, string? stationName, bool isConstructionShip = false)
    {
        if (!string.IsNullOrWhiteSpace(stationType) && SkippedStationTypes.Contains(stationType.Trim()))
        {
            return true;
        }
    ...
    public static string NormalizeDockStationName(string? stationName)
    {
        ...
        else if (name.StartsWith(OrbitalConstructionPrefix, StringComparison.Ordinal))
        {
    ...
    {
        return !site.HasExplicitStatus || site.Status == ColonizationSystemSiteStatus.Complete;
    }
...
)
{
    public ColonizationSystemSitePatch CreatePatch()
    {
        return Field switch
        {
            ColonizationBuildSiteRepairField.MarketId => new ColonizationSystemSitePatch { MarketId = MarketId },
            ColonizationBuildSiteRepairField.Name => new ColonizationSystemSitePatch { Name = NormalizedStationName },
            _ => throw new InvalidOperationException($"Unsupported system-site repair field '{Field}'."),
        };
    }
}
