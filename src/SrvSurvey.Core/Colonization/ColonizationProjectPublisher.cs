...
public sealed class ColonizationProjectPublisher(IRavenColonialClient client)
{
    private const int MaximumOrderCorrectionAttempts = 2;

    private readonly IRavenColonialClient client = client ?? throw new ArgumentNullException(nameof(client));
    ...
    )
    {
        ...
        {
            return new ColonizationProjectPublishResult(created, ColonizationPrimarySiteOrderStatus.NotRequired, null);
        }
    ...
    )
    {
        ...
        for (var attempt = 0; attempt < MaximumOrderCorrectionAttempts; attempt++)
        {
            ...
            if (string.Equals(orderedSiteIds[0], primarySiteId, StringComparison.Ordinal))
            {
            ...
            correctedOrder.AddRange(
                orderedSiteIds.Where(siteId => !string.Equals(siteId, primarySiteId, StringComparison.Ordinal))
            );

            await client
                .UpdateSystemSitesAsync(
                    systemKey,
                    new ColonizationSystemSiteUpdate { OrderedSiteIds = correctedOrder },
                    apiKey,
                    cancellationToken
                )
                .ConfigureAwait(false);
            correctionSent = true;
            latest = await client.GetSystemSitesAsync(systemKey, cancellationToken).ConfigureAwait(false);
        }
    ...
    private static string? GetPrimarySiteId(IReadOnlyList<ColonizationSystemSite> sites)
    {
        if (sites.Count == 0)
        {
            return null;
        ...
        return string.IsNullOrWhiteSpace(sites[0].Id) ? null : sites[0].Id.Trim();
    }

    private static List<string> GetOrderedSiteIds(IReadOnlyList<ColonizationSystemSite> sites)
    {
        if (sites.Count == 0)
        {
            throw new InvalidDataException("Raven returned no system sites after creating the project.");
        }
        ...
        foreach (var persistedId in sites.Select(site => site.Id))
        {
            ...
            {
                throw new InvalidDataException(
                    "Raven returned a system site without a persisted ID after " + "creating the project."
                );
            }
            ...
            {
                throw new InvalidDataException(
                    $"Raven returned duplicate system site ID '{id}' after creating " + "the project."
                );
            }
    ...
    )
    {
        return sites.Any(site =>
            (
                ...
                && string.Equals(site.BuildId, created.BuildId, StringComparison.OrdinalIgnoreCase)
            )
            || (
                !string.IsNullOrWhiteSpace(request.SystemSiteId)
                && string.Equals(site.Id, request.SystemSiteId, StringComparison.Ordinal)
            )
