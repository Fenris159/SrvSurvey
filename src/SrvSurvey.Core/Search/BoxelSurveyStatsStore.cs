...
public sealed class BoxelSurveyStatsStore
{
    ...
    private static readonly SearchValues<char> InvalidFileNameSearchValues = SearchValues.Create(
    ...
    )
    {
        ...
        if (File.Exists(indexPath))
        {
            try
            {
                return await ReadCatalogAsync(frontierId, indexPath, cancellationToken).ConfigureAwait(false);
            }
        ...
        return await RecoverCatalogAsync(frontierId, directory, cancellationToken).ConfigureAwait(false);
    }
    ...
    )
    {
        ...
        await SaveBoxelsAsync(frontierId, [document], cancellationToken).ConfigureAwait(false);
    }
    ...
    )
    {
        ...
        try
        {
            ...
            await WriteJsonAsync(Path.Combine(directory, IndexFileName), WriteCatalog(updated), cancellationToken)
                .ConfigureAwait(false);
        }
        finally
        {
    ...
    )
    {
        ...
        return new BoxelSurveyStatsCatalog(
            ...
            BoxelSurveyStatsCatalog.CurrentSchemaVersion,
            DateTimeOffset.UtcNow,
            entries.OrderBy(entry => entry.Prefix, StringComparer.Ordinal).ToArray()
        );
    }

    private string GetCommanderDirectory(string frontierId) => Path.Combine(rootDirectory, frontierId);
    ...
    )
    {
        ...
        {
            candidates = new[] { preferred }.Concat(
                Directory.EnumerateFiles(directory, SanitizePrefix(prefix) + "-*.json", SearchOption.TopDirectoryOnly)
            );
        }
    ...
    private static BoxelSurveyBoxelSnapshot CreateSnapshot(BoxelSurveyBoxelDocument document)
    {
    ...
    )
    {
        ...
        {
            throw new InvalidDataException("The boxel survey index belongs to a different commander profile.");
        }
    ...
    )
    {
        ...
        if (root["systems"] is JsonArray array)
        {
            foreach (var node in array.OfType<JsonObject>())
            {
                ...
                systems.Add(
                    new BoxelSurveySystemContribution(
                        generatedName,
    ...
    private static List<BoxelSurveyBodyContribution> ReadBodies(JsonObject system)
    {
        ...
        foreach (var node in array.OfType<JsonObject>())
        {
            ...
            bodies.Add(
                new BoxelSurveyBodyContribution(
                    bodyId.Value,
                    classified,
    ...
    private static JsonValue? WriteDate(DateTimeOffset? value) => value is null ? null : JsonValue.Create(value.Value);
