...
public sealed class EmptyBoxelStore : IBoxelEmptyStore
{
    ...
    public EmptyBoxelStore(string dataDirectory)
    {
        ...
        emptyBoxelDirectory = Path.Combine(Path.GetFullPath(dataDirectory), "emptyBoxels");
    }
            ...
            try
            {
                ...
                )
                {
                    await JsonSerializer
                        .SerializeAsync(stream, emptyBoxels, SerializerOptions, cancellationToken)
    ...
    public string GetFilePath(BoxelAddress boxel)
    {
        ...
        {
            throw new InvalidDataException($"The generated empty-boxel file name is invalid: {fileName}");
        }
    ...
    private static async Task<HashSet<string>> LoadFileAsync(string path, CancellationToken cancellationToken)
    {
        if (!File.Exists(path))
        {
            return [];
        ...
        try
        {
            ...
            return await JsonSerializer
                    .DeserializeAsync<HashSet<string>>(stream, cancellationToken: cancellationToken)
                    .ConfigureAwait(false)
