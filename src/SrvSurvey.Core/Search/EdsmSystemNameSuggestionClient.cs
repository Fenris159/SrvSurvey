...
public sealed class EdsmSystemNameSuggestionClient : ISystemNameSuggestionClient
{
    ...
    private static readonly Uri DefaultApiUri = new("https://www.edsm.net/api-v1/systems");
    private static readonly HttpClient SharedClient = CreateSharedClient();

    private readonly HttpClient client;
    private readonly Uri apiUri;

    public EdsmSystemNameSuggestionClient(HttpClient? client = null, Uri? apiUri = null)
    {
    ...
    )
    {
        ...
        return (payload ?? [])
            .Where(entry => entry.SystemAddress > 0 && !string.IsNullOrWhiteSpace(entry.Name) && names.Add(entry.Name))
            .Take(MaximumSuggestions)
            .Select(entry => new SystemNameSuggestion(entry.Name.Trim(), entry.SystemAddress, "EDSM"))
            .ToArray();
    }
    ...
    private sealed record EdsmSystemSuggestion(string Name, [property: JsonPropertyName("id64")] long SystemAddress);
}
