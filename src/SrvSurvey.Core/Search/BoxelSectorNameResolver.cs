...
// Ported from the legacy Boxel.Sector implementation, which in turn derives
// from EDTS's procedural galaxy naming algorithm with Cmdr Alot's permission.
internal readonly record struct SectorCoordinate(int x, int y, int z)
{
    ...
    {
        return new SectorCoordinate(left.x + right.x, left.y + right.y, left.z + right.z);
    }

    public static SectorCoordinate operator *(SectorCoordinate value, int multiplier)
    {
        return new SectorCoordinate(value.x * multiplier, value.y * multiplier, value.z * multiplier);
    }
}

internal static class BoxelSectorNameResolver
{
    public static string? GetSectorName(int x, int y, int z)
    {
    ...
    /// </summary>
    public static string? get_sector_name(SectorCoordinate pos, bool allow_ha = true)
    {
        /*
    ...
    */
    ...
    /// </summary>
    private static int _c1_get_offset(SectorCoordinate pos)
    {
    ...
    */
    ...
    private static int _c1_get_offset_from_name(List<string> frags)
    {
        ...
        catch
        {
            ...
            Debug.WriteLine("Failed to look up prefixes/suffixes in _c1_get_offset_from_name; bad sector name?");
            Debugger.Break();
            return 0;
        }
    }
    ...
    */
    ...
    private static string PascalAllWords(string value)
    {
        return string.Join(
            ' ',
            value.Split(' ').Select(
