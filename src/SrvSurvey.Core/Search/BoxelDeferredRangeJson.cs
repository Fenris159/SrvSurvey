...
{
    public static BoxelDeferredRangeSnapshot[] Read(JsonObject root, string propertyName)
    {
            ...
            result.Add(
                new JsonObject
                {
                    ...
                    ["exceptions"] = new JsonArray(
                        range.Exceptions.Distinct().Order().Select(number => JsonValue.Create(number)).ToArray()
                    ),
                }
            );
        }
    ...
    private static int[] ReadIntArray(JsonObject root, string propertyName)
    {
