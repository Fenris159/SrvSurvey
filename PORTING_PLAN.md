...
| Secondary features | Nearest systems, colonisation project forms, system notes, Commander Journeys, followed routes, and per-process commander journal selection are implemented; remaining rows are tracked in `docs/UI_PORTING_MATRIX.md` | Implemented slices are integrated into Search, Travel, and Colonisation; legacy/new frontier-ID startup options support isolated multi-commander processes, while the picker and other secondary forms remain open | Automated journal fixtures select the newest matching FID, ignore a newer foreign-commander journal, and rotate only after the requested identity arrives; see remaining per-feature rows and UI matrix | Not tested |
