...
| Human settlements | All 28 shipped templates, compatible-site filtering, pad-layout subtype inference, dock/manual-foot heading recovery, ship cockpit offsets, commander navigation, vehicle lifecycle, processed terminals, pickup locations, threat metadata, lossless system geometry, and legacy `footMatStats` persistence implemented | Passive Raven vector map with approach/docking state, map content preferences, automatic/manual/large zoom, ship/SRV/former-ship markers, 500 m and 2 km boundaries, material dots, station/survey arbitration, global map actions, and `.threat` handling implemented; authoring and quest markers remain | Reducer, geometry, storage, settings, view-model, transform, placement, XAML, and full-suite checks passed; per request, no human-site window was opened and visual/theme QA is deferred to the final pass | Not tested |
...
| Secondary features | Nearest systems, colonisation project forms, system notes, Commander Journeys, followed routes, commander profile discovery, and per-process commander journal selection are implemented; remaining rows are tracked in `docs/UI_PORTING_MATRIX.md` | Implemented slices are integrated into Search, Travel, Colonisation, and the Overview multi-commander card; legacy/new frontier-ID startup options and argument-safe process launch support isolated multi-commander processes, while next-window focus and other secondary forms remain open | Automated catalog/launcher/journal fixtures isolate malformed profiles, merge live/legacy identities, exclude the current commander, pass the selected journal folder, select the newest matching FID, ignore a newer foreign-commander journal, and rotate only after the requested identity arrives; see remaining per-feature rows and UI matrix | Not tested |
