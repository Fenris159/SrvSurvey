...
- [x] Keep the legacy in-game overlay palette independent from the Avalonia
  shell theme, preserve imported `theme.json` colours, and add categorized
  editing plus named saved states without changing the legacy file until the
...
Imported `systems/<FID>` exploration history is now activated read-only on the
first matching live system context rather than merely copied. The typed loader
accepts both the original stored parent-body shape and journal-shaped parents,
requires the expected system address, and restores missing body classes,
environment values, materials, rings, parents, organism progress, and geological
discoveries without replacing newer journal state. Loading never writes the
file; malformed histories remain byte-identical and are reported. Subsequent
normal journal persistence uses the existing unknown-field-preserving atomic
merge and the original WinForms-compatible parent shape.
...
The original no-reinstall reference delivery contract remains intact. The main
window checks the published version index once on startup; increasing a remote
catalog version downloads only the missing/newer Codex, biology-criteria,
Guardian, human-settlement, Green Gas Giant, Boxel-name, or nickname data. The
same check is available manually in Diagnostics. Candidates are now bounded,
staged, coverage/schema validated, backed up, and atomically activated before a
restart, so a bad GitHub-hosted file cannot replace the last valid or embedded
catalog.
...
Current-system body enrichment now matches the legacy EDSM/Spansh pipeline.
Both providers are requested concurrently and fail independently; address,
shape, timeout, and 16 MiB response guards reject unsafe data. Valid responses
fill only journal/profile gaps for body classes, orbital and environmental
values, materials, parents, and rings, retain the post-2022 Spansh biological
signal cutoff, and persist through the lossless legacy system file. The master
external-data switch prevents every body-enrichment request, while the separate
external-biology preference controls downloaded genus confirmations without hiding safe
signal counts. A system or consent change cancels/replaces stale work.
...
Overlay interaction now has a separate, default `Alt+Shift+O` action in the
focus-aware input catalog and an Edit Overlay Positions action beside Overlay
Behavior/Layout. The action opens a top-centre transactional workspace even
when Elite is not running: it uses the active game client when available and
otherwise uses the primary display. All 26 positionable overlays are represented
by legacy-sized, overlay-themed drag previews in five categories (exploration
and navigation, biology and surface, sites and quests, combat and colonisation,
and status and utilities), with only the selected category visible at once.
The workspace temporarily hides runtime overlays so live state cannot overwhelm
or compete with the previews. Category changes preserve unsaved moves; the
check button applies every changed legacy anchor/offset through the existing
verified backup and atomic `plotters.json` writer, while the X button, Escape,
closing the workspace, or toggling the shortcut again cancels without changing
the shared layout or disk. Hidden runtime overlays are restored afterward.
Windows and X11 retain their native passive click-through implementations for
normal runtime overlays, and the stream-composition and VR surfaces retain
their purpose-built placement behavior. Automated category coverage, anchor
conversion, no-game startup, category switching, cancel isolation, verified
commit, input-catalog, XAML contract, and compilation checks pass; live Windows
and X11 preview drag/focus checks remain in the final runtime pass.
...
- [x] Cargo, missions, massacre/foot combat, and colonization projects.
  ...
  operation is isolated so a Raven failure cannot roll back local journal
  state. The later Ravencolonial EDMC completed-site repair is ported too:
  live `Docked` and docked `Location` events with a saved commander key
  conservatively match one completed/statusless Raven site, PATCH only its
  missing/stale `marketId` or unique stale name, exclude carriers, megaships,
  construction depots/ships, active rows, non-colony market ranges, and
  ambiguous matches, retry transient site reads, and persist the latest 50
  successful `(MarketID, normalized station name)` repairs across restarts.
  Bootstrap replay, missing consent/credentials, and failed/no-match attempts
  never write; unsuccessful matches remain retryable after Raven data changes.
  The project list supports the original set/clear-primary operation explicitly.
  Fleet Carrier
...
- [x] Keep Windows publishing self-contained and multi-file. Single-file output
  is incompatible with the current per-file checksum manifest, same-volume
  staged replacement, native Avalonia/SDL/OpenVR diagnostics, and recoverable
  rollback contract.
- [x] Build the Linux AppImage from the tested, checksum-indexed `linux-x64`
  publish output. CI validates its AppDir metadata and extracts the finished
  image to verify the entry point, manifest, icon, AppStream metadata, and
  license without launching the UI. Native startup remains in the final Linux
  runtime pass.
...
| Raven shell and overlay themes | Five shell definitions plus an independent legacy-compatible overlay palette; automated coverage verifies theme persistence, named overlay states, unknown-colour retention, verified backups, picker/hex synchronization, trailing-alpha compatibility, and the no-cross-effect boundary | Five-theme application gallery and a separate categorized in-game overlay appearance submenu with named-state load/save/delete, active-theme reload, defaults, validation, an interactive alpha-capable picker and exact legacy hex field for every known or imported colour, and explicit Apply | Blue dark/light shell switching was inspected earlier; the new overlay appearance submenu, colour-picker flyouts, and live overlay recolouring intentionally await the final visual/runtime pass | Not tested |
| Settings/data migration | OS paths, legacy discovery, manifests, verified backup/staging/import/final activation with automatic rollback, pre/post-swap source and destination drift detection, immediate non-destructive legacy preference translation including all 26 overlay-scale indices, the multi-game label preference, and the Raven development-service override, retired commander/body organic-history conversion, read-only activation of imported system/body exploration history, lossless commander profile updates, lossless Guardian component-material hydration, lossless legacy local quest hydration plus verified atomic development-progress updates, the original quest opt-in, all legacy screenshot/notification preferences, first-footfall screen-inference colour/tolerance/threshold, and all 30 legacy input binding names/defaults implemented; recursive import also carries `theme.json` and `plotters.json` byte-for-byte so existing overlay colours, anchors, offsets, opacity, scale, and VR suffixes remain available on first Avalonia startup | Explicit backup-and-import workflow stops and awaits the live journal monitor before activation, translates preferences and retired organic history immediately, safely hydrates matching live system state from imported exploration files with journal data taking precedence, reports preserved malformed legacy settings/history without replacing or corrupting current Avalonia data, plus opt-in global keyboard/controller settings, validation, default restore, SDL device picker, refresh, reconnect status, global overlay-scale selector, multi-game Commander-label toggle, portable screenshot folders/banner/deletion controls, and all original in-game notification toggles; a valid imported Raven service URI consistently redirects Colonisation, Quest, and Green Gas Giant clients while malformed/non-HTTP values fail closed to production | Automated importer coverage proves recursive byte retention, malformed-file preservation, current-only merge retention, collision records, backup and activated-manifest identity, repeat-import refusal, path isolation, pre-swap abort, during-swap rollback, monitor shutdown ordering, immediate preference availability, byte-identical source/backup/retired organic files, idempotent additive system conversion, weak-ID and first-footfall repair, malformed-shape and reward-overflow refusal, byte-identical system-history reads with typed body/parent/organism/geology projection, journal-authoritative state merging, and byte-identical Guardian component surveys across source, backup, and activation before typed loading; translated overlay scale, multi-game label, Raven service, screenshot, notification, Guardian, and first-footfall inference settings preserve unknown imported section fields during later saves; overlay theme/layout stores preserve imported custom and unknown entries and create verified backups before user-requested writes; quest migration proves complete known-field loading, byte-identical read sources, verified pre-write backups, unknown-field retention, and malformed-file refusal; real profile restart comparison remains for the final runtime pass | Not tested |
...
| Overlays/input | Passive overlay contract, monitor-aware placement, all legacy global scale modes, Windows native click-through/client tracking plus reversible interactive styles, X11 XShape click-through/default-input restoration and EWMH tracking, Wayland overlay gating, multi-Elite window inventory/cycling, SharpHook keyboard input, and SDL3 controller input implemented | Guardian live map/current-obelisk, Guardian system summary, Ram Tah site guidance, human settlements, followed-route Galaxy Map, next-jump intelligence, FSS/body/system survey, high-gravity warning, biology survey, compact genetic-sampler, Canonn prior-scan/radar, surface-history/tracker radar, compact trackers, station information, ground-target, ground-combat, massacre-mission, transient notification, and the multi-game Commander label share overlay theme/layout registration, follow foreground Elite, and close on unsafe states; every detached plotter window shares native-or-forced DPI-aware scaling; `Alt+Shift+O` (customizable beside Overlay Behavior/Layout) opens or cancels a game-independent, top-centre position workspace with five selectable categories, forced legacy-sized previews, explicit check-to-save, X/Escape-to-cancel, runtime-overlay suppression/restoration, and verified atomic legacy-compatible persistence; the persisted `SrvSurveyWindowOne` stream surface clones live overlays into one game-sized capture target; all 30 legacy chords plus the new interaction action are editable and keyboard/controller events share focus-aware routing | Placement, categorized coverage of all 26 layouts, dragged-anchor conversion, no-game editor startup, category switching, transactional cancel and verified save, runtime layout replacement, scale mapping/math/persistence/migration, multi-game inventory/label gating, capability, identity, stream projection/settings/migration, route/jump/system-survey/biology/prior-scan/surface-radar/human-site/station-info/ground-target/combat/Guardian auxiliary/notification state, hook, controller lifecycle, chord tests, and explicit information-group contracts for all 22 legacy overlay designers passed; route, jump, FSS, body, system-status, biology survey, and compact sampler overlays were visually previewed in Blue dark/light, while the revised categorized interaction mode, scale rendering, and all newly ported overlay visual QA are deferred to the final UI pass; no live multi-client/Elite overlay, stream capture, or physical controller session run | Native libraries present in Linux publish; XShape input-region transitions compile against the published ABI and follow the protocol, but runtime is not tested |
...
| Colonisation projects | Shipped build costs, construction depot and music state, commander Raven profile, project creation validation, aggregate commodity planning, Cargo/Market readers, carrier inventories, corruption-safe Fleet Carrier registration plus fresh-market and live buy/sell/transfer Raven cargo reconciliation/endpoints, commander-resolving and lossless API-key persistence, system-site journal inference, targeted completed-site MarketID/name repair with persistent last-50 successful-context suppression, concurrency-safe three-way site reconciliation, multi-client-safe Cargo suppression, and bootstrap-safe live contribution/depot/completion/docking project synchronization implemented | Opt-in project workspace and creation review, depot progress, set/clear primary project, explicit current-carrier publish/link with display-name recovery and existing-cargo preservation, active-commander key validation, market/ship/FC shopping overlay with legacy Market-after-docking, construction-site, right-panel, manual, and Squadron-bank music auto-show rules, overlay preferences, credential-gated linked-carrier sync and completed-site repair, and the guarded Raven system-site editor with explicit body-import and publish confirmations implemented | Main project workspace and shopping overlay were previously checked in Blue dark; credential match/refusal/source-integrity, Fleet Carrier registration/preserve-cargo/reconciliation, event-delta/SRV/squadron/bootstrap/malformed-response rules, targeted PATCH/escaping/matching/retry/restart-cache rules, project-mutation contracts, startup suppression, untracked-site resolution, cargo clear/publish refusal, and updated lifecycle/system-editor transaction/XAML coverage pass automatically, with their visual pass intentionally deferred; no external publish was run | Not tested |
...
| Packaging/AppImage | Self-contained multi-file publish and per-file/package SHA-256 manifests configured; AppImage is built from that same tested Linux payload | Not applicable | `win-x64` publish passed | Cross-publish and manifest generation passed on Windows; CI builds and structurally extracts the standards-conformant AppImage; native runtime not tested |

Update this table only with evidence from the relevant exit gate.

## Latest validation evidence

Validation updated on 2026-07-26 using Windows build `10.0.26200` and .NET SDK
`10.0.103`:
...
- Automated overlay appearance coverage verifies legacy RGB/ARGB, HTML,
  reference, null, nested, missing, unknown, and malformed colour behavior;
  atomic active-theme writes; checksum-verified backups; named-state
  add/update/delete and corruption refusal; two-way visual-picker/legacy-hex
  synchronization including trailing alpha; and the invariant that switching
  the Avalonia application theme neither reapplies nor changes overlay colours.
- Automated overlay interaction coverage verifies the new input action and
  default, persistence with the 30 imported legacy actions, arming before any
  overlay is visible, conversion of every legacy anchor pair after a drag,
  shared runtime-layout replacement without changing scale/opacity, and XAML
  compilation. No overlay window was opened for visual testing.
- Framework-dependent `win-x64` and `linux-x64` publishes include the expected
  ...
  The Settings input cards, the 30 legacy binding editors, and the SDL device picker
  were visually/accessibility checked in the saved Blue (light) theme.
  The 31st overlay-interaction binding compiles but awaits the final visual pass.
...
- Live Windows/X11 overlay recolouring, categorized position-preview dragging,
  save/cancel controls, runtime-overlay restoration, focus retention, and
  post-restart position verification.
