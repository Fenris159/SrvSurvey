...
focus-aware input catalog and an editor beside Overlay Behavior/Layout. It
switches every registered plotter between passive click-through and clickable
position-editing modes, keeps overlays alive while they temporarily own focus,
and lets a left-button drag update the existing legacy anchor and offset rather
than creating a competing layout model. Dragged positions update the shared
runtime layout immediately and settle through the existing atomic,
checksum-backed `plotters.json` writer. Windows removes and restores only the
interaction-related extended styles and refreshes the native frame; X11 uses
an empty Shape input region for pass-through and removes that client input
region to restore the default interactive region. The stream-composition and
VR surfaces retain their purpose-built placement behavior. Automated anchor,
runtime-layout, persistence, input-catalog, and XAML compilation checks pass;
live Windows and X11 drag/focus checks remain in the final runtime pass.
...
- Live Windows/X11 overlay recolouring, click-through/edit-mode transitions,
  drag persistence, focus retention, and post-restart position verification.
