...
- [ ] Human settlements and post-processing tools. The passive human-settlement
  ...
  route/marker presentation, threat metadata, and post-processing tools remain.
...
| Human settlements | All 28 shipped templates, compatible-site filtering, pad-layout subtype inference, dock/manual-foot heading recovery, ship cockpit offsets, commander navigation, vehicle lifecycle, processed terminals, pickup locations, lossless system geometry, and legacy `footMatStats` persistence implemented | Passive Raven vector map with approach/docking state, map content preferences, automatic/manual/large zoom, ship/SRV/former-ship markers, 500 m and 2 km boundaries, material dots, station/survey arbitration, and global map actions implemented; authoring, quest markers, and threat metadata remain | Reducer, geometry, storage, settings, view-model, transform, placement, XAML, and full-suite checks passed; per request, no human-site window was opened and visual/theme QA is deferred to the final pass | Not tested |
...
- `dotnet test SrvSurvey.CrossPlatform.slnx --configuration Release --no-restore`
  passed all 740 tests: 469 Core tests and 271 Desktop tests.
