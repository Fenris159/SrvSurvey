...
- [x] Cargo, missions, massacre/foot combat, and colonization projects.
  ...
  being mislabeled as invalid credentials.
...
| Colonisation projects | Shipped build costs, construction depot and music state, commander Raven profile, project creation validation, aggregate commodity planning, Cargo/Market readers, carrier inventories, corruption-safe Fleet Carrier registration plus Raven cargo reconciliation/endpoints, commander-resolving and lossless API-key persistence, system-site journal inference, concurrency-safe three-way site reconciliation, multi-client-safe Cargo suppression, and bootstrap-safe live contribution/depot/completion/docking project synchronization implemented | Opt-in project workspace and creation review, depot progress, set/clear primary project, explicit current-carrier publish/link with display-name recovery and existing-cargo preservation, active-commander key validation, market/ship/FC shopping overlay with legacy Market-after-docking, construction-site, right-panel, manual, and Squadron-bank music auto-show rules, overlay preferences, credential-gated linked-carrier sync, and the guarded Raven system-site editor with explicit body-import and publish confirmations implemented | Main project workspace and shopping overlay were previously checked in Blue dark; credential match/refusal/source-integrity, Fleet Carrier registration/preserve-cargo/reconciliation, project-mutation contracts, startup suppression, untracked-site resolution, cargo clear/publish refusal, and updated lifecycle/system-editor transaction/XAML coverage pass automatically, with their visual pass intentionally deferred; no external publish was run | Not tested |
