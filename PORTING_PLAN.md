...
| Raven shell and overlay themes | Five shell definitions plus an independent legacy-compatible overlay palette; automated coverage verifies theme persistence, named overlay states, unknown-colour retention, verified backups, and the no-cross-effect boundary | Five-theme application gallery and a separate categorized in-game overlay appearance submenu with named-state load/save/delete, active-theme reload, defaults, validation, previews, and explicit Apply | Blue dark/light shell switching was inspected earlier; the new overlay appearance submenu and live overlay recolouring intentionally await the final visual/runtime pass | Not tested |
...
- `dotnet test SrvSurvey.CrossPlatform.slnx --configuration Release --no-build`
  passed all 1,448 tests: 820 Core tests and 628 Desktop tests.
...
- Automated overlay appearance coverage verifies legacy RGB/ARGB, HTML,
  ...
  add/update/delete and corruption refusal; and the invariant that switching
