...
| Quest communications | Read-only legacy commander development-quest state and definition loading preserves objectives, chapters, messages, variables, kept journal events, routes, tags, and unknown source fields without writes; malformed files and unsafe identities are isolated | Not implemented; Raven catalog/commander APIs, Lua runtime, journal dispatch, communications workspace, and compact indicator remain | Automated migration fixtures only; visual testing intentionally deferred | Not tested |
...
- `dotnet test SrvSurvey.CrossPlatform.slnx --configuration Release --no-restore`
  passed all 805 tests: 492 Core tests and 313 Desktop tests.
