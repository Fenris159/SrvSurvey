...
| Secondary features | Nearest systems, colonisation project forms, system notes, Commander Journeys, followed routes, commander profile discovery, and per-process commander journal selection are implemented; remaining rows are tracked in `docs/UI_PORTING_MATRIX.md` | Implemented slices are integrated into Search, Travel, Colonisation, and the Overview multi-commander card; legacy/new frontier-ID startup options, argument-safe process launch, and Windows/X11 next-window focus support isolated multi-commander processes | Automated catalog/launcher/journal fixtures isolate malformed profiles, merge live/legacy identities, exclude the current commander, pass the selected journal folder, cycle/wrap deterministic window identities, select the newest matching FID, ignore a newer foreign-commander journal, and rotate only after the requested identity arrives; see remaining per-feature rows and UI matrix | Not tested |
