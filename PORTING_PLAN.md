...
- [ ] System/body/FSS information and route overlays.
  ...
  rest of `PlotGrounded` and the remaining route/search plotter modes stay open.
...
| Organic scans | Complete 1,070-entry Codex reference, global and 42-region commander discovery ledgers, live and historical journal ingestion, Canonn Challenge import, confirmed manual overrides, discovery-location resolution, three-sample state, surface separation, first-footfall reward, sale, death, reset, compatible profile fields, current-system organism/geology history, sample-distance catalog, exact Canonn surface-POI ingestion/planning, and the complete embedded v4 prediction evaluator with region/star/nebula/Guardian context implemented | Live Overview/Exobiology projections, two-step unclaimed reset, passive `PlotBioSystem`, compact `PlotBioStatus`, single-instance `FormPredictions`, single-instance `FormShowCodex` browser with bounded image cache, single-instance `FormCodexBingo` workspace, and passive `PlotPriorScans` guidance with its Canonn grounded-radar slice are implemented | Empty/live-profile page, dense body/system predictions, the standalone predictions workspace, compact active/summary/DSS overlay states, the Codex browser with a real cached reference image, and a three-entry Codex Bingo hierarchy checked in Blue dark/light; prior-scan visual QA is intentionally deferred to the final UI pass, and active Elite sampling/attachment, interactive remote imports, and Linux were not run | Not tested |
...
| Overlays/input | Passive overlay contract, monitor-aware placement, Windows native click-through/client tracking, X11 XShape click-through/EWMH tracking, Wayland overlay gating, SharpHook keyboard input, and SDL3 controller input implemented; remaining plotters pending | Guardian live map/current-obelisk, followed-route Galaxy Map, next-jump intelligence, FSS/body/system survey, biology survey, compact genetic-sampler, Canonn prior-scan/radar, and ground-target overlays follow foreground Elite and close on unsafe states; all 30 legacy chords are editable and keyboard/controller events share focus-aware routing | Placement, capability, identity, route/jump/system-survey/biology/prior-scan/ground-target state, hook, controller lifecycle, and chord tests passed; route, jump, FSS, body, system-status, biology survey, and compact sampler overlays were visually previewed in Blue dark/light, while prior-scan and ground-target visual QA is deferred to the final UI pass; no live Elite overlay or physical controller session run | Native libraries present in Linux publish; runtime not tested |
...
- `dotnet test SrvSurvey.CrossPlatform.slnx --configuration Release --no-restore`
  passed all 619 tests: 395 Core tests and 224 Desktop tests.
